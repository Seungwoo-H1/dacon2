...
models = [
    ("MLP_SKL_D_Classifier", {"verbose": -1}),
    ("FTT_D_Classifier", {"module_d_token": 8, "module_d_ffn_factor": 1, "module_n_layers": 1, "module_n_heads": 2, "verbose": -1}),
    ("MLP_RTDL_D_Classifier", {"verbose": -1}),
]
...
for name, params in models:
    ...
    t0 = time.time()
    m = ModelClass(**params)
    print(f"  Creating model: {time.time()-t0:.1f}s")
    
    t0 = time.time()
    m.fit(X[ti], y[ti])
    print(f"  Fitting: {time.time()-t0:.1f}s")
