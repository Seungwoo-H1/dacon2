#!/usr/bin/env python3
"""
V513 — Reduced Meta Features: Mean + Std of Seed Predictions

Hypothesis: The meta learner uses 15 seed predictions as features → 15D meta space.
This high dimensionality causes meta overfitting → large gap.

Solution: Reduce meta features to just 2 features:
- mean(seed predictions)
- std(seed predictions)

This directly addresses the root cause: meta overfitting on high-dimensional
seed prediction space. If the 15 seed predictions are very similar (std small),
mean alone might be enough.

Also try: mean only (1 feature), mean + std + min + max (4 features)
"""
...
def main():
    ...
    log.info("=" * 70)
    log.info("V513 — Reduced Meta: Mean+Std of Seed Predictions (2 features)")
    log.info("Hypothesis: 15D meta → 2D meta (mean+std) reduces meta overfitting")
    log.info("=" * 70)
    ...
    # We'll test 3 meta configs: full 15, mean+std (2), mean only (1)
    # But first, let's just run the mean+std version
    ...
    for t_idx, target in enumerate(TARGETS):
        ...
        # Compute student avg OOF per seed
        ...
        # Level 1: Reduced meta features (mean + std)
        seed_array = np.column_stack(per_seed_oofs)
        reduced_meta = np.column_stack([
            np.mean(seed_array, axis=1),   # mean
            np.std(seed_array, axis=1),     # std
        ])
        
        meta = LogisticRegression(C=META_C, max_iter=2000, random_state=SEED)
        meta.fit(reduced_meta, y)
        
        train_oof[target] = meta.predict_proba(reduced_meta)[:, 1]
        ll = log_loss(y, np.clip(train_oof[target], 0.001, 0.999))
        log.info(f"    {target} Reduced Meta OOF (2 features: mean+std, C={META_C}): {ll:.5f}")
        
        test_meta = np.column_stack([
            np.mean(per_seed_test_preds, axis=1),
            np.std(per_seed_test_preds, axis=1),
        ])
        test_preds[target] = meta.predict_proba(test_meta)[:, 1]
    ...
    log.info(f"\n{'='*70}")
    log.info("V513 RESULTS (Reduced Meta: Mean+Std)")
    log.info(f"{'='*70}")
    ...
    sub_path = SUBMIT / f'submission_v513_reduced_meta_{timestamp}.csv'
    ...
    result = {
        'version': 'V513',
        'name': 'Reduced Meta: Mean+Std of Seed Predictions (2 features)',
    ...
    result_path = EXPERIMENTS / f'v513_{timestamp}.json'
