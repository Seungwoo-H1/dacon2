#!/usr/bin/env python3
"""
V526 submission regeneration — best config from partial run

V526 best: xgb_strong_n5
- avg_gap=0.01595, vs308=7/7
- S1 gap=0.00431 (best ever!)
- equal_mix (0.5 XGB + 0.5 LGBM)
...
def main():
    t_start = time.time()
    log.info("V526-SUBMIT — Regenerate V526 best config submission")
    ...
    log.info("Training all targets with equal_mix (0.5 XGB + 0.5 LGBM)...")
    ...
    for ti, target in enumerate(TARGETS):
        ...
        log.info(f"  {target}: OOF_meta={_: .5f}, gap={gap:.5f} (elapsed: {time.time()-t_start:.0f}s)")
    ...
    log.info(f"\n{'='*60}")
    log.info(f"V526 BEST: avg_gap={avg_gap:.5f}, vs308={vs308}/7")
    for t in TARGETS:
    ...
    sub_path = SUBMIT / f'submission_v526_s1_n5_equal_{ts}.csv'
    ...
    result = {
        'version': 'V526',
        'name': 'S1 n_feat=5 + equal mix (0.5 XGB + 0.5 LGBM)',
        'config': 'Q1=7,q_narrow / Q2=14,q_deep / Q3=7,q_strong / S1=5,q_strong / S2=7,s_wide / S3=23,q_strong / S4=20,q_deep',
        'avg_gap': float(avg_gap),
        'target_gaps': {t: float(g) for t, g in target_gaps.items()},
        'vs308': vs308,
        'timestamp': ts,
        'total_time_s': round(time.time() - t_start, 1),
    }
    result_path = EXPERIMENTS / f'v526_s1n5_{result["timestamp"]}.json'
