...
V308 was the ONLY verified model that beat baseline with small gap.
Gap analysis: V308 gap=0.01658, V496 gap=0.06629 (4x worse)
The answer is NOT lower OOF, it's SMALLER GAP.

Changes from V308:
1. 20 seeds instead of 15 (more diversity for better stacking)
2. Stronger meta regularization C=5 instead of C=10
3. Same V308 feature selection (V53_SWEEP)
4. Same CFGS
5. Same z-score generation (matching V308 exactly)
6. Same GroupKFold 5-fold
"""
...
SEED = 42
N_FOLDS = 5
N_SEEDS = 20  # V308 was 15
META_C = 5.0  # Stronger regularization than V308's C=10
...
def rank_features(feat_df, feat_cols, target, seed=SEED):
    """Rank features by LGBM gain importance — EXACTLY as V308."""
...
def generate_test_zscore(train_df, test_df):
    """EXACTLY as V308 — generate z-scores for test using train stats."""
...
def main():
    ...
    log.info("=" * 70)
    log.info("V505 — V308 Exact + 20 Seeds + Stronger Meta Reg")
    log.info("V308: OOF=0.62235, LB=0.63893, gap=0.01658")
    log.info("V496: OOF=0.6095,  LB=0.67579, gap=0.06629 (4x worse)")
    log.info("Strategy: Keep V308 architecture, 20 seeds, C=5")
    log.info("=" * 70)
    ...
    # Generate z-score for test (V308 method)
    test_df, zscore_cols = generate_test_zscore(train_df, test_df)
    
    # Also add z-scores to train (self-referential)
    ...
    all_seed_oofs = []  # store all per-seed OOFs for gap analysis
    ...
    for t_idx, target in enumerate(TARGETS):
        ...
        # Feature ranking — EXACTLY V308 method
        ...
        if len(sel_cols_test) != len(sel_cols):
            ...
            log.warning(f"    {target}: {len(missing)} features missing in test: {missing}")
        ...
        for si in range(N_SEEDS):
            seed = SEED + si * 7  # Same seed spacing as V308
        ...
        # Level 1: Stack → LR meta-learner with C=5
        ...
        all_seed_oofs.append(per_seed_oofs)
    ...
    avg_gap = np.mean([np.mean([log_loss(train_df[t].values, so) for so in all_seed_oofs[ti]]) - per_target_oof[t]
                       for ti, t in enumerate(TARGETS)])
    log.info(f"  AVG student-meta gap: {avg_gap:.5f}")
    ...
    for t in TARGETS:
        ...
        meta_t.fit(np.column_stack([all_seed_oofs[ti][i] for i in range(N_SEEDS)]), y_t)
    ...
    sub_path = SUBMIT / f"submission_v505_gap_constrained_{ts}.csv"
    sub.to_csv(sub_path, index=False)
    log.info(f"\nSaved submission: {sub_path}")
    
    meta_data = {
        'version': 'V505',
        'name': f'V308 Exact + {N_SEEDS} seeds + meta C={META_C}',
        ...
        'avg_student_meta_gap': round(float(avg_gap), 5),
        'submission_file': str(sub_path),
        'timestamp': ts,
        'total_time_s': round(time.time() - t_start, 0),
    }
    
    meta_path = EXPERIMENTS / f'v505_{ts}.json'
