#!/usr/bin/env python3
"""
V520 — Full production: Q1=7 + Q2=14 + Q3=7 + Q4=XGB + S1=10 + S2=19 + S3=23 + S4=20
Best from V519: Q3=Q3_7 (n_feat=7, q_strong) + S1=S1_10 (n_feat=10, wide)
Tests both 15D and 2D meta.

Expected: avg_gap ~0.042, significantly better than V518's 0.048
"""
...
V520_CONFIGS = {
    ...
    'Q3':  {'n_feat': 7,  'learner': 'xgb', 'cfg_name': 'q_strong'},
    'S1':  {'n_feat': 10, 'learner': 'lgbm', 'cfg_name': 'wide'},
...
def main():
    ...
    log.info("=" * 70)
    log.info("V520 — Full: Q1=7 + Q2=14 + Q3=7 + S1=10 + S2=19 + S3=23 + S4=20")
    log.info("Best from V519: Q3=7 (strong) + S1=10 (wide), both 15D and 2D meta")
    log.info("=" * 70)
    ...
    for t_idx, target in enumerate(TARGETS):
        ...
        # Meta: 15 seed predictions → LR (15D)
        ...
        log.info(f"    {target} Stacking OOF (15D, C={META_C}): {ll:.5f}")
    ...
    # Now compute 2D meta results
    log.info(f"\n{'='*70}")
    log.info("COMPUTING 2D META (Mean+Std) RESULTS")
    log.info(f"{'='*70}")
    
    # 2D meta: mean + std of seed predictions
    train_oof_2d = {t: np.zeros(n_train) for t in TARGETS}
    test_preds_2d = {t: np.zeros(n_test) for t in TARGETS}
    
    for t in TARGETS:
        oofs_arr = np.column_stack(all_seed_oofs[t])  # (n_train, 15)
        test_arr = np.column_stack(all_seed_test[t])   # (n_test, 15)
        
        avg_pred = np.mean(oofs_arr, axis=1)  # (n_train,)
        std_pred = np.std(oofs_arr, axis=1)    # (n_train,)
        test_avg = np.mean(test_arr, axis=1)
        test_std = np.std(test_arr, axis=1)
        ...
        train_oof_2d[t] = meta_2d.predict_proba(np.column_stack([avg_pred, std_pred]))[:, 1]
        test_preds_2d[t] = meta_2d.predict_proba(np.column_stack([test_avg, test_std]))[:, 1]
        
        ll = log_loss(train_df[t].values, np.clip(train_oof_2d[t], 0.001, 0.999))
        log.info(f"  {t} 2D meta: OOF={ll:.5f}")
    ...
    avg_gap_15d = 0
    for t in TARGETS:
        per_target_oof_15d[t] = log_loss(train_df[t].values, np.clip(train_oof_15d[t], 0.001, 0.999)) if False else log_loss(train_df[t].values, np.clip(train_oof[t], 0.001, 0.999))
        t_y = train_df[t].values
        t_student_lls = [log_loss(t_y, so) for so in all_seed_oofs[t]]
        meta_ll_15d = per_target_oof_15d[t]
        avg_student = np.mean(t_student_lls)
        gap = avg_student - meta_ll_15d
        per_target_gap_15d[t] = gap
        avg_oof_15d += meta_ll_15d
        avg_gap_15d += gap
        t_seed_std = np.std(t_student_lls)
        log.info(f"  [15D] {t}: OOF={meta_ll_15d:.5f} gap={gap:.5f} student={avg_student:.5f} std={t_seed_std:.5f} vs V308 {v308_gaps[t]:.3f} {'✅' if gap < v308_gaps[t] else '❌'}")
    
    avg_oof_15d /= len(TARGETS)
    avg_gap_15d /= len(TARGETS)
    ...
    avg_gap_2d = 0
    for t in TARGETS:
        per_target_oof_2d[t] = log_loss(train_df[t].values, np.clip(train_oof_2d[t], 0.001, 0.999))
        t_y = train_df[t].values
        t_student_lls = [log_loss(t_y, so) for so in all_seed_oofs[t]]
        meta_ll_2d = per_target_oof_2d[t]
        avg_student = np.mean(t_student_lls)
        gap = avg_student - meta_ll_2d
        per_target_gap_2d[t] = gap
        avg_oof_2d += meta_ll_2d
        avg_gap_2d += gap
        t_seed_std = np.std(t_student_lls)
        log.info(f"  [2D]  {t}: OOF={meta_ll_2d:.5f} gap={gap:.5f} student={avg_student:.5f} std={t_seed_std:.5f} vs V308 {v308_gaps[t]:.3f} {'✅' if gap < v308_gaps[t] else '❌'}")
    
    avg_oof_2d /= len(TARGETS)
    avg_gap_2d /= len(TARGETS)
    
    log.info(f"\n{'='*70}")
    log.info("COMPARISON")
    log.info(f"{'='*70}")
    log.info(f"  15D: OOF={avg_oof_15d:.5f}  Gap={avg_gap_15d:.5f}")
    log.info(f"  2D:  OOF={avg_oof_2d:.5f}  Gap={avg_gap_2d:.5f}")
    ...
    sub_path = SUBMIT / f'submission_v520_optimal_cfg_{timestamp}.csv'
    sub_df.to_csv(sub_path, index=False)
    log.info(f"\n  ✅ Submission: {sub_path}")
    
    result = {
        'version': 'V520',
        'name': 'Q1=7 + Q2=14 + Q3=7(strong) + S1=10(wide) + S2=19 + S3=23 + S4=20',
        ...
        'per_target_gap_15d': {k: float(v) for k, v in per_target_gap_15d.items()},
        'per_target_oof_2d': {k: float(v) for k, v in per_target_oof_2d.items()},
        'per_target_gap_2d': {k: float(v) for k, v in per_target_gap_2d.items()},
        'submission_file': str(sub_path),
        'timestamp': timestamp,
        'total_time_s': round(time.time() - t_start, 1),
    }
    
    result_path = EXPERIMENTS / f'v520_{timestamp}.json'
