#!/usr/bin/env python3
"""
V503 — V496 + Target-Specific HP (XGB+CB only, NO LGBM)
Changes from V496:
1. Feature ranking: XGB (no LGBM)
2. Training: XGB + CB only (no LGBM to avoid libgomp issue)
3. Target-specific HP: rate>0.60 → more regularization
"""
...
def main():
    t_start = time.time()
    log.info("V503 — V496 + Target-Specific HP (XGB+CB only, NO LGBM)")
    ...
    # 3. Feature ranking (XGB-based, 20 rounds)
    log.info("\n--- 3. Feature ranking (XGB) ---")
    predictions = {}
    target_results = {}
    ...
    for target in TARGETS:
        ...
        # Quick ranking: XGB 20 rounds
        log.info("  Ranking (XGB 20 rounds)...")
        t0 = time.time()
        dtrain_rank = xgb.DMatrix(X_train, label=y)
        dval_rank = xgb.DMatrix(X_train[:100], label=y[:100])
        mr = xgb.train({'objective': 'binary:logistic', 'eval_metric': 'logloss', 
                        'max_depth': 4, 'eta': 0.05, 'n_estimators': 20},
                       dtrain_rank, num_boost_round=20,
                       evals=[(dval_rank, 'val')], verbose_eval=False, early_stopping_rounds=5)
        ...
        log.info(f"  Rank done ({time.time()-t0:.1f}s)")
        ...
        # Per-target HP
        if train[target].mean() > 0.60:
            xgb_cfg = {'max_depth': 3, 'eta': 0.015, 'n_estimators': 400,
                       'subsample': 0.6, 'colsample_bytree': 0.5, 'alpha': 3.0,
                       'lambda': 8.0, 'min_child_weight': 5, 'random_state': 42}
            cb_cfg = {'iterations': 400, 'learning_rate': 0.015, 'depth': 3,
                      'subsample': 0.6, 'colsample_bylevel': 0.5, 'l2_leaf_reg': 8.0,
                      'min_data_in_leaf': 15, 'random_state': 42,
                      'loss_function': 'Logloss', 'verbose': False,
                      'use_best_model': True}
        else:
            xgb_cfg = {'max_depth': 4, 'eta': 0.02, 'n_estimators': 400,
                       'subsample': 0.7, 'colsample_bytree': 0.7, 'alpha': 2.0,
                       'lambda': 5.0, 'min_child_weight': 3, 'random_state': 42}
            cb_cfg = {'iterations': 400, 'learning_rate': 0.02, 'depth': 4,
                      'subsample': 0.7, 'colsample_bylevel': 0.7, 'l2_leaf_reg': 5.0,
                      'min_data_in_leaf': 10, 'random_state': 42,
                      'loss_function': 'Logloss', 'verbose': False,
                      'use_best_model': True}

        # OOF evaluation (seed=42)
        log.info("  OOF evaluation...")
        ...
        for fold, (tr, va) in enumerate(gkf.split(X_top, y, groups)):
            ...
            mx = xgb.train(xgb_cfg, dtrain, num_boost_round=400,
                           evals=[(dval, 'val')], early_stopping_rounds=50, verbose_eval=False)
            oof_xgb[va] = mx.predict(dval)
            
            mcb = cb.CatBoostClassifier(**cb_cfg)
            mcb.fit(X_top[tr], y[tr], eval_set=(X_top[va], y[va]))
            oof_cb[va] = mcb.predict_proba(X_top[va])[:, 1]

        oof_avg = (oof_xgb + oof_cb) / 2.0
        avg_oof = logloss(y, oof_avg)
        log.info(f"  OOF: XGB={logloss(y, oof_xgb):.4f}, CB={logloss(y, oof_cb):.4f}, AVG={avg_oof:.4f}")
        ...
        for seed in SEEDS:
            ...
            for fold, (tr, va) in enumerate(gkf.split(X_top, y, groups)):
                ...
                mx = xgb.train(cfg_x, dtrain, num_boost_round=400, verbose_eval=False)
                final_xgb += mx.predict(dtest) / n_div
            
            for fold, (tr, va) in enumerate(gkf.split(X_top, y, groups)):
                mcb = cb.CatBoostClassifier(**cfg_c)
                mcb.fit(X_top[tr], y[tr], verbose=False)
                final_cb += mcb.predict_proba(X_test_top)[:, 1] / n_div

        test_preds = (final_xgb + final_cb) / 2.0
        predictions[target] = np.clip(test_preds, 0.0001, 0.9999)

        target_results[target] = {
            'n_feat': N_FEAT, 'avg_oof': float(avg_oof),
            'xgb_oof': float(logloss(y, oof_xgb)),
            'cb_oof': float(logloss(y, oof_cb)),
            'time': time.time() - t1,
        }
        log.info(f"  {target}: AVG={avg_oof:.4f}, T={time.time()-t1:.0f}s")
        gc.collect()

    # Summary
    avg_oof = np.mean([v['avg_oof'] for v in target_results.values()])
    log.info(f"\n{'='*70}")
    log.info("V503 RESULTS")
    log.info(f"{'='*70}")
    for t in TARGETS:
        r = target_results[t]
        log.info(f"  {t}: XGB={r['xgb_oof']:.4f}, CB={r['cb_oof']:.4f}, AVG={r['avg_oof']:.4f}, T={r['time']:.0f}s")
    ...
    for t in TARGETS:
        sub[t] = predictions[t]
    sub_path = SUBMIT / f"submission_v503_no_lgbm_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
