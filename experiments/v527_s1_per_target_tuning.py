#!/usr/bin/env python3
"""
V527 — V526 + S1 n_feat=3,4 + per-target n_feat tuning

V526: avg_gap=0.01595, 7/7 vs308
- S1 n_feat=5 optimal (gap 0.00431)
- Try S1 n_feat=3, 4 to see if even smaller is better
- Also try per-target n_feat variations for Q1, S4 (largest remaining gaps after S1/S3)

Baseline configs (from V526):
  Q1: n_feat=7, xgb=q_narrow, lgbm=wide, n_est=600
  Q2: n_feat=14, xgb=q_deep, lgbm=wide, n_est=800
  Q3: n_feat=7, xgb=q_strong, lgbm=safety, n_est=500
  S1: n_feat=5, xgb=q_strong, lgbm=wide, n_est=500
  S2: n_feat=7, xgb=s_wide, lgbm=wide, n_est=300
...
def main():
    t_start = time.time()
    log.info("=" * 70)
    log.info("V527 — S1 n_feat=3,4 + per-target tuning")
    log.info("=" * 70)
    ...
    BASELINE = {
        ...
        'S1':  {'n_feat': 5,  'xgb_cfg': 'q_strong',  'lgbm_cfg': 'wide',   'n_est': 500},
    ...
    # Configs to test
    configs = {
        'S1_n3': dict(BASELINE, **{'S1': {'n_feat': 3, 'xgb_cfg': 'q_strong', 'lgbm_cfg': 'wide', 'n_est': 500}}),
        'S1_n4': dict(BASELINE, **{'S1': {'n_feat': 4, 'xgb_cfg': 'q_strong', 'lgbm_cfg': 'wide', 'n_est': 500}}),
        'S1_n5': dict(BASELINE, **{'S1': {'n_feat': 5, 'xgb_cfg': 'q_strong', 'lgbm_cfg': 'wide', 'n_est': 500}}),
        'S1_n6': dict(BASELINE, **{'S1': {'n_feat': 6, 'xgb_cfg': 'q_strong', 'lgbm_cfg': 'wide', 'n_est': 500}}),
        # Try Q1 n_feat=5 (smaller)
        'Q1_n5_S1_n5': {**BASELINE, 'Q1': {'n_feat': 5, 'xgb_cfg': 'q_narrow', 'lgbm_cfg': 'wide', 'n_est': 600}},
        # Try S4 n_feat=15
        'S4_n15_S1_n5': {**BASELINE, 'S4': {'n_feat': 15, 'xgb_cfg': 'q_deep', 'lgbm_cfg': 'wide', 'n_est': 300}},
        # Both Q1=5 + S4=15
        'Q1_n5_S4_n15_S1_n5': {**BASELINE, 'Q1': {'n_feat': 5, 'xgb_cfg': 'q_narrow', 'lgbm_cfg': 'wide', 'n_est': 600}, 'S4': {'n_feat': 15, 'xgb_cfg': 'q_deep', 'lgbm_cfg': 'wide', 'n_est': 300}},
    }
    ...
    for r in sorted(all_results, key=lambda x: x['avg_gap']):
        marker = ""
        if r['avg_gap'] < 0.015: marker = " 🎯🎯🎯"
        elif r['avg_gap'] < 0.017: marker = " ⭐⭐⭐"
        elif r['avg_gap'] < 0.020: marker = " ⭐⭐"
    ...
    result = {
        'version': 'V527',
        'name': 'S1 n_feat 3-6 + per-target tuning',
