"""
V500 — Per-Subject Normalization + 10 Seeds + 3-Model Ensemble (Faster)
...
def main():
    t_start = time.time()
    log.info("=" * 70)
    log.info("V500 — Per-Subject Normalization + 10 Seeds + 3-Model (Optimized)")
    log.info("=" * 70)
    ...
    for target in TARGETS:
        ...
        for n_feat in [20, 30, 40, 50, 60, 80]:
            ...
            lgb_cfg = {
                ...
                'n_estimators': 400, 'subsample': 0.7, 'colsample_bytree': 0.7,
            ...
            # CB config
            cb_cfg = {
                'iterations': 400, 'learning_rate': 0.02,
        ...
        for seed in [42, 123, 456, 789, 101, 202, 303, 404, 505, 606]:  # 10 seeds for more diversity
            # LGBM
            lgb_cfg_s = {**lgb_cfg, 'random_state': seed, 'n_estimators': 300}
            for fold, (tr, va) in enumerate(gkf.split(X_top_all, y, groups)):
                ...
                final_lgb_preds += m.predict(X_test_top_all) / (10 * 5)
            
            # XGB
            xgb_cfg_s = {**xgb_cfg, 'random_state': seed, 'n_estimators': 300}
            for fold, (tr, va) in enumerate(gkf.split(X_top_all, y, groups)):
                m = xgb.XGBClassifier(**xgb_cfg_s)
                m.fit(X_top_all[tr], y[tr], verbose=False)
                final_xgb_preds += m.predict_proba(X_test_top_all)[:, 1] / (10 * 5)
            
            # CB
            cb_cfg_s = {**cb_cfg, 'random_state': seed, 'iterations': 300}
            for fold, (tr, va) in enumerate(gkf.split(X_top_all, y, groups)):
                m = cb.CatBoostClassifier(**cb_cfg_s)
                m.fit(X_top_all[tr], y[tr], verbose=False)
                final_cb_preds += m.predict_proba(X_test_top_all)[:, 1] / (10 * 5)
    ...
    for t in TARGETS:
        sub[t] = predictions[t]
    sub_path = SUBMIT / f"submission_v500_more_seeds_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
    sub.to_csv(sub_path, index=False)
    log.info(f"  Submission: {sub_path}")

    # ── Save meta ──
    meta = {
        'version': 'V500_more_seeds',
        'name': 'Per-Subject Z-Score + 3-Model Ensemble (10 seeds, n_est=400)',
