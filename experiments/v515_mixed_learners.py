#!/usr/bin/env python3
"""
V515 — Mixed Base Learners: XGB for Q targets, LGBM for S targets

Hypothesis: Q targets have high seed variance (0.008-0.015 std) while S targets
have low variance (0.002-0.006). Different algorithms may handle Q targets better.

Try:
- Q targets: XGB instead of LGBM (different inductive bias, may reduce variance)
- S targets: LGBM (already low variance, no change needed)

This addresses: "Different base learner for Q targets" (hypothesis #4)
"""
...
XGB_CFGS = {
    ...
    'q_narrow': {'max_depth': 3, 'learning_rate': 0.05, 'n_estimators': 500,
                'subsample': 0.8, 'colsample_bytree': 0.8, 'reg_alpha': 2.0, 'reg_lambda': 5.0,
                'min_child_weight': 3, 'random_state': 42},
}

V53_SWEEP = {
    'Q1':  {'cfg': 'deep',   'n_feat': 19, 'learner': 'xgb',  'xgb_cfg': 'q_deep'},
    'Q2':  {'cfg': 'deep',   'n_feat': 14, 'learner': 'xgb',  'xgb_cfg': 'q_deep'},
    'Q3':  {'cfg': 'v48',    'n_feat': 11, 'learner': 'xgb',  'xgb_cfg': 'q_narrow'},
    'S1':  {'cfg': 'wide',   'n_feat': 21, 'learner': 'lgbm', 'xgb_cfg': None},
    'S2':  {'cfg': 'deep',   'n_feat': 19, 'learner': 'lgbm', 'xgb_cfg': None},
    'S3':  {'cfg': 'safety', 'n_feat': 23, 'learner': 'lgbm', 'xgb_cfg': None},
    'S4':  {'cfg': 'wide',   'n_feat': 20, 'learner': 'lgbm', 'xgb_cfg': None},
}
...
def train_lgbm(X_tr, y_tr, X_va, sel_cols, cfg, seed, n_est):
...
def train_xgb(X_tr, y_tr, X_va, sel_cols, cfg, seed):
    import xgboost as xgb
    params = {
...
def main():
    ...
    log.info("=" * 70)
    log.info("V515 — Mixed Base Learners: XGB for Q, LGBM for S")
    log.info("Hypothesis: XGB may reduce Q target seed variance vs LGBM")
    log.info("=" * 70)
    ...
    for t_idx, target in enumerate(TARGETS):
        log.info(f"\n{'='*60}")
        log.info(f"Target: {target} (rate={train_df[target].mean():.3f}, learner={V53_SWEEP[target]['learner']})")
        y = train_df[target].values.astype(np.float64)
        feat_cols_clean = remove_leak(train_feat_cols, target)
        
        n_feat = V53_SWEEP[target]['n_feat']
        cfg_name = V53_SWEEP[target]['cfg']
        learner = V53_SWEEP[target]['learner']
        ...
        is_xgb = (learner == 'xgb')
        cfg_name_actual = V53_SWEEP[target]['xgb_cfg'] if is_xgb else cfg_name
        
        log.info(f"    Config: {cfg_name_actual}, n_feat: {n_feat}, n_sel: {len(sel_cols)}, learner: {learner}")
        ...
        for si in range(N_SEEDS):
            ...
            for fold, (tr_idx, va_idx) in enumerate(gkf.split(train_df, y, group)):
                ...
                if is_xgb:
                    cfg = XGB_CFGS[cfg_name_actual]
                    m = train_xgb(X_tr, y_tr, X_va, sel_cols, cfg, seed)
                    ...
                    cfg = LGBM_CFGS[cfg_name_actual]
                    m = train_lgbm(X_tr, y_tr, X_va, sel_cols, cfg, seed, cfg['n_estimators'])
    ...
    log.info(f"\n{'='*70}")
    log.info("V515 RESULTS (Mixed Base Learners: XGB for Q, LGBM for S)")
    log.info(f"{'='*70}")
    ...
    for t in TARGETS:
        ...
        learner = V53_SWEEP[t]['learner']
        log.info(f"  {t}: OOF={t_meta_ll:.5f} gap={t_gap:.5f} student_ll={np.mean(t_student_lls):.5f} seed_std={t_seed_std:.5f} learner={learner} vs V308 {v308_gap:.3f} {status}")
    ...
    sub_path = SUBMIT / f'submission_v515_mixed_learners_{timestamp}.csv'
    sub_df.to_csv(sub_path, index=False)
    log.info(f"\n  ✅ Submission: {sub_path}")
    
    result = {
        'version': 'V515',
        'name': 'Mixed Base Learners: XGB for Q, LGBM for S',
        ...
        'learner_config': {t: V53_SWEEP[t]['learner'] for t in TARGETS},
        'n_seeds': N_SEEDS,
        'meta_c': META_C,
    ...
    result_path = EXPERIMENTS / f'v515_{timestamp}.json'
