#!/usr/bin/env python3
"""
V522 — Full production: BEST CONFIG FOUND!

Q1=7(xgb/narrow) + Q2=14(xgb/deep) + Q3=7(xgb/strong) + S1=10(lgbm/wide) 
+ S2=7(xgb/s_wide) + S3=23(lgbm/safety) + S4=20(lgbm/wide)

Tests both 15D and 2D meta. V521 found avg_gap=0.02550 with 2D meta.

This is the closest we've been to the 0.025 gap target!
"""
...
V522_CONFIGS = {
    ...
    'S2':  {'n_feat': 7,  'learner': 'xgb', 'cfg_name': 's_wide'},
...
def main():
    ...
    log.info("=" * 70)
    log.info("V522 — PRODUCTION BEST CONFIG")
    log.info("Q1=7 + Q2=14 + Q3=7 + S1=10 + S2=7(XGB) + S3=23 + S4=20")
    log.info("Expected avg_gap < 0.026 (V521: 0.0255)")
    log.info("=" * 70)
    ...
    for t in TARGETS:
        ...
        test_arr = np.column_stack(all_seed_test[t])
    ...
    avg_gap_15d = 0
    for t in TARGETS:
        meta_ll = log_loss(train_df[t].values, np.clip(train_oof[t], 0.001, 0.999))
        ...
        per_target_oof_15d[t] = meta_ll
        per_target_gap_15d[t] = gap
        avg_oof_15d += meta_ll
        avg_gap_15d += gap
        t_seed_std = np.std(t_student_lls)
        log.info(f"  [15D] {t}: OOF={meta_ll:.5f} gap={gap:.5f} student={avg_student:.5f} std={t_seed_std:.5f} vs V308 {v308_gaps[t]:.3f} {'✅' if gap < v308_gaps[t] else '❌'}")
    ...
    avg_gap_2d = 0
    for t in TARGETS:
        meta_ll = log_loss(train_df[t].values, np.clip(train_oof_2d[t], 0.001, 0.999))
        t_y = train_df[t].values
        t_student_lls = [log_loss(t_y, so) for so in all_seed_oofs[t]]
        avg_student = np.mean(t_student_lls)
        gap = avg_student - meta_ll
        per_target_oof_2d[t] = meta_ll
        per_target_gap_2d[t] = gap
        avg_oof_2d += meta_ll
        avg_gap_2d += gap
        t_seed_std = np.std(t_student_lls)
        log.info(f"  [2D]  {t}: OOF={meta_ll:.5f} gap={gap:.5f} student={avg_student:.5f} std={t_seed_std:.5f} vs V308 {v308_gaps[t]:.3f} {'✅' if gap < v308_gaps[t] else '❌'}")
    ...
    best_test_preds = test_preds if best_meta == '15D' else test_preds_2d
    ...
    if best_avg_gap < 0.025:
        log.info("  🎯🎯🎯 GAP TARGET HIT! (< 0.025) 🎯🎯🎯")
    elif best_avg_gap < 0.030:
        log.info("  🎯 Close to gap target! (< 0.030)")
    else:
    ...
    sub_path = SUBMIT / f'submission_v522_production_{timestamp}.csv'
    sub_df.to_csv(sub_path, index=False)
    log.info(f"\n  ✅ Submission: {sub_path}")
    
    result = {
        'version': 'V522',
        'name': 'Q1=7 + Q2=14 + Q3=7 + S1=10 + S2=7(XGB) + S3=23 + S4=20',
    ...
    result_path = EXPERIMENTS / f'v522_{timestamp}.json'
