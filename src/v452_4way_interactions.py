"""
V452 — V446 Baseline + Feature Expansion (4-way interactions) + Refined Ranking

Hypothesis: V446(V339 0.589)에서 student OOF 0.601. 더 낮추려면:
1. 4-way baseline interactions: bl²×z, bl×z² 등 higher-order baseline features
2. Per-subject z-score distribution stats: skewness, kurtosis per subject per feature
3. Better feature ranking: rank by median importance across 10 fold ranks
4. 20 seeds is enough but 40 might help slightly

Key: Keep V446 meta config (reg_alpha=0.01, reg_lambda=0.0) → gap ~1.22x
"""
...
def main():
    ...
    log.info("=" * 70)
    log.info("V452 — 4-Way Interactions + Refined Ranking")
    log.info("=" * 70)
    ...
    log.info("  Adding 4-way interaction features...")
    # z³, bl×z², bl²×z for key features (not all to keep size manageable)
    # Key features: top 50 most common features
    for t in TARGETS:
        train_bl = np.array([baselines[t][sid] for sid in train_subjects])
        test_bl = np.array([baselines[t][sid] for sid in test_df['subject_id'].values])
        for col in num_cols:
    ...
    for t_idx, target in enumerate(TARGETS):
        ...
        # Rank features 10 times for better stability
        fold_ranks = []
        ...
        for fl in fold_ranks:
            for rank, f in enumerate(fl):
                if f not in all_ranks:
                    all_ranks[f] = []
                all_ranks[f].append(rank)
        median_ranks = {f: np.median(r) for f, r in all_ranks.items()}
        ranked_features = sorted(median_ranks.items(), key=lambda x: x[1])
    ...
    for t_idx, target in enumerate(TARGETS):
        ...
        # Same-group cross-target with weight 1.0, different-group with 0.5
        ...
        # V446 config: reg_alpha=0.01, reg_lambda=0.0
    ...
    sub_path = SUBMIT / f"submission_v452_4way_int_{ts}.csv"
    sub.to_csv(sub_path, index=False)
    log.info(f"Saved: {sub_path}")

    meta_data = {
        'version': 'V452',
        'name': '4-Way Interactions + Refined Ranking',
    ...
    meta_path = EXPERIMENTS / f'v452_{ts}.json'
