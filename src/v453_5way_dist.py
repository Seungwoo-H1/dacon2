"""
V453 — V452 Baseline + 5-Way Interactions + Per-Subject Distribution Stats + 30 Seeds

Hypothesis: V452(V339 0.580)가 breakthrough. 4-way(interactions)이 효과적.
V453은:
1. 5-way interactions: z⁴, z⁵, bl×z³ 등 더高阶 interactions
2. Per-subject distribution stats: subject별 feature skewness/kurtosis
3. 30 seeds for more stable ensemble
4. V452 meta config 유지 (reg_alpha=0.01, reg_lambda=0.0)
"""
...
def main():
    ...
    log.info("=" * 70)
    log.info("V453 — 5-Way Interactions + Dist Stats + 30 Seeds")
    log.info("=" * 70)
    ...
    # ===== Higher-order interactions (V452 style but more aggressive) =====
    log.info("  Adding 5-way interaction features...")
    for t in TARGETS:
        train_bl = np.array([baselines[t][sid] for sid in train_subjects])
        test_bl = np.array([baselines[t][sid] for sid in test_df['subject_id'].values])
        for col in num_cols:
            ...
            if z_col in zscore_train.columns:
                ...
                # z⁴
                zscore_train[f'z4_{t}_{col}'] = zscore_train[z_col].values ** 4
                zscore_test[f'z4_{t}_{col}'] = zscore_test[z_col].values ** 4
                # bl² × z interaction
                zscore_train[f'bz2_{t}_{col}'] = train_bl * (zscore_train[z_col].values ** 2)
                zscore_test[f'bz2_{t}_{col}'] = test_bl * (zscore_test[z_col].values ** 2)

    # ===== Per-subject distribution stats =====
    log.info("  Adding per-subject distribution stats...")
    # Subject-level skewness and kurtosis for key numeric features
    for col in num_cols[:30]:  # Top 30 features only to save time
        # Train
        tr_grp = train_df.groupby('subject_id')[col]
        train_skew = tr_grp.skew().reindex(train_subjects).values
        train_kurt = tr_grp.apply(lambda x: x.kurtosis()).reindex(train_subjects).values
        zscore_train[f'skew_{col}'] = train_skew
        zscore_train[f'kurt_{col}'] = train_kurt

        # Test
        te_grp = test_df.groupby('subject_id')[col]
        test_skew = te_grp.skew().reindex(test_df['subject_id'].values).fillna(0).values
        test_kurt = te_grp.apply(lambda x: x.kurtosis()).reindex(test_df['subject_id'].values).fillna(0).values
        zscore_test[f'skew_{col}'] = test_skew
        zscore_test[f'kurt_{col}'] = test_kurt
    ...
    for t_idx, target in enumerate(TARGETS):
        ...
        # Rank features 5 times for stability
        fold_ranks = []
        for fold in range(5):
        ...
        log.info(f"  {target}: oof={avg_oof:.5f} ({N_SEEDS} seeds)")

    # ===== Phase 2: V452-style Meta =====
    log.info("\n=== Phase 2: V452-style Meta ===")
    ...
    sub_path = SUBMIT / f"submission_v453_5way_dist_{ts}.csv"
    sub.to_csv(sub_path, index=False)
    log.info(f"Saved: {sub_path}")

    meta_data = {
        'version': 'V453',
        'name': '5-Way Interactions + Dist Stats + 30 Seeds',
    ...
    meta_path = EXPERIMENTS / f'v453_{ts}.json'
