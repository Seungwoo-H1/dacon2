"""
V446 — Z-Score + Baseline Poly + Log Transform + 30 Seeds

Hypothesis: V443이 현재 최고(V339 0.59208).
V446은:
1. Raw feature non-linear transforms: log(x+1), sqrt(x+eps), 1/(x+eps) before z-score
2. More LGBM seeds: 30 seeds for better ensemble averaging
3. Feature interaction: z × z (pairwise within domain)
4. Per-target cross-validation stacking: 3-fold instead of 5-fold for more training data per meta
5. Target-pair correlation features: per-subject correlation between target pairs
"""
...
def main():
    ...
    log.info("=" * 70)
    log.info("V446 — Non-Linear Transforms + 30 Seeds + Target Pair Correlation")
    log.info("=" * 70)
    ...
    log.info("  Adding non-linear feature transforms...")
    transform_features = pd.DataFrame(index=train_df.index)
    for col in num_cols:
        vals = train_df[col].values.astype(np.float64)
        eps = 1e-8

        # Log transform (positive features only)
        pos_mask = vals > 0
        if pos_mask.sum() > 0:
            transform_features[f'log_{col}'] = vals.copy()
            transform_features.loc[~pos_mask, f'log_{col}'] = 0

        # Sqrt
        transform_features[f'sqrt_{col}'] = np.sqrt(np.abs(vals) + eps) * np.sign(vals)

        # 1/x (inverse)
        inv_vals = 1.0 / (np.abs(vals) + eps)
        transform_features[f'inv_{col}'] = np.sign(vals) * inv_vals

    # ===== Z-Score on all features =====
    all_base_cols = num_cols + [c for c in transform_features.columns]
    log.info("  Computing z-scores on {} features...".format(len(all_base_cols)))

    # Add subject_id to transform_features for groupby
    tr_feat_ext = transform_features.copy()
    tr_feat_ext = tr_feat_ext.reindex(train_df.index)
    tr_feat_ext['subject_id'] = train_df['subject_id'].values
    te_feat_ext = transform_features.copy()
    te_feat_ext = te_feat_ext.reindex(test_df.index)
    te_feat_ext['subject_id'] = test_df['subject_id'].values
    ...
    for col in all_base_cols:
        ...
        elif col in transform_features.columns:
            tr_mean = tr_feat_ext.groupby('subject_id')[col].transform('mean')
            tr_std = tr_feat_ext.groupby('subject_id')[col].transform('std').fillna(0).replace(0, 1)
            te_mean = te_feat_ext.groupby('subject_id')[col].transform('mean')
            te_std = te_feat_ext.groupby('subject_id')[col].transform('std').fillna(0).replace(0, 1)
            zscore_train[f'z_{col}'] = (transform_features[col] - tr_mean) / tr_std
            zscore_test[f'z_{col}'] = (transform_features[col] - te_mean) / te_std
    ...
    # ===== Target-pair correlation features =====
    log.info("  Adding target-pair correlation features...")
    for i, t1 in enumerate(TARGETS):
        for t2 in TARGETS[i+1:]:
            zscore_train[f'corr_{t1}_{t2}'] = train_df[t1] * train_df[t2]

    # Add target-pair corr to test too
    for i, t1 in enumerate(TARGETS):
        for t2 in TARGETS[i+1:]:
            zscore_test[f'corr_{t1}_{t2}'] = 0.0  # default to 0 for test (target unknown)

    all_train_features = pd.concat([transform_features, zscore_train], axis=1)
    all_test_features = pd.concat([transform_features, zscore_test], axis=1)
    ...
    for t_idx, target in enumerate(TARGETS):
        ...
        log.info(f"  {target}: nl_trans_oof={avg_oof:.5f} ({N_SEEDS} seeds)")
    ...
    sub_path = SUBMIT / f"submission_v446_nl_trans_{ts}.csv"
    sub.to_csv(sub_path, index=False)
    log.info(f"Saved: {sub_path}")

    meta_data = {
        'version': 'V446',
        'name': 'Non-Linear Transforms + 30 Seeds + Target Pair Correlation',
    ...
    meta_path = EXPERIMENTS / f'v446_{ts}.json'
