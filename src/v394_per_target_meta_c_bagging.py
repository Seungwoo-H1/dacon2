...
V392 proved per-target meta C helps (Q→C=10, S→C=100, predicted LB -0.00563).
V368 proved bagging + CV ranking helps (OOF 0.60492).
Hypothesis: Combining per-target meta C with feature bagging reduces OOF-LB gap.

Key changes from V308:
1. Use V392's per-target meta C (Q→10, S→100)
2. Feature bagging: each seed gets a random subset of features (bag_ratio=0.7)
3. Keep V53_SWEEP top-K + ranking, but sample within top-K
4. 15 seeds × GroupKFold 5-fold → per-target meta-learner

Why this is different from V380 (bagging + C sweep):
- V380 used equal bagging + C sweep but all targets same meta C
- V394 uses per-target meta C which matched student avg to V308 (0.692 identical)
- V394 uses feature sampling within top-K (not full feature bagging)
- Expected: lower OOF without inflating student avg
"""
...
N_FOLDS = 5
N_SEEDS = 15
BAG_RATIO = 0.7  # sample 70% of selected features per seed
...
def main():
    ...
    log.info("=" * 70)
    log.info("V394 — Per-Target Meta C + Feature Bagging")
    log.info("Hypothesis: V392 per-target meta C + feature bagging reduces OOF-LB gap")
    log.info("V392: per-target C (Q=10, S=100), predicted LB 0.6333 (Δ -0.00563)")
    log.info(f"V394: + feature bagging (ratio={BAG_RATIO}) within top-K selected features")
    log.info("=" * 70)
    ...
    for t in TARGETS:
        ...
        log.info(f"    Config: {cfg_name}, n_feat: {n_feat}, meta C: {meta_c}")
        log.info(f"    Selected {len(sel_cols)} features")

        cfg = CFGS[cfg_name]
        rng = random.Random(SEED)

        # Level 0: N_SEEDS LGBM models with feature bagging
        per_seed_oofs = []
        for si in range(N_SEEDS):
            ...
            # Feature bagging: sample BAG_RATIO fraction of selected features
            bag_cols = rng.sample(sel_cols, max(1, int(len(sel_cols) * BAG_RATIO)))
            bag_cols_test = [c for c in bag_cols if c in test_feat_cols]

            if len(bag_cols_test) < 3:
                log.warning(f"    Seed {si}: too few bag features, using full set")
                bag_cols_test = [c for c in sel_cols if c in test_feat_cols]
            ...
            for fold, (tr_idx, va_idx) in enumerate(gkf.split(train_df, y, group)):
                ...
                seed_test += m.predict(test_df[bag_cols_test].fillna(0).values.astype(np.float64))
            ...
            if si < 5 or si % 3 == 0:
                ...
                log.info(f"    Seed {si:2d} (s{seed}, bag={len(bag_cols)}/{len(sel_cols)}): OOF={ll_seed:.5f}")
    ...
    log.info(f"\n{'='*70}")
    log.info(f"V394 RESULTS (Per-Target Meta C + Feature Bagging, bag_ratio={BAG_RATIO})")
    log.info(f"{'='*70}")
    for t in TARGETS:
    ...
    v308_per_target = {
        ...
        'S1': 0.55802, 'S2': 0.63488, 'S3': 0.57165, 'S4': 0.63511,
    }
    for t in TARGETS:
        v308_val = v308_per_target[t]
        delta = per_target_oof[t] - v308_val
        log.info(f"  {t} Δ vs V308: {delta:+.5f}")

    # Check student-meta gap
    ...
    sub_path = SUBMIT / f"submission_v394_per_target_meta_c_bagging_{ts}.csv"
    ...
    meta_data = {
        'version': 'V394',
        'name': 'Per-Target Meta C + Feature Bagging',
    ...
    meta_path = EXPERIMENTS / f'v394_{ts}.json'
