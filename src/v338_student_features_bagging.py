"""
V338: Student Features + Feature Bagging — Enhanced V337

V337 Two-Stage Stacking had a bug where column naming mismatch caused Stage 2 to fail.
V338 fixes the naming, adds more feature engineering:
1. Same V329 feature pipeline as V337
2. Stage 1: 15 seeds of V329 students → OOF predictions
3. Add Stage 1 predictions (mean, std, min, max) as permanent features
4. Stage 2: Train students on augmented features with aggressive feature bagging (50%)
5. LR meta on Stage 2 students
6. Per-target feature ranking on augmented set

Key difference from V337: Feature bagging fraction = 0.50 (more diversity) +
additional prediction statistics (min, max) as features.
"""
...
N_SEEDS = 15
META_C = 10.0
FEATURE_BAG_FRACTION = 0.50  # More aggressive bagging for diversity
...
def generate_base_features(train_df, test_df):
    """Generate z-scores, interactions, and per-subject features."""
...
def train_students_v2(train_df, all_feat_cols, y, group, gkf, n_seeds, cfg, n_feat, seed=SEED,
                      pred_cols=None):
    """Train V338-style students with feature bagging.
    pred_cols: optional list of prediction feature column names to always include.
    """
    ...
    for si in range(n_seeds):
        ...
        # Always include prediction columns if provided
        base_feats = [c for c in all_feat_cols if c not in (pred_cols or [])]
        n_bag = max(int(len(base_feats) * FEATURE_BAG_FRACTION), n_feat)
        bag = rng.choice(base_feats, size=min(n_bag, len(base_feats)), replace=False).tolist()
        if pred_cols:
            bag = bag + pred_cols
        sel = list(set(bag))  # dedupe
...
def main():
    ...
    t_start = time.time()
    log.info("=" * 70)
    log.info("V338 — Student Features + Aggressive Feature Bagging (50%)")
    log.info("=" * 70)
    ...
    for t in TARGETS:
        ...
        oofs = train_students_v2(train_df, ranked, y, group, gkf, N_SEEDS, cfg,
                                 V53_SWEEP[t]['n_feat'], seed=SEED, pred_cols=None)
        stage1_oofs[t] = oofs
        avg_soof = np.mean([log_loss(y, p) for p in oofs])
        log.info(f"  {t}: avg student OOF = {avg_soof:.5f}")

    # ========== STAGE 2: Student predictions as permanent features ==========
    log.info("\nSTAGE 2: Building augmented features (pred_mean, pred_std, pred_min, pred_max)...")
    ...
    for t in TARGETS:
        ...
        # Build prediction stats from Stage 1
        oof_matrix = np.column_stack(stage1_oofs[t])
        pred_mean = np.mean(oof_matrix, axis=1)
        pred_std = np.std(oof_matrix, axis=1)
        pred_min = np.min(oof_matrix, axis=1)
        pred_max = np.max(oof_matrix, axis=1)

        # Other target predictions (means only)
        ...
        pred_col_names = [f's1_{t}_pred_mean', f's1_{t}_pred_std', f's1_{t}_pred_min', f's1_{t}_pred_max']
        aug_df['s1_' + t + '_pred_mean'] = pred_mean
        aug_df['s1_' + t + '_pred_std'] = pred_std
        aug_df['s1_' + t + '_pred_min'] = pred_min
        aug_df['s1_' + t + '_pred_max'] = pred_max
        for i, ot in enumerate(other_targets):
            aug_df[f's1_{ot}_pred_mean'] = other_pred[:, i]

        pred_cols = list(aug_df.columns[len(feat_cols_clean):])
        ...
        # Train Stage 2 students with feature bagging on augmented features
        oofs = []
        for si in range(N_SEEDS):
            seed = SEED + si * 7
    ...
    log.info(f"\n{'='*70}")
    log.info(f"V338 RESULTS — Student Features + Aggressive Bagging (50%)")
    log.info(f"{'='*70}")
    for t in TARGETS:
    ...
    sub_path = SUBMIT / f"submission_v338_student_features_bagging_{ts}.csv"
    sub.to_csv(sub_path, index=False)
    log.info(f"Saved submission: {sub_path}")

    meta_data = {
        'version': 'V338',
        'name': 'Student Features + Aggressive Feature Bagging (50%)',
        ...
        'key_difference': 'V337 + more aggressive feature bagging (50% vs 75%) + pred_min/pred_max features',
    }
