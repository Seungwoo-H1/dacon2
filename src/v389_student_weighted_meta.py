"""
V389 — Student-aware Meta-Weighting

Hypothesis: V308 trains LR meta-learner with equal weight on all 15 seeds.
Seeds with high student OOF (poor calibration) drag the meta-learner toward
their prediction patterns. This causes the meta-learner to overfit to
low-quality signal → larger OOF-LB gap.

V389: Weight each seed's OOF contribution by inverse student OOF.
Seeds with better calibration → higher weight → meta learns from the best seeds.

Method:
- Each seed has student OOF (log_loss on training fold predictions)
- Weight = 1/student_OOF (normalized to sum=1)
- stacked_train_weighted[i] = weight[i] * seed_oof[i]
- OR: meta.fit(stacked_train, y) but with sample_weight

Expected:
- Student avg: lower (meta learns from better seeds)
- Meta OOF: ~0.620-0.625 (similar or slightly better)
- Predicted LB: ~0.635-0.640 (if gap shrinks due to better student)
- Risk: Low (should at least maintain V308 performance)

Changes from V308:
1. Meta-learner training: weighted by inverse student OOF
2. Everything else identical (features, seeds, GroupKFold, etc.)
"""
...
def main():
    ...
    log.info("=" * 70)
    log.info("V389 — Student-aware Meta-Weighting")
    log.info("Hypothesis: Weight meta-learner by inverse student OOF → better calibration")
    ...
    for t in TARGETS:
        ...
        # Use per-target top-K ranking (same as V308)
        log.info(f"    Config: {cfg_name}, n_feat: {n_feat}")
        
        # V308-style ranking: single ranking on full training data
        ...
        # V389: Student-aware meta-weighting
        # Weight = 1/student_OOF, normalized
        student_weights = np.array(student_oofs)
        inv_weights = 1.0 / np.clip(student_weights, 0.01, None)
        meta_sample_weights = inv_weights / inv_weights.sum() * N_SEEDS  # normalize to sum=N_SEEDS
        
        log.info(f"    Seed student OOFs: {[f'{w:.4f}' for w in student_oofs]}")
        log.info(f"    Inverse weights: {[f'{w:.4f}' for w in meta_sample_weights]}")
        
        # Meta-learner: weight each seed column by inverse student OOF
        stacked_train = np.column_stack(per_seed_oofs)
        stacked_test = np.column_stack(per_seed_test)
        
        # Scale each seed's predictions by its inverse student OOF weight
        # This makes the meta-learner pay more attention to well-calibrated seeds
        for i in range(N_SEEDS):
            stacked_train[:, i] *= meta_sample_weights[i]
            stacked_test[:, i] *= meta_sample_weights[i]
        ...
        # Also compute equal-average for comparison
        ...
        all_results.append({
            ...
            'student_avg': student_avg, 'student_weights': meta_sample_weights.tolist()
        })
        all_student_oofs.extend(student_oofs)
        all_per_seed_test[t] = per_seed_test
        all_per_seed_oofs[t] = per_seed_oofs
        
        log.info(f"    V389: meta_OOF={meta_oof:.5f} (V308: {V308_OOF[t]:.5f}, Δ: {meta_oof-V308_OOF[t]:+.5f})")
        log.info(f"    V389: equal_OOF={equal_avg_ll:.5f} (Δ: {equal_avg_ll-V308_OOF[t]:+.5f})")
        log.info(f"    V389: Student avg={student_avg:.5f} (V308: 0.69212, Δ: {student_avg-0.69212:+.5f})")
    ...
    meta_data = {
        'version': 'V389',
        'name': 'Student-aware Meta-Weighting',
    ...
    meta_path = EXPERIMENTS / f'v389_{ts}.json'
    ...
    if meta_beats or equal_beats:
        log.info(f"\nV389 BEATS V308! Creating submission...")
        sub = pd.DataFrame()
        ...
        for t in TARGETS:
            ...
            sw = np.array(r['student_weights'])
            for i in range(N_SEEDS):
                stacked_train[:, i] *= sw[i]
                stacked_test[:, i] *= sw[i]
        ...
        sub_path = SUBMIT / f"submission_v389_student_weighted_{ts}.csv"
        sub.to_csv(sub_path, index=False)
        log.info(f"Saved submission: {sub_path}")
        meta_data['submission_file'] = str(sub_path)
    else:
        log.info(f"\nV389 does NOT beat V308. No submission.")
