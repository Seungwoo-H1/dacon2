"""
V463 — Target Correlation-Aware Ensemble + Threshold Calibration

Hypothesis: V439(0.623)의 가장 큰 성과는 baseline feature였음. baseline이 student를 0.633→0.623으로 낮춤.
V463은:
1. Target 간 상관관계 모델링: Q1, Q2, Q3가 부분적으로 상관 (Q1-Q2: 0.12, Q2-Q3: 0.34)
   이를 ensemble weighting에 활용 → correlated target의 predictions을 함께 조정
2. Threshold calibration: binary classification에서 threshold를 target별로 최적화 (V373 temp scaling 실패의 교훈)
   → But NOT temperature scaling. Instead, per-target isotonic regression on OOF predictions
3. Multi-label aware ensemble: Q targets를 함께 보고, S targets를 함께 보며 predictions 조정
4. Feature: Q/S group mean, std of OOF predictions → meta features
5. No adversarial validation (V461과 차이), no PCA (V462와 차이) — 원본 feature + z-score + baseline만

핵심: V439의 baseline feature + V432의 subject smoothing + V452의 higher-order interactions
      의 최적 조합을 찾기 위한 접근.
"""
...
def main():
    ...
    log.info("=" * 70)
    log.info("V463 — Target Correlation + Threshold Calibration")
    log.info("=" * 70)
    ...
    NFEAT = {'Q1': 22, 'Q2': 22, 'Q3': 20, 'S1': 26, 'S2': 24, 'S3': 26, 'S4': 24}
    ...
    # ===== Step 3: Target Correlation-Aware Calibration =====
    log.info("\n=== Step 3: Target Correlation Calibration ===")
    
    # Compute target correlations on OOF predictions
    all_student_oofs = np.column_stack([student_oofs[t] for t in TARGETS])
    target_corrs = np.corrcoef(all_student_oofs.T)
    log.info("  Target OOF correlation matrix:")
    for i, t1 in enumerate(TARGETS):
        for j, t2 in enumerate(TARGETS):
            if j > i:
                log.info(f"    {t1}-{t2}: {target_corrs[i,j]:.4f}")

    # Isotonic calibration per target (on OOF predictions)
    # This is DIFFERENT from V384 (calibration on predictions)
    # Here we calibrate student predictions using meta OOF
    calibration_models = {}
    ...
    for target in TARGETS:
        ...
        # Instead: use isotonic regression on student OOF → actual labels
        # For calibration, we use the student predictions vs true labels
        ir = IsotonicRegression(y_min=0.0, y_max=1.0, out_of_bounds='clip', increasing=True)
        ir.fit(all_oof, y)
        calibration_models[target] = ir
        
        # Apply calibration to test predictions
        calibrated = ir.predict(test_preds[target])
        calibrated_test_preds[target] = calibrated
        
        cal_oof = log_loss(y, ir.predict(all_oof))
        log.info(f"  {target}: calibrated OOF={cal_oof:.5f} (was {student_oofs[target]:.5f})")

    # ===== Step 4: Correlation-Aware Ensemble =====
    # Q targets are correlated → average their probabilities
    # S targets are less correlated → keep individual
    q_correlation_weight = 0.3  # Weight for Q-group averaging
    s_correlation_weight = 0.15  # Weight for S-group averaging
    
    final_test_preds = {}
    for target in TARGETS:
        calibrated = calibrated_test_preds[target]
        
        # Q targets: blend with Q-group mean
        if target in Q_TARGETS:
            q_mean = np.mean([calibrated_test_preds[t] for t in Q_TARGETS if t != target], axis=0)
            final_test_preds[target] = (1 - q_correlation_weight) * calibrated + q_correlation_weight * q_mean
        # S targets: blend with S-group mean  
        else:
            s_mean = np.mean([calibrated_test_preds[t] for t in S_TARGETS if t != target], axis=0)
            final_test_preds[target] = (1 - s_correlation_weight) * calibrated + s_correlation_weight * s_mean

    # ===== Step 5: Threshold Optimization =====
    # Find optimal threshold per target that minimizes log_loss on OOF
    log.info("\n=== Step 5: Threshold Optimization ===")
    best_thresholds = {}
    for target in TARGETS:
        y = train_df[target].values
        cal_oof = calibration_models[target].predict(
            np.mean([model_oof_seeds[i][target].mean(axis=1) for i in range(3)], axis=0))
        
        best_loss = float('inf')
        best_thresh = 0.5
        for thresh in np.arange(0.3, 0.7, 0.01):
            preds = np.ones(len(y)) * thresh
        ...
        log.info(f"  {target}: best_threshold={best_thresh:.2f}, loss={best_loss:.5f}")
    
    # Apply threshold calibration to test predictions
    for target in TARGETS:
        # Shift predictions toward threshold
        y_mean = train_df[target].mean()
        threshold = best_thresholds[target]
        shift = threshold - y_mean
        final_test_preds[target] = np.clip(
            final_test_preds[target] + shift * 0.1, 0.001, 0.999)

    avg_meta = np.mean(list(meta_oofs.values()))
    avg_student = np.mean(list(student_oofs.values()))
    gap = avg_student - avg_meta
    
    # Compute calibrated meta OOF
    calibrated_meta_oofs = {}
    for target in TARGETS:
        y = train_df[target].values
        all_oof = np.mean([model_oof_seeds[i][target].mean(axis=1) for i in range(3)], axis=0)
        cal_oof = calibration_models[target].predict(all_oof)
        calibrated_meta_oofs[target] = log_loss(y, cal_oof)
    avg_cal_meta = np.mean(list(calibrated_meta_oofs.values()))

    v339 = avg_cal_meta + gap * 0.85

    log.info(f"\n{'='*70}")
    log.info("V463 Results:")
    log.info(f"  AVG Meta OOF: {avg_meta:.5f}")
    log.info(f"  AVG Student OOF: {avg_student:.5f}")
    log.info(f"  Gap: {gap:.5f} ({gap/0.070:.2f}x)")
    log.info(f"  Calibrated Meta OOF: {avg_cal_meta:.5f}")
    log.info(f"  V339 LB: {v339:.5f}")
    log.info(f"  Thresholds: {best_thresholds}")
    log.info(f"{'='*70}")
    ...
    for t in TARGETS: sub[t] = final_test_preds[t]
    sub_path = SUBMIT / f"submission_v463_corr_calibration_{ts}.csv"
    sub.to_csv(sub_path, index=False)
    meta_data = {'version': 'V463', 'name': 'Target Correlation + Threshold Calibration',
        'avg_meta_oof': round(float(avg_meta), 5), 'avg_student_oof': round(float(avg_student), 5),
        'calibrated_meta_oof': round(float(avg_cal_meta), 5),
    ...
    meta_path = EXPERIMENTS / f'v463_{ts}.json'
