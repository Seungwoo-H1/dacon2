"""
V423 — Per-Subject Bias Removal (Random Effect)

Hypothesis: Each subject has a systematic bias (baseline skill level).
If we estimate and remove subject-level bias, the remaining signal
may be more generalizable and reduce overfitting to subject-specific patterns.

V423:
1. For each subject, estimate their baseline prediction (mean OOF per subject)
2. Subtract baseline from predictions
3. Apply meta-learner on bias-corrected predictions
4. Add baseline back for final output
"""
...
def main():
    ...
    log.info("=" * 70)
    log.info("V423 — Per-Subject Bias Removal (Random Effect)")
    log.info("Hypothesis: Removing subject-level bias improves generalization")
    log.info("=" * 70)
    ...
    # ===== Phase 2: Per-Subject Bias Correction =====
    log.info("\n=== Phase 2: Per-Subject Bias Correction ===")
    ...
    for t_idx, target in enumerate(TARGETS):
        ...
        oof_preds = all_oof_seed[target].mean(axis=1)  # simple average OOF

        # Subject-level bias: for each subject, the average deviation from true label
        subject_ids = train_df['subject_id'].values
        bias = {}
        for sid in np.unique(subject_ids):
            mask = subject_ids == sid
            if y[mask].sum() > 0:  # only if there are positive examples
                bias[sid] = oof_preds[mask].mean() - y[mask].mean()
            else:
                bias[sid] = 0

        # Corrected OOF predictions
        corrected_oof = oof_preds.copy()
        for i, sid in enumerate(subject_ids):
            corrected_oof[i] = oof_preds[i] - bias[sid]

        # Clip to valid probability range
        corrected_oof = np.clip(corrected_oof, 0.01, 0.99)

        meta_oofs[target] = log_loss(y, corrected_oof)

        # Student = original average (for gap tracking)
        student_oofs[target] = log_loss(y, oof_preds)

        # Test: estimate subject bias from training data
        test_subjects = test_df['subject_id'].values
        corrected_test = oof_preds.mean() * np.ones(len(test_df))  # global mean as prior
        for i, sid in enumerate(test_subjects):
            if sid in bias:
                # Partial shrinkage: use training bias with 50% weight
                corrected_test[i] = oof_preds.mean() - bias[sid] * 0.5
            else:
                corrected_test[i] = oof_preds.mean()

        corrected_test = np.clip(corrected_test, 0.01, 0.99)
        test_preds[target] = corrected_test

        log.info(f"  {target}: corrected_meta={meta_oofs[target]:.5f}, "
                 f"student={student_oofs[target]:.5f}, "
                 f"bias_range=[{min(bias.values()):.3f}, {max(bias.values()):.3f}]")
    ...
    sub_path = SUBMIT / f"submission_v423_subject_bias_{ts}.csv"
    sub.to_csv(sub_path, index=False)
    log.info(f"Saved submission: {sub_path}")

    meta_data = {
        'version': 'V423',
        'name': 'Per-Subject Bias Removal (Random Effect)',
        ...
        'v308_gap': 0.070,
        'meta_type': 'subject_bias_removed',
    ...
    meta_path = EXPERIMENTS / f'v423_{ts}.json'
