"""
V154 — More Seeds on V152 architecture

Architecture: Same as V152 (2 LGBM + XGB + CB)
Change: Seeds 7 → 12 per model family (48 students: 2 LGBM + XGB + CB × 12)
C sweep: 100, 500, 1000, 3000, 5000 (from V152 sweet spot range)
"""
...
SEED = 42
N_FOLDS = 5
N_SEEDS = 12  # 12 seeds per model family
META_C_SWEEP = [100.0, 500.0, 1000.0, 3000.0, 5000.0]
TOP_K = 30
...
def v154_run(train_df, test_df, feat_cols):
    ...
    log.info(f"C sweep: {META_C_SWEEP}")
    ...
    log.info(f"V152 AVG OOF: 0.61207, Δ: {best_oof - 0.61207:+.5f}")
    log.info(f"{'='*70}")
    ...
    sub_path = SUBMIT / f"submission_v154_more_seeds_{ts}.csv"
    sub.to_csv(sub_path, index=False)
    log.info(f"Saved: {sub_path}")
    
    meta_data = {
        'version': 'V154',
        'name': 'More Seeds (12 per model, 48 students, C 100-5000)',
        ...
        'delta_vs_v152': round(float(best_oof - 0.61207), 5),
    ...
    meta_path = SUBMIT / f'meta_v154_{ts}.json'
...
t_start = time.time()
log.info("=" * 70)
log.info("V154 — More Seeds (12 per model family)")
log.info("=" * 70)
...
avg_oof, meta = v154_run(train_df, test_df, feat_cols)
