"""
V29 — Rolling(3d,7d) Only — Submission (Fixed Top-30, no per-target tuning)

Based on V26 ablation: rolling alone Top-30 → CV=0.5778 (BEST).
Use FIXED Top-30 (no per-target N tuning) for speed.
10 seeds for CV + 10 seeds for submission prediction.
"""
...
def main():
    log.info("="*70)
    log.info("V29 — Rolling(3d,7d) Only — Fixed Top-30 — Submission")
    log.info("V26 ablation: rolling alone Top-30 → CV=0.5778 (BEST)")
    log.info("="*70)
    ...
    # ── Per-target CV with FIXED Top-30 ──
    log.info("\n=== Per-target CV (rolling only, fixed Top-30) ===")
    ...
    for target in TARGET_COLS:
        ...
        oof=cv_f(feat_r, sel, target)
        cal=mm(oof, train_rate[target])
        loss=log_loss(feat_r[target],cal,labels=[0,1])
        all_oof[target]=oof; all_sel[target]=sel
        log.info(f"  Fixed N=30, Cal OOF={loss:.4f}, train_rate={train_rate[target]:.3f}")
        log.info(f"  Top 5: {[r[0].split('_rm')[0] if '_rm' in r[0] else r[0].split('_rs')[0] for r in ranked[:5]]}")

    avg=np.mean([log_loss(feat_r[t],mm(all_oof[t],train_rate[t]),labels=[0,1]) for t in TARGET_COLS])
    log.info(f"\nV29 Cal OOF Avg: {avg:.4f} (V10: 0.6038, Δ: {avg-0.6038:+.4f})")
    ...
    meta={'version':'v29','submission_file':str(sp),'timestamp':ts,'n_samples':len(predictions),
