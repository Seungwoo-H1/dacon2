"""
V339: Student Features + Cross-Subject Z-Scores + Aggressive Bagging

V338 was V337-style two-stage stacking with 50% feature bagging.
V339 adds cross-subject z-score features on top, same as V329 cross_ps.

Key hypothesis: Cross-subject z-scores + student predictions + aggressive bagging
will produce a stronger ensemble.
"""
...
def add_cross_subject_features(train_df, test_df):
    """Add cross-subject z-scores — same as V329 cross_ps."""
    train_df = train_df.copy()
    test_df = test_df.copy()
    base_cols = [c for c in train_df.columns if c not in META_COLS | set(TARGETS) | {'date'}
                 and not c.endswith('_zscore') and not c.endswith('_interact')
...
def main():
    ...
    t_start = time.time()
    log.info("=" * 70)
    log.info("V339 — Student Features + Cross-Subject Z-Scores + Aggressive Bagging")
    log.info("V329 cross_ps: OOF=0.54365 | This adds cross-subject z to V338 pipeline")
    log.info("=" * 70)
    ...
    log.info("Adding cross-subject features...")
    ...
    # ========== STAGE 2 ==========
    log.info("\nSTAGE 2: Building augmented features + training students...")
    ...
    for t in TARGETS:
        ...
        # Train Stage 2 students with feature bagging
        oofs = []
        for si in range(N_SEEDS):
            seed = SEED + si * 7
    ...
    log.info(f"\n{'='*70}")
    log.info(f"V339 RESULTS — Student Features + Cross-Subject Z + Aggressive Bagging")
    log.info(f"{'='*70}")
    for t in TARGETS:
    ...
    sub_path = SUBMIT / f"submission_v339_cs_student_features_{ts}.csv"
    sub.to_csv(sub_path, index=False)
    log.info(f"Saved submission: {sub_path}")

    meta_data = {
        'version': 'V339',
        'name': 'Student Features + Cross-Subject Z-Scores + Aggressive Bagging',
        ...
        'key_difference': 'V329 cross-subject z-scores + student predictions (mean/std/min/max) + 50% bagging',
    }
