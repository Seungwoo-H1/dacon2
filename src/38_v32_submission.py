"""
V32 — Rolling(3d,7d) Only — Submission (ALL features, 5 seeds)

Skip ALL feature selection/ranking/subset search.
Use ALL non-leak features directly. Just CV + submission.
"""
...
def main():
    log.info("="*70)
    log.info("V32 — Rolling(3d,7d) Only — ALL Non-Leak Features")
    log.info("="*70)
    ...
    for target in TARGET_COLS:
        ...
        log.info(f"  Using ALL {len(sel)} features (after leak removal)")
    ...
    log.info(f"\nV32 Cal OOF Avg: {avg:.4f} (V10: 0.6038, Δ: {avg-0.6038:+.4f})")
    ...
    meta={'version':'v32','submission_file':str(sp),'timestamp':ts,'n_samples':len(predictions),
          ...
          'strategy':'rolling(3d,7d) only — ALL non-leak features, no selection',
          'cv_avg':float(avg),
          'per_target':{}}
