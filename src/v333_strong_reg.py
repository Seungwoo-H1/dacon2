"""
V333 — V329 + Stronger Regularization (Student OOF Improvement)

Hypothesis: Student OOF is 0.65~0.69 (much higher than Q-targets ~0.58).
Stronger regularization on student models should reduce student OOF,
which should also improve the OOF-LB gap.

Approach: V329 features + more aggressive student HP (smaller trees,
more regularization) with balanced configs across all targets.
"""
...
def main():
    ...
    log.info("=" * 70)
    log.info("V333 — V329 + Stronger Regularization (Student OOF Improvement)")
    log.info("Hypothesis: Stronger regularization → lower student OOF → better gap")
    log.info("=" * 70)
    ...
    log.info("Step 4: Day-of-week patterns...")
    ...
    # V333 configs: stronger regularization than V329
    CFGS = {
        # V329 wide (default)
        ...
        # V333 STRONG: much stronger regularization
        'strong': {'num_leaves': 10, 'max_depth': 3, 'learning_rate': 0.03, 'n_estimators': 500,
                   'subsample': 0.6, 'colsample_bytree': 0.5, 'reg_alpha': 5.0, 'reg_lambda': 20.0, 'min_child_samples': 30},
        # V333 STRONGER: extreme regularization
        'stronger': {'num_leaves': 8, 'max_depth': 2, 'learning_rate': 0.02, 'n_estimators': 800,
                     'subsample': 0.5, 'colsample_bytree': 0.4, 'reg_alpha': 10.0, 'reg_lambda': 30.0, 'min_child_samples': 40},
    }
    
    # V333: Apply stronger regularization to ALL targets
    # Strategy: use 'strong' for S targets (high OOF), 'deep' for Q targets (already ok)
    # But keep uniform config to avoid test overfitting (V316 lesson)
    V53_SWEEP = {
        'Q1':  {'cfg': 'strong', 'n_feat': 19},
        'Q2':  {'cfg': 'strong', 'n_feat': 14},
        'Q3':  {'cfg': 'strong', 'n_feat': 11},
        'S1':  {'cfg': 'stronger', 'n_feat': 21},
        'S2':  {'cfg': 'stronger', 'n_feat': 19},
        'S3':  {'cfg': 'stronger', 'n_feat': 23},
        'S4':  {'cfg': 'stronger', 'n_feat': 20},
    }
    ...
    for t in TARGETS:
        log.info(f"\n{'='*60}")
        log.info(f"Target: {t} (cfg={V53_SWEEP[t]['cfg']})")
        ...
        for si in range(N_SEEDS):
            ...
            if si < 3 or si == N_SEEDS - 1:
                ...
                log.info(f"    Seed {si:2d}: OOF={s_oof:.5f} ({cfg_name})")
    ...
    log.info(f"\n{'='*70}")
    log.info(f"V333 RESULTS (Stronger Regularization)")
    log.info(f"{'='*70}")
    for t in TARGETS:
    ...
    sub_path = SUBMIT / f"submission_v333_strong_reg_{ts}.csv"
    sub.to_csv(sub_path, index=False)
    log.info(f"Saved submission: {sub_path}")
    
    meta_data = {
        'version': 'V333',
        'name': 'V329 + Stronger Regularization (Student OOF)',
