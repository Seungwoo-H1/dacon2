"""
V451 — Gap-Controlled Stacking + Feature Importance Re-Weighting

Hypothesis: V446(V339 0.589)이 plateau. Gap이 1.22x로 넓어서 V339가 실제보다 낮게 예측됨.
V451은:
1. Gap control: Meta OOF를 V446 수준(0.516)으로 유지하면서 student 낮춤
2. Feature importance re-weighting: CI-weighted importance (Shap-like)
3. Gradient-aware stacking: meta learner가 gradient residual을 학습하도록
4. Per-target cross-target count tuning: fewer cross-target features to reduce gap
"""
...
# Per-target feature counts: tuned for gap control
# Fewer features → less overfitting → gap stays close to V308
NFEAT = {'Q1': 22, 'Q2': 22, 'Q3': 20, 'S1': 28, 'S2': 26, 'S3': 28, 'S4': 26}

# Cross-target weights: reduce weight to keep gap closer to V308
CROSS_WEIGHTS = {
    'Q': {'Q': 0.8, 'S': 0.3},  # Lower cross-group weight
    'S': {'Q': 0.3, 'S': 0.8},
}
...
def main():
    ...
    log.info("=" * 70)
    log.info("V451 — Gap-Controlled Stacking")
    log.info("=" * 70)
    ...
    for t_idx, target in enumerate(TARGETS):
        ...
        stu_oof = log_loss(y, oof_preds.mean(axis=1))
        student_oofs[target] = stu_oof

        # Cross-target features with gap-controlled weights
        ...
        for t_cross in group_targets:
            if t_cross == target:
                continue
            w = CROSS_WEIGHTS[group][group]
            cross_oof_list.append(np.mean(all_oof_seed[t_cross], axis=1) * w)
            cross_test_list.append(np.mean(all_test_seed[t_cross], axis=1) * w)

        # Add cross-group targets with smaller weight
        for t_cross in other_group:
            if t_cross.startswith(group):
                continue
            w = CROSS_WEIGHTS[group]['S'] if group == 'Q' else CROSS_WEIGHTS[group]['Q']
            cross_oof_list.append(np.mean(all_oof_seed[t_cross], axis=1) * w)
            cross_test_list.append(np.mean(all_test_seed[t_cross], axis=1) * w)
        ...
        # Meta features: student avg + cross-target means (not individual)
        # This reduces meta dimensionality → less overfitting → better gap control
        X_meta = np.column_stack([
            oof_preds.mean(axis=1),
            cross_arr.mean(axis=1),
        ])
        X_test = np.column_stack([
            test_preds_raw.mean(axis=1),
            cross_arr_test.mean(axis=1),
        ])

        # Stronger regularization to prevent meta from going too low
        mm = XGBClassifier(n_estimators=15, max_depth=3, reg_alpha=1.0, reg_lambda=5.0,
            gamma=0.1, learning_rate=0.05, subsample=0.7, colsample_bytree=0.7,
    ...
    log.info(f"  V448 V339: 0.58750")
    log.info(f"  V450 V339: 0.58621")
    log.info(f"{'='*70}")
    ...
    sub_path = SUBMIT / f"submission_v451_gap_control_{ts}.csv"
    sub.to_csv(sub_path, index=False)
    log.info(f"Saved: {sub_path}")

    meta_data = {
        'version': 'V451',
        'name': 'Gap-Controlled Stacking',
    ...
    meta_path = EXPERIMENTS / f'v451_{ts}.json'
