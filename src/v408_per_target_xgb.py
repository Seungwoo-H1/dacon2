"""
V408 — Per-Target XGB Meta Params

Hypothesis: V402 used uniform XGB params (n_est=15, md=3, lr=0.1) for all
targets. But Q targets (Student OOF ~0.82) and S targets (Student OOF ~0.60-0.74)
have fundamentally different characteristics. Q targets need more complex meta-
learner to capture the noisy seed patterns, while S targets may need less.

V408: Per-target XGB hyperparameters:
- Q targets: higher n_estimators (20, 30), md=4
- S targets: lower n_estimators (10, 15), md=3
- Sweep per-target to find best combo
"""
...
def main():
    ...
    log.info("=" * 70)
    log.info("V408 — Per-Target XGB Meta Params")
    log.info("Hypothesis: Q targets need different XGB params than S targets")
    log.info("Sweep per-target (n_est, md) independently")
    log.info("=" * 70)
    ...
    best_xgb_params = {t: {} for t in TARGETS}

    # Sweep configs per target type
    XGB_CONFIGS = [
        {'n_estimators': 10, 'max_depth': 2, 'learning_rate': 0.1},
        {'n_estimators': 10, 'max_depth': 3, 'learning_rate': 0.1},
        {'n_estimators': 15, 'max_depth': 2, 'learning_rate': 0.1},
    ...
    for t in TARGETS:
        ...
        # Per-target XGB meta param sweep
        ...
        for xgb_cfg in XGB_CONFIGS:
            mdl = train_xgb_meta(stacked, y, xgb_cfg['n_estimators'],
                               xgb_cfg['max_depth'], xgb_cfg['learning_rate'])
            preds = np.clip(mdl.predict(stacked), 0.001, 0.999)
            meta_oof = log_loss(y, preds)
            gap = student_oof - meta_oof
            score = meta_oof + 0.3 * gap

            log.info(f"      n={xgb_cfg['n_estimators']},md={xgb_cfg['max_depth']},lr={xgb_cfg['learning_rate']}: "
                     f"Meta={meta_oof:.5f}, Gap={gap:.5f}, Score={score:.5f}")

            if score < best_score:
                best_score = score
                best_params = xgb_cfg.copy()
                best_meta_oof_t = meta_oof

        best_xgb_params[t] = best_params
        log.info(f"    → Best: {best_params}")

        # Train final model
        mdl_final = train_xgb_meta(stacked, y, best_params['n_estimators'],
                                   best_params['max_depth'], best_params['learning_rate'])
        train_oof[t] = np.clip(mdl_final.predict(stacked), 0.001, 0.999)
    ...
    log.info(f"\n{'='*70}")
    log.info(f"V408 RESULTS (Per-Target XGB Meta Params)")
    log.info(f"{'='*70}")
    for t in TARGETS:
        p = best_xgb_params[t]
        log.info(f"  {t}: Meta={per_target_oof[t]:.5f}, Student={per_target_student[t]:.5f}, "
                 f"XGB(n={p['n_estimators']},md={p['max_depth']},lr={p['learning_rate']})")
    ...
    sub_path = SUBMIT / f"submission_v408_per_target_xgb_{ts}.csv"
    sub.to_csv(sub_path, index=False)
    log.info(f"Saved submission: {sub_path}")

    meta_data = {
        'version': 'V408',
        'name': 'Per-Target XGB Meta Params',
        ...
        'n_seeds': N_SEEDS,
        'meta_type': 'per_target_xgb',
        'best_xgb_params': {t: best_xgb_params[t] for t in TARGETS},
    ...
    meta_path = EXPERIMENTS / f'v408_{ts}.json'
