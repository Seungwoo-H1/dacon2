"""V116: V54 pipeline reproduction with 50 seeds

Key learning from V115:
- personalization + pairwise + isotonic calibration = best strategy
- V115 (8 seeds) OOF = 0.54759
- V54 file (50 seeds) OOF = 0.53971
- Need 50 seeds to match V54

V116: Use SEEDS = range(1, 51) with V115 strategy
- personalization + pairwise + isotonic calibration + feature selection(8,10,15,20)
- 8 configs × 50 seeds × 4 n_feat × 3 strategies × 7 targets
"""
...
SEEDS = list(range(1, 51))  # 50 seeds like V54
...
# ============================================================
# V54 strategy: all combos with 50 seeds
# Only use pairwise strategy (confirmed best in V115)
# And also base + transformed for comparison
# ============================================================
log.info(f"\n--- V115: 8 configs × 4 n_feat × {len(SEEDS)} seeds × 3 strategies × 7 targets ---")
...
for target in TARGETS:
    ...
    # Strategy A: Base (personalization only)
    base_ok = True
        ...
        for n_feat in [8, 10, 15, 20]:
            ...
            try:
                ...
                if ll < best_cal:
                    ...
                    best_method_type = 'base'
            except Exception as e:
                log.warning(f"  {target} base_{cfg_name}_n{n_feat}: {e}")
                base_ok = False
                break
        if not base_ok:
            break
    if base_ok:
        log.info(f"  {target}: base best={best_config_str} Cal={best_cal:.5f}")
    gc.collect()
    
    # Strategy B: Pairwise (best in V115)
    best_cal_base = best_cal
    best_oof_base = best_oof.copy()
    best_config_base = best_config_str
    ...
    for cfg_name, cfg in CFGS.items():
        for n_feat in [8, 10, 15, 20]:
            ...
            try:
                ...
                if ll < best_cal_pair:
                    best_cal_pair = ll
                    best_oof_pair = iso_cal.copy()
                    best_config_pair = config_str
            except Exception as e:
                log.warning(f"  {target} pair_{cfg_name}_n{n_feat}: {e}")
                break
    
    log.info(f"  {target}: pair best={best_config_pair} Cal={best_cal_pair:.5f}")
    ...
    best_cal_trans = float('inf')
    best_oof_trans = None
    best_config_trans = None
    ...
    for cfg_name, cfg in CFGS.items():
        for n_feat in [8, 10, 15, 20]:
            ...
            try:
                ...
                if ll < best_cal_trans:
                    best_cal_trans = ll
                    best_oof_trans = iso_cal.copy()
                    best_config_trans = config_str
            except Exception as e:
                log.warning(f"  {target} trans_{cfg_name}_n{n_feat}: {e}")
                break
    
    log.info(f"  {target}: trans best={best_config_trans} Cal={best_cal_trans:.5f}")
    
    # Choose best among all strategies
    if best_cal_pair <= best_cal_base and best_cal_pair <= best_cal_trans:
        best_cal = best_cal_pair
        best_oof = best_oof_pair
        best_config_str = best_config_pair
        best_method_type = 'pair'
    elif best_cal_trans <= best_cal_base:
        best_cal = best_cal_trans
        best_oof = best_oof_trans
        best_config_str = best_config_trans
        best_method_type = 'trans'
    # else keep base
    
    all_results[target] = {
        'best_method': best_config_str,
        'best_method_type': best_method_type,
        'cal_oof': best_oof,
        'cal_loss': best_cal,
    }
    log.info(f"  {target}: FINAL {best_method_type} {best_config_str} Cal={best_cal:.5f} (time: {time.time()-tgt_t:.0f}s)")
    gc.collect()
...
for t in TARGETS:
    ...
    log.info(f"  {t}: {all_results[t]['best_method_type']:5s} {all_results[t]['best_method']:30s} Cal={ll:.5f}")
log.info(f"  AVG: {avg_cal:.5f}")
log.info(f"  V54 file: 0.53971")
log.info(f"  V115:     0.54759")
...
oof_path = DATA / f'oof_v116_{ts}.csv'
...
exp_log = {
    ...
    'results': {t: {
        ...
        'best_method_type': all_results[t]['best_method_type'],
        'cal_loss': round(all_results[t]['cal_loss'], 5),
    } for t in TARGETS},
    'avg_cal_loss': round(avg_cal, 5),
    'total_time_s': round(time.time() - t_start, 0),
}
with open(EXPERIMENTS / f'v116_{ts}.json', 'w') as f:
    json.dump(exp_log, f, indent=2, default=str)

log.info(f"\nSaved OOF: {oof_path}")
log.info(f"Log: {EXPERIMENTS / f'v116_{ts}.json'}")
