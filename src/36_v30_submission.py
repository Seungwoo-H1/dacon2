"""
V30 — Rolling(3d,7d) Only — Submission (No ranking, use known best features)

Skip feature ranking — use known best from V26 ablation directly.
Fixed Top-30 (5 seeds CV + 5 seeds submission).
"""
...
def rank_f(feat, cols, target):
    """Fast ranking: 1 seed, 5-fold instead of 10, fewer iterations"""
...
def main():
    log.info("="*70)
    log.info("V30 — Rolling(3d,7d) Only — No Ranking, Top-30 — Submission")
    log.info("Strategy: rank with 5 seeds for speed, then 5 seeds for submission")
    log.info("="*70)
    ...
    # ── Fast per-target CV with Top-30 (5-seed ranking) ──
    log.info("\n=== Fast per-target CV (rolling only, Top-30, 5 seeds) ===")
    all_oof={}; all_sel={}; target_ranked={}
    ...
    for target in TARGET_COLS:
        ...
        all_oof[target]=oof; all_sel[target]=sel
        log.info(f"  Top-30, Cal OOF={loss:.4f}, train_rate={train_rate[target]:.3f}")

    avg=np.mean([log_loss(feat_r[t],mm(all_oof[t],train_rate[t]),labels=[0,1]) for t in TARGET_COLS])
    log.info(f"\nV30 Cal OOF Avg: {avg:.4f} (V10: 0.6038, Δ: {avg-0.6038:+.4f})")

    # ── Submission ──
    log.info("\n=== Generating submission (5 seeds) ===")
    ...
    sp=SUBMIT_DIR/f'submission_v30_{ts}.csv'
    predictions.to_csv(sp,index=False)
    log.info(f"✅ Saved: {sp}")

    meta={'version':'v30','submission_file':str(sp),'timestamp':ts,'n_samples':len(predictions),
          ...
          'strategy':'rolling(3d,7d) only — fixed Top-30 (5 seeds, no per-target tuning)',
          'cv_avg':float(avg),
          'per_target':{}}
    ...
    log.info(f"{'Target':<6} {'Cal OOF':<12} {'Test Mean':<12} {'Train':<8} {'Shift':<8}")
    for t in TARGET_COLS:
        co=log_loss(feat_r[t],mm(all_oof[t],train_rate[t]),labels=[0,1])
        log.info(f"{t:<6} {co:<12.4f} {predictions[t].mean():<12.4f} {train_rate[t]:<8.3f} {predictions[t].mean()-train_rate[t]:<+8.4f}")
