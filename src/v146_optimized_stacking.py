"""
V146 — Optimized Stacking (C=10, 5 seeds)

Key improvements over V140:
  - Meta-learner C: 0.1 → 10.0 (less regularization on meta features)
  - Seeds: 3 → 5 (more model diversity for stacking)
  
V140 structure preserved exactly:
  - Same config→target mapping
  - Same feature selection (top-K stability-ranked per target)
  - Same leak removal
  - Same GroupKFold 5-fold

Benchmark: C=10, n_seeds=5 → AVG OOF=0.63169 vs V140's 0.64110
"""
...
SEED = 42
N_FOLDS = 5
N_SEEDS = 5        # ← V140: 3, V146: 5
META_C = 10.0      # ← V140: 0.1, V146: 10.0 (less meta regularization)
...
def proper_stacking_and_submit(train_df, test_df, feat_cols):
    """
    Proper CV Stacking: N_SEEDS × GroupKFold 5-fold → LR meta-learner.
    Level 0: N_SEEDS LGBM models (different seeds)
    Level 1: LogisticRegression meta-learner on out-of-fold preds
    
    Test: each seed predicts on test, meta-learner stacks predictions.
    """
    ...
    for t in TARGETS:
        ...
        # Level 1: Stack per-seed OOF → LR meta-learner (META_C)
        ...
        # Test prediction: meta on N_SEEDS test predictions per sample
    ...
    sub_path = SUBMIT / f"submission_v146_optimized_{ts}.csv"
    ...
    meta_data = {
        'version': 'V146',
        'name': f'Optimized Stacking (C={META_C}, {N_SEEDS} seeds)',
    ...
    meta_path = SUBMIT / f'meta_v146_{ts}.json'
...
t_start = time.time()
log.info("=" * 70)
log.info("V146 — Optimized Stacking (C=10, 5 seeds)")
