"""
V31 — Rolling(3d,7d) Only — Fast Submission

No feature ranking. Use ALL non-leak features directly.
5 seeds for CV + 5 seeds for submission.
Target: ~20 min per target total.
"""
...
def main():
    log.info("="*70)
    log.info("V31 — Rolling(3d,7d) Only — All Non-Leak Features — Fast")
    log.info("No feature ranking. Using ALL features after leakage fix.")
    log.info("="*70)
    ...
    # ── Per-target CV with ALL non-leak features ──
    log.info("\n=== Per-target CV (rolling only, ALL features, 5 seeds) ===")
    all_oof={}; all_sel={}; all_losses={}; best_oof_n={}; best_loss_n={}; best_oof_avg_n={}; best_loss_avg_n={}

    for target in TARGET_COLS:
        log.info(f"--- {target} ---")
        leak=LEAK_S if target.startswith('S') else LEAK_Q
        avail=[c for c in all_cols if c not in leak]
        log.info(f"  Available (after leak removal): {len(avail)} features")

        # Try a few N values
        best_n=0; best_cv=float('inf'); best_oof=None
        for n in [30,50,100,200,500,'all']:
            if n=='all':
                sel=avail
            else:
                # Random subset of size n (deterministic per seed)
                rng=np.random.RandomState(42)
                idx=rng.permutation(len(avail))[:n]
                sel=[avail[i] for i in idx]
            oof=cv_f(feat_r, sel, target, SEEDS)
            cal=mm(oof, train_rate[target])
            loss=log_loss(feat_r[target],cal,labels=[0,1])
            log.info(f"  N={n:>5}, Cal OOF={loss:.4f}")
            if loss<best_cv:
                best_cv=loss; best_n=n; best_oof=oof
        ...
        if best_n=='all':
            ...
            idx=rng.permutation(len(avail))[:best_n]
            best_sel=[avail[i] for i in idx]
        all_sel[target]=best_sel
        all_losses[target]=best_cv
        best_oof_n[target]=best_n
        log.info(f"  ✅ Best N={best_n}, Cal OOF={best_cv:.4f}")

    avg=np.mean([all_losses[t] for t in TARGET_COLS])
    log.info(f"\nV31 Cal OOF Avg: {avg:.4f} (V10: 0.6038, Δ: {avg-0.6038:+.4f})")
    ...
    sp=SUBMIT_DIR/f'submission_v31_{ts}.csv'
    predictions.to_csv(sp,index=False)
    log.info(f"✅ Saved: {sp}")

    meta={'version':'v31','submission_file':str(sp),'timestamp':ts,'n_samples':len(predictions),
          ...
          'strategy':'rolling(3d,7d) only — random subset selection (no ranking)',
          'cv_avg':float(avg),
          'per_target':{}}
    for t in TARGET_COLS:
        meta['per_target'][t]={'n_features':len(all_sel[t]),'cal_oof_loss':float(all_losses[t]),
            ...
            'best_n':best_oof_n[t]}
    mp=SUBMIT_DIR/f'meta_v31_{ts}.json'
    ...
    log.info(f"{'Target':<6} {'Best N':<8} {'Cal OOF':<12} {'Test Mean':<12} {'Train':<8} {'Shift':<8}")
    for t in TARGET_COLS:
        log.info(f"{t:<6} {best_oof_n[t]:<8} {all_losses[t]:<12.4f} {predictions[t].mean():<12.4f} {train_rate[t]:<8.3f} {predictions[t].mean()-train_rate[t]:<+8.4f}")
