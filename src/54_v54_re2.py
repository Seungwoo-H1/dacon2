"""
V54_re2 — Pairwise + Transformed Features + Expanded Configs (lightweight)

V54 killed by SIGKILL — triple interactions + combined features too heavy.
This version: pairwise + transforms + expanded configs only.
1,344 models total (5 strategies × 6 configs × 4-5 n_feat × 7 targets)

Simpler and safer for 16GB RAM.
"""
...
# Expanded configs — include V53 winners
...
def main():
    t_start = time.time()
    log.info("=" * 70)
    log.info("V54_re2 — Pairwise + Transformed + Expanded Configs")
    log.info("=" * 70)
    ...
    for target in TARGET_COLS:
        ...
        # Strategy C: Transformed features (log/sqrt/abs)
    ...
    oof_path = DATA_PROCESSED / "oof_v54_re2.csv"
    ...
    meta = {
        'version': 'V54_re2',
        'name': 'Pairwise + Transformed + Expanded Configs',
    ...
    meta_path = DATA_PROCESSED / "v54_re2_meta.json"
