"""
V153 — Wide C Sweep on V152 architecture

Architecture: Same as V152 (2 LGBM + XGB + CB × 7 seeds, top-30 features)
Change: Meta C sweep 100→5000 → 100→50000 (finer granularity at high end)
"""
...
N_FOLDS = 5
N_SEEDS = 7
META_C_SWEEP = [100.0, 300.0, 500.0, 1000.0, 2000.0, 3000.0,
                5000.0, 8000.0, 10000.0, 15000.0, 20000.0, 30000.0, 50000.0]
TOP_K = 30
...
def v153_run(train_df, test_df, feat_cols):
    ...
    sub_path = SUBMIT / f"submission_v153_c_sweep_ext_{ts}.csv"
    sub.to_csv(sub_path, index=False)
    log.info(f"Saved: {sub_path}")
    
    meta_data = {
        'version': 'V153',
        'name': 'Wide C Sweep (100-50000, 7 seeds, top-30)',
    ...
    meta_path = SUBMIT / f'meta_v153_{ts}.json'
...
t_start = time.time()
log.info("=" * 70)
log.info("V153 — Wide C Sweep (100-50000)")
log.info("=" * 70)
...
avg_oof, meta = v153_run(train_df, test_df, feat_cols)
