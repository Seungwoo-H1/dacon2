"""
V52 — Aggressive Feature Selection + Isotonic Calibration

Key insight: V48 (0.5885) beats V50/V51 which used ensemble of 20 seeds.
V48 used binary classification per target with isotonic calibration.
V48 used 20 seeds but averaged predictions.

Hypothesis: The improvement from V48 comes from:
1. Binary classification (not regression)
2. Isotonic calibration on OOF predictions  
3. Simple, focused feature set (per-target top features from LGBM importance)

V52 tries to improve by:
1. More aggressive feature selection: 5, 8, 10, 15, 20 features
2. Per-target feature counts (optimal n per target)
3. Single model per fold with isotonic calibration (no seed averaging)
4. Better LGBM params tuned per target (different lr, nl, md)
5. Also try rank-fusion feature selection (LGBM importance + MI)

The key question: Can we get < 0.5885 avg cal?
"""
...
# Wider, shallower, more aggressive
...
def rank_features_mi(feat, feat_cols, target):
...
def rank_features_fusion(feat, feat_cols, target):
    """Rank features by rank fusion (LGBM + MI)."""
    lgb_ranked = rank_features_importance(feat, feat_cols, target)
    mi_ranked = rank_features_mi(feat, feat_cols, target)
    
    # Convert to rank (1 = best)
...
def main():
    t_start = time.time()
    log.info("=" * 70)
    log.info("V52 — Aggressive Feature Selection + Isotonic Calibration")
    log.info("=" * 70)
    ...
    # ── 3. Experiment ──
    log.info("\n--- 3. Aggressive feature selection + config experiments ---")
    ...
    for target in TARGET_COLS:
        ...
        # Try all config x feature_count combinations
        for cfg_name, cfg in CFGS.items():
            for n_feat in [5, 8, 10, 15, 20]:
                ...
                config_str = f"{cfg_name}_n{n_feat}"
                log.info(f"    {config_str}: iso_cal={iso_cal_loss:.4f}")
    ...
    oof_path = DATA_PROCESSED / "oof_v52.csv"
    ...
    meta = {
        'version': 'V52',
        'name': 'Aggressive Feature Selection + Isotonic Calibration',
    ...
    meta_path = DATA_PROCESSED / "v52_meta.json"
