"""
V16: Personalized + Feature Selection Grid Search — beat V10 (0.6038)

Key differences from V10:
1. Per-subject z-score personalization (same as V10)
2. Feature selection: top-K from ranking (V10 uses top-20, we test 10/20/30/40/50)
3. Config grid: V10, LIGHT, TINY
4. 10 seeds (not 20) to reduce compute
5. n_jobs=1 to avoid SIGKILL (was causing OOM with n_jobs=-1 on this system)
6. OMP_NUM_THREADS=4 for controlled CPU usage
"""
...
def quick_rank(feat, feature_cols, target, n_trees=100):
    """Quick ranking to select top-K features."""
...
# Config presets — n_jobs=1 critical for this system
LGB_V10 = {
...
def cv_predict(scols, target, seeds, spw, cfg=None):
    """GroupKFold × seeds → OOF avg predictions."""
    # Use closure over feat/global
    feat = _FEAT
    y = feat[target].values
...
def cv_predict_light(scols, target, seeds, spw, cfg):
    """Like cv_predict but with custom config."""
    feat = _FEAT
    y = feat[target].values
    gkf = GroupKFold(n_splits=N_SPLITS)
    oof = np.zeros(len(y))
    ...
    for fold, (tr_idx, va_idx) in enumerate(gkf.split(feat, y, feat['subject_id'])):
        ...
        for seed in seeds:
            ...
            vad = lgb.Dataset(Xva, label=yva, reference=trd)
            mdl = lgb.train(sc, trd, num_boost_round=ne,
                valid_sets=[vad])
            seed_sum += mdl.predict(Xva)
        oof[va_idx] = seed_sum / N_SEEDS
    return oof
...
def main():
    ...
    t_total = time.time()
    pr("=" * 70)
    pr("V16: Personalized + Feature Selection Grid Search")
    pr("=" * 70)
    ...
    # Feature ranking per target (100 trees)
    pr("\nRanking features per target...")
    all_ranked = {}
    for target in TARGET_COLS:
        leak = remove_leak(feat_cols, target)
        ranked = quick_rank(_FEAT, leak, target, n_trees=100)
        all_ranked[target] = ranked
        pr(f"  {target}: {len(leak)} features ranked in {time.time()-t0:.1f}s")

    # Configs: V10 (n_jobs=1), LIGHT, TINY
    configs = [
        ('V10', LGB_V10, cv_predict),
        ('LIGHT', LGB_LIGHT, cv_predict_light),
        ('TINY', LGB_TINY, cv_predict_light),
    ]
    ...
    for cname, cfg, cv_fn in configs:
        pr(f"\n--- Config: {cname} ---")
