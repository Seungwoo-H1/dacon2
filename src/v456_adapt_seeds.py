"""
V456 — V452 Pipeline + Per-Target Seed Count Optimization

Hypothesis: V452(0.580)에서 25 seeds 모든 target에 동일 적용.
각 target별 optimal seed count가 다를 수 있음:
- Q1(0.655): noisy → 더 많은 seed 필요
- S1(0.530): easy → 적은 seed로 충분
V456은:
1. Target별 adaptive seed count: Q1=30, Q2=25, Q3=25, S1=20, S2=22, S3=22, S4=25
2. V452 pipeline 그대로
3. V446 meta config 유지
"""
...
ADAPTIVE_SEEDS = {'Q1': 30, 'Q2': 25, 'Q3': 25, 'S1': 20, 'S2': 22, 'S3': 22, 'S4': 25}
...
def main():
    ...
    log.info("=" * 70)
    log.info("V456 — Per-Target Adaptive Seed Count")
    log.info("=" * 70)
    ...
    log.info("  Adding z-score + baseline features...")
    ...
    sub_path = SUBMIT / f"submission_v456_adapt_seeds_{ts}.csv"
    sub.to_csv(sub_path, index=False)
    meta_data = {'version': 'V456', 'name': 'Per-Target Adaptive Seeds',
        ...
        'student_meta_gap': round(float(gap), 5), 'submission_file': str(sub_path),
        'timestamp': ts, 'total_time_s': round(time.time() - t_start, 0)}
    meta_path = EXPERIMENTS / f'v456_{ts}.json'
