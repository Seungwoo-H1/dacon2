"""
V51 — Isotonic Calibration + V37-style Per-Target Configs

Key insight from V48/V50:
- Isotonic calibration consistently improves calibration loss
- V48 used binary classification + isotonic → best per-target cal
- V50 used multi-seed ensemble + isotonic → slightly worse than V48
- Key difference: V48 used individual models per target, V50 used ensemble of seeds

V51 tries:
1. V37-style per-target configs (more aggressive for each target)
2. Isotonic calibration on OOF predictions
3. Multiple seeds (20 seeds like V37)
4. Feature counts: 10, 15, 20 per target
5. Also try V48's binary classification approach with V37 configs
"""
...
# V48-style configs (binary + isotonic worked best)
V48_CFGS = {
    "Q1": {"nl": 15, "md": 4, "lr": 0.03, "ne": 1000, "ss": 0.7, "cb": 0.7, "ra": 1.0, "rl": 3.0, "mc": 10},
    "Q2": {"nl": 15, "md": 4, "lr": 0.03, "ne": 1000, "ss": 0.7, "cb": 0.7, "ra": 1.0, "rl": 3.0, "mc": 10},
    "Q3": {"nl": 15, "md": 4, "lr": 0.03, "ne": 1000, "ss": 0.7, "cb": 0.7, "ra": 1.0, "rl": 3.0, "mc": 10},
    "S1": {"nl": 15, "md": 4, "lr": 0.03, "ne": 1000, "ss": 0.7, "cb": 0.7, "ra": 1.0, "rl": 3.0, "mc": 10},
    "S2": {"nl": 15, "md": 4, "lr": 0.03, "ne": 1000, "ss": 0.7, "cb": 0.7, "ra": 1.0, "rl": 3.0, "mc": 10},
    "S3": {"nl": 15, "md": 4, "lr": 0.03, "ne": 1000, "ss": 0.7, "cb": 0.7, "ra": 1.0, "rl": 3.0, "mc": 10},
    "S4": {"nl": 15, "md": 4, "lr": 0.03, "ne": 1000, "ss": 0.7, "cb": 0.7, "ra": 1.0, "rl": 3.0, "mc": 10},
}
...
def train_cv_oof(feat, cols, target, seeds, cfg=None, n_folds=5):
    ...
    cfg_full = {
        ...
        'num_leaves': cfg.get('nl', 15), 'max_depth': cfg.get('md', 4),
        'learning_rate': cfg.get('lr', 0.03), 'n_estimators': cfg.get('ne', 1000),
        'subsample': cfg.get('ss', 0.7), 'colsample_bytree': cfg.get('cb', 0.7),
        'reg_alpha': cfg.get('ra', 1.0), 'reg_lambda': cfg.get('rl', 3.0),
        'min_child_samples': cfg.get('mc', 10),
    }
    ...
    for si, seed in enumerate(seeds):
        ...
        for tr, va in gkf.split(feat, y, feat['subject_id']):
            ...
            m = lgb.train(cfg_seed, ds, num_boost_round=cfg.get('ne', 1000),
                         valid_sets=[vd],
...
def main():
    t_start = time.time()
    log.info("=" * 70)
    log.info("V51 — Isotonic Calibration + V37-style Per-Target Configs")
    log.info("=" * 70)
    ...
    # ── 3. Experiment: V37 configs + isotonic ──
    log.info("\n--- 3. V37 config + isotonic experiments ---")
    ...
    for target in TARGET_COLS:
        ...
        # Evaluate multiple feature counts: 10, 15, 20
        # With V37 configs + isotonic
        ...
        for n_feat in [10, 15, 20]:
            ...
            # V37-style config
            oof_v37 = train_cv_oof(feat, sel_cols, target, SEEDS, cfg=V37_CFGS.get(target, V48_CFGS.get(target, {})))
            oof_v37_avg = np.clip(oof_v37.mean(axis=1), 0.0001, 0.9999)

            iso_v37, ok_v37 = isotonic_calibrate(oof_v37_avg, y)
            if ok_v37:
                iso_v37 = mean_match(iso_v37, train_rates[target])
                iso_v37_loss = log_loss(y, iso_v37, labels=[0, 1])
            else:
                iso_v37_loss = log_loss(y, oof_v37_avg, labels=[0, 1])
                iso_v37 = oof_v37_avg

            results_detail[f"v37_n{n_feat}"] = {
                'no_cal': log_loss(y, oof_v37_avg, labels=[0, 1]),
                'iso_cal': iso_v37_loss,
            }

            log.info(f"    V37 n={n_feat}: no_cal={results_detail[f'v37_n{n_feat}']['no_cal']:.4f}, iso_cal={iso_v37_loss:.4f}")

            if iso_v37_loss < best_overall_cal:
                best_overall_cal = iso_v37_loss
                best_overall_oof = iso_v37
                best_config = f"v37_n{n_feat}"
                best_n = n_feat
    ...
    oof_path = DATA_PROCESSED / "oof_v51.csv"
    ...
    meta = {
        'version': 'V51',
        'name': 'Isotonic Calibration + V37-style Per-Target Configs',
    ...
    meta_path = DATA_PROCESSED / "v51_meta.json"
