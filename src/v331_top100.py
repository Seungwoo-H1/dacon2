"""
V331 — V329 + Aggressive Top-100 Feature Selection (Noise Removal)

Hypothesis: V329's 2047 features contain noise. Top-100 selection should
improve generalization, especially for student models.

Approach: Keep V329 feature engineering pipeline, but rank features
and only use top-100 per target for actual training.
"""
...
TOP_K = 100  # V331: only top-100 features
...
def main():
    ...
    log.info("=" * 70)
    log.info("V331 — V329 + Aggressive Top-100 Feature Selection")
    log.info("Hypothesis: Top-100 from 2047 features reduces noise, improves generalization")
    log.info("=" * 70)
    ...
    log.info(f"\nTotal features before selection: train={len(train_feat_cols)}, test={len(test_feat_cols)}")
    ...
    for t in TARGETS:
        ...
        log.info(f"  Ranked {len(feat_cols_clean)} -> top {TOP_K}")
        
        for si in range(N_SEEDS):
            seed = SEED + si * 7
            rng = np.random.RandomState(seed)
            
            # Feature bagging on top-K
            n_bag = max(int(TOP_K * FEATURE_BAG_FRACTION), TOP_K // 2)
            bag = rng.choice(top_feats, size=n_bag, replace=False)
            bag_set = set(bag)
            
            sel_cols = [c for c in top_feats if c in test_feat_cols]
            if len(sel_cols) == 0:
                sel_cols = [c for c in top_feats if c in test_feat_cols]
            ...
            if si < 3 or si == N_SEEDS - 1:
                ...
                log.info(f"    Seed {si:2d}: OOF={s_oof:.5f} (top-{TOP_K})")
    ...
    log.info(f"\n{'='*70}")
    log.info(f"V331 RESULTS (Top-{TOP_K} Feature Selection)")
    log.info(f"{'='*70}")
    for t in TARGETS:
    ...
    sub_path = SUBMIT / f"submission_v331_top100_{ts}.csv"
    sub.to_csv(sub_path, index=False)
    log.info(f"Saved submission: {sub_path}")
    
    meta_data = {
        'version': 'V331',
        'name': f'V329 + Aggressive Top-{TOP_K} Feature Selection',
        'avg_oof': round(float(avg_oof), 5),
        'n_features_total': len(train_feat_cols),
        'n_features_used': TOP_K,
        'n_seeds': N_SEEDS,
