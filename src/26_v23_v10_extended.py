"""
V23: V10-based pipeline with Extended Features + XGBoost + Bayesian Opt
...
4. Per-subject z-score personalization (V10 style)
5. Feature leakage fix: wrist nighttime data excluded from S targets
...
def main():
    ...
    t_total = time.time()
    pr("=" * 70)
    pr("V23: V10 Extended — Features + XGBoost + Bayesian Opt")
    pr("=" * 70)
    ...
    pr(f"\nV23 best-per-target avg: {best_avg:.6f} (V10: 0.6038, Δ={best_avg-0.6038:+.6f})")
    ...
    sub_path = SUBMIT_DIR / f"submission_v23_{timestamp}.csv"
    predictions.to_csv(sub_path, index=False)
    
    meta_path = SUBMIT_DIR / f"meta_v23_{timestamp}.json"
