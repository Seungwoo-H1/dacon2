"""
V334 — V329 + Meta C=500 + 30 Seeds Combined

Hypothesis: V312 showed C=500 > C=10. V313 showed 30 seeds > 15 seeds.
V334 combines both for maximum ensemble power with V329's strong features.

This is the "kitchen sink" variant: best features (V329) + best ensemble size (30) +
best meta learning (C=500).

Note: V313 with C=500 + 30 seeds had OOF 0.595 but LB 0.647 (OOF-LB gap too large).
V334 tests if V329's features can overcome that gap while keeping the ensemble power.
"""
...
SEED = 42
N_FOLDS = 5
N_SEEDS = 30       # Combined: 30 seeds
META_C = 500.0     # Combined: C=500
...
def main():
    ...
    log.info("=" * 70)
    log.info("V334 — V329 + Meta C=500 + 30 Seeds Combined")
    log.info("Best features + Best ensemble + Best meta learner")
    log.info("=" * 70)
    ...
    for t in TARGETS:
        ...
        for si in range(N_SEEDS):
            ...
            if si < 3 or si % 10 == 0 or si == N_SEEDS - 1:
                s_oof = log_loss(y, seed_oof)
                log.info(f"    Seed {si:2d}: OOF={s_oof:.5f} (30 seeds, C=500, {cfg_name})")
    
    # Meta learner with C=500
    target_oofs = {}
    student_avg_oofs = {}
    ...
    log.info(f"\n{'='*70}")
    log.info(f"V334 RESULTS (30 Seeds + Meta C=500 Combined)")
    log.info(f"{'='*70}")
    for t in TARGETS:
    ...
    log.info(f"  V329:  0.54365 | V332: ?? | V313: 0.59512")
    ...
    sub_path = SUBMIT / f"submission_v334_combined_c500_30seeds_{ts}.csv"
    sub.to_csv(sub_path, index=False)
    log.info(f"Saved submission: {sub_path}")
    
    meta_data = {
        'version': 'V334',
        'name': 'V329 + Meta C=500 + 30 Seeds Combined',
