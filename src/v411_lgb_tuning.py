"""
V411 — LGBM Seed Hyperparameter Tuning

Hypothesis: V410 showed that reducing features to K=13(Q1) lowered student OOF from 0.820 to 0.744.
The remaining student OOF bottleneck (Q1 0.794, Q3 0.785) is mainly due to LGBM seed overfitting.
By using more aggressive LGBM hyperparameters (lower lr, more reg), we can reduce seed overfitting
and lower student OOF further.

V411: Two levels of LGBM hyperparameters:
- Standard (V402): cfg from V53_SWEEP
- Aggressive: lower lr (0.01), higher reg_alpha (2.0), higher reg_lambda (5.0)
- Sweep per-target to find optimal trade-off
"""
...
def main():
    ...
    log.info("=" * 70)
    log.info("V411 — LGBM Seed Hyperparameter Tuning")
    log.info("Hypothesis: Stronger LGBM reg → lower student OOF")
    log.info("=" * 70)
    ...
    # Per-target LGBM configs to sweep
    # For Q targets: try aggressive and medium
    # For S targets: try aggressive, medium, and original
    LGB_SWEEP = {
        'Q1': ['aggressive', 'medium', 'deep'],
        'Q2': ['aggressive', 'medium', 'deep'],
        'Q3': ['aggressive', 'medium', 'v48'],
        'S1': ['aggressive', 'medium', 'wide'],
        'S2': ['aggressive', 'medium', 'deep'],
        'S3': ['aggressive', 'medium', 'safety'],
        'S4': ['aggressive', 'medium', 'wide'],
    }
    ...
    for t in TARGETS:
        ...
        log.info(f"    Original config: {original_cfg_name}, n_feat: {n_feat}")

        best_score = float('inf')
        best_lgb_name = original_cfg_name
        results = {}

        for lgb_name in LGB_SWEEP[t]:
    ...
    log.info(f"\n{'='*70}")
    log.info(f"V411 RESULTS (LGBM Seed Hyperparameter Tuning)")
    log.info(f"{'='*70}")
    for t in TARGETS:
    ...
    sub_path = SUBMIT / f"submission_v411_lgb_tuning_{ts}.csv"
    sub.to_csv(sub_path, index=False)
    log.info(f"Saved submission: {sub_path}")

    meta_data = {
        'version': 'V411',
        'name': 'LGBM Seed Hyperparameter Tuning',
    ...
    meta_path = EXPERIMENTS / f'v411_{ts}.json'
