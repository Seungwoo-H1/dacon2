"""
V397 — Aggressive Per-Target Meta C

Hypothesis: More aggressive per-target meta C improves meta OOF while
maintaining V392's calibration balance.

V392: Q→C=10, S→C=100 → meta 0.617, student 0.692 (same as V308)
V392 Q targets have high student (Q1: 0.784, Q3: 0.743)
V392 S targets have low student (S1: 0.599, S3: 0.626)

V397: Q→C=5 (more regularization to reduce Q student), S→C=200 (more capacity)
→ expect: Q student↓, S meta↓, overall student closer to V308
"""
...
# V397: More aggressive per-target C
META_C_PER_TARGET = {
    'Q1': 5.0, 'Q2': 5.0, 'Q3': 5.0,
    'S1': 200.0, 'S2': 200.0, 'S3': 200.0, 'S4': 200.0,
}
...
def main():
    ...
    log.info("=" * 70)
    log.info("V397 — Aggressive Per-Target Meta C")
    log.info("Hypothesis: Q→C=5, S→C=200 reduces Q student while improving S meta")
    log.info("V392: Q→C=10, S→C=100 → meta 0.617, student 0.692")
    log.info("V392 Q student: Q1=0.784, Q2=0.702, Q3=0.743 (high)")
    log.info("V397: Q→C=5 (stronger reg), S→C=200 (more capacity)")
    log.info("=" * 70)
    ...
    log.info(f"\n{'='*70}")
    log.info(f"V397 RESULTS (Aggressive Per-Target Meta C: Q→5, S→200)")
    log.info(f"{'='*70}")
    for t in TARGETS:
    ...
    # V392 comparison
    log.info(f"\n  V392 comparison:")
    log.info(f"    V392 meta: 0.61672, student: 0.69212")
    log.info(f"    V397 meta: {avg_oof:.5f}, student: {avg_student:.5f}")
    ...
    sub_path = SUBMIT / f"submission_v397_aggressive_per_target_c_{ts}.csv"
    sub.to_csv(sub_path, index=False)
    log.info(f"Saved submission: {sub_path}")

    meta_data = {
        'version': 'V397',
        'name': 'Aggressive Per-Target Meta C',
    ...
    meta_path = EXPERIMENTS / f'v397_{ts}.json'
