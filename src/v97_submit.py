"""
V97 Submission: V53 Swept + Temperature Scaling Calibration
Temperature scaling improved AVG CalOOF from 0.6813 to 0.6354 (-0.0459).

Optimal T per target (fitted on OOF, applied to test):
  Q1: 3.246, Q2: 1.917, Q3: 1.733, S1: 1.236,
  S2: 2.260, S3: 1.292, S4: 1.602
"""
...
def main():
    t_start = time.time()
    log.info("="*80)
    log.info("V97 Submission: V53 Swept + Temperature Scaling")
    log.info("="*80)
    ...
    for target in TARGETS:
        ...
        log.info(f"  CV={cv_loss:.4f}, CalOOF={cal_oof_loss:.4f}, T={T:.3f}, "
    ...
    sub_path = SUBMIT / f"submission_v97_temp_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
    ...
    log.info(f"\n{'='*80}")
    log.info(f"✅ V97 Submission saved: {sub_path}")
    log.info(f"  Shape: {sub.shape}")
    avg_cal = np.mean([v['cal_oof'] for v in all_meta.values()])
    log.info(f"  AVG CalOOF: {avg_cal:.4f}")
    for t in TARGETS:
        log.info(f"  {t}: CV={all_meta[t]['cv']:.4f} CalOOF={all_meta[t]['cal_oof']:.4f} T={all_meta[t]['T']:.3f} "
    ...
    # Save meta
    meta = {
        'version': 'V97_temperature',
        'name': 'V53 Swept + Temperature Scaling',
        'n_seeds': n_seeds,
        'cal_method': 'temperature_scaling',
        'v53_sweep': V53_SWEEP,
    ...
    meta_path = SUBMIT / f'meta_v97_temperature_{datetime.now().strftime("%Y%m%d_%H%M%S")}.json'
