"""
V448 — V446 Baseline + Feature Importance Re-ranking + Double Ensembling

Hypothesis: V446(V339 0.589)이 stabilizer.
V448은 V446 pipeline 그대로 + feature ranking을 두 번:
1. First pass: rank with shallow model (num_leaves=10, depth=3)
2. Second pass: rank with deeper model (num_leaves=30, depth=6)
3. Average the ranks → more stable feature selection
4. Double ensembling: (LGBM v1 + LGBM v2) / 2 where v2 uses different params
"""
...
NFEAT = {'Q1': 28, 'Q2': 28, 'Q3': 24, 'S1': 32, 'S2': 30, 'S3': 32, 'S4': 30}
...
def rank_features_lgb(feat_df, feat_cols, target, seed=SEED, params_override=None):
...
def main():
    ...
    log.info("=" * 70)
    log.info("V448 — Double Ensembling + Stable Feature Ranking")
    log.info("=" * 70)
    ...
    # ===== Phase 1: Double Ensembling LGBM =====
    all_oof_seed = {}
    all_test_seed = {}
    ...
    for t_idx, target in enumerate(TARGETS):
        ...
        # Stable feature ranking: average of shallow + deep rankings
        rank_shallow = []
        rank_deep = []
        for fold in range(N_FOLDS):
            r1 = rank_features_lgb(train_with_target, feat_cols, target,
                                   seed=SEED + fold,
                                   params_override={'num_leaves': 10, 'max_depth': 3})
            r2 = rank_features_lgb(train_with_target, feat_cols, target,
                                   seed=SEED + fold + 100,
                                   params_override={'num_leaves': 40, 'max_depth': 7, 'n_estimators': 50})
            rank_shallow.append(r1)
            rank_deep.append(r2)

        # Average ranks
        all_ranks = rank_shallow + rank_deep
        feat_counts = {}
        for fl in all_ranks:
            for rank, f in enumerate(fl):
                feat_counts[f] = feat_counts.get(f, 0) + rank
        ranked_features = sorted(feat_counts.items(), key=lambda x: x[1])  # lower rank = better
        top_features = [f for f, r in ranked_features[:n_feat]]
    ...
    log.info(f"  V446 V339: 0.58859 (for comparison)")
    log.info(f"{'='*70}")
    ...
    sub_path = SUBMIT / f"submission_v448_double_ens_{ts}.csv"
    sub.to_csv(sub_path, index=False)
    log.info(f"Saved: {sub_path}")

    meta_data = {
        'version': 'V448',
        'name': 'Double Ensembling + Stable Feature Ranking',
    ...
    meta_path = EXPERIMENTS / f'v448_{ts}.json'
