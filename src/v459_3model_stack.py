"""
V459 — V452 Features + 3-Model Stacking + Gap-Controlled Meta

Hypothesis: V458(0.574)은 heterogeneous ensemble 효과적 but gap이 너무 좁음(0.92x).
V459는:
1. V452 pipeline (z³ + bz²) — proven feature set
2. 3-model stacking (LGBM-shallow, LGBM-balanced, LGBM-deep) — model diversity
3. Meta learner가 V452와 동일한 reg config(reg_alpha=0.01, reg_lambda=0.0) 사용 → gap 1.2-1.3x 유지
4. Per-target feature ranking by averaging 3-model importance → 더 robust한 feature selection

Key: V452의 gap 안정성 + V458의 diversity 결합.
"""
...
# 3 configs: V459의 핵심 — LGBM diversity
CFGS = [
    # Config 1: Shallow + Strong Regularization (V452 narrow-ish)
    {'num_leaves': 12, 'max_depth': 3, 'learning_rate': 0.015, 'n_estimators': 2000,
     'subsample': 0.55, 'colsample_bytree': 0.55, 'reg_alpha': 6.0, 'reg_lambda': 25.0,
     'min_child_samples': 30},
    # Config 2: Balanced (V452 default)
    ...
    # Config 3: Deep + Low Regularization
    {'num_leaves': 35, 'max_depth': 5, 'learning_rate': 0.02, 'n_estimators': 1200,
...
def main():
    ...
    log.info("=" * 70)
    log.info("V459 — V452 Features + 3-Model Stacking + V446 Meta")
    log.info("=" * 70)
    ...
    log.info("\n=== Phase 1: 3-Model Stacking ===")
    # model_oof_seeds[cfg_idx][target] = array shape (n_train, n_seeds)
    ...
    for cfg_idx in range(len(CFGS)):
        ...
        for t_idx, target in enumerate(TARGETS):
            ...
            # 5-fold rank averaging for stable feature selection
            fold_ranks = []
            for fold in range(5):
    ...
    # ===== Phase 2: V446 Meta (reg_alpha=0.01, reg_lambda=0.0 for stable gap) =====
    log.info("\n=== Phase 2: V446 Meta (Gap-Controlled) ===")
    student_oofs = {}; test_preds = {}; meta_oofs = {}

    for t_idx, target in enumerate(TARGETS):
        y = train_df[target].values
        # Average predictions across all 3 configs
        ...
        # V446 meta config: reg_alpha=0.01 → stable gap ~1.2x
        ...
        log.info(f"  {target}: meta={meta_oofs[target]:.5f}, student={student_oofs[target]:.5f}, gap={student_oofs[target]-meta_oofs[target]:.5f}")
    ...
    log.info(f"  V458: 0.57436 | V452: 0.58019")
    log.info(f"{'='*70}")
    ...
    sub_path = SUBMIT / f"submission_v459_3model_stack_{ts}.csv"
    sub.to_csv(sub_path, index=False)
    meta_data = {'version': 'V459', 'name': '3-Model Stacking + V446 Meta',
    ...
    meta_path = EXPERIMENTS / f'v459_{ts}.json'
