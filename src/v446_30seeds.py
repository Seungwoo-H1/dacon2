"""
V446 — V443 Pipeline + More Seeds + Feature Selection Tuning

Hypothesis: V443(z-score+baseline poly)이 현재 최고(V339 0.59208).
V446은 V443 pipeline 그대로 + 더 많은 seeds(30) + stricter feature selection.
"""
...
def main():
    ...
    log.info("=" * 70)
    log.info("V446 — V443 Pipeline + 30 Seeds")
    log.info("=" * 70)
    ...
    sub_path = SUBMIT / f"submission_v446_30seeds_{ts}.csv"
    sub.to_csv(sub_path, index=False)
    log.info(f"Saved: {sub_path}")

    meta_data = {
        'version': 'V446',
        'name': 'V443 Pipeline + 30 Seeds',
