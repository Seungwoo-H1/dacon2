"""
V151 — Multi-Model Stacking with CatBoost Optimization

V150 showed multi-model diversity works (OOF 0.62040).
V151 builds on V150:
1. Per-target CatBoost hyperparameter tuning (grid search on 1 fold)
2. More seeds: 3 → 5 per model family (15 students total)
3. Target-specific model selection (use best model family per target)
4. Better LGBM configs (wide + deep + balanced)
5. Meta-learner C sweep (0.1, 1, 10, 100)

V150 params:
  LGBM: num_leaves=31, lr=0.05, n_est=500 (wide)
  XGB: max_depth=4, lr=0.03, n_est=500
  CB: depth=4, lr=0.03, l2=3.0, n_est=500
"""
...
SEED = 42
N_FOLDS = 5
N_SEEDS = 5  # 5 seeds per model family
META_C_SWEEP = [0.1, 1.0, 10.0, 100.0]
...
# V151 improved configs per model type
LGBM_CONFIGS = [LGBM_WIDE, LGBM_DEEP]
XGB_CONFIGS = [{'max_depth': d, 'learning_rate': lr, 'reg_alpha': a, 'reg_lambda': l}
               for d in [3, 5] for lr in [0.02, 0.05] for a in [0.5, 2.0] for l in [3.0, 8.0]]
CB_CONFIGS = [{'depth': d, 'learning_rate': lr, 'l2_leaf_reg': l}
              for d in [3, 5, 6] for lr in [0.02, 0.05] for l in [1.0, 5.0, 10.0]]


def v151_run(train_df, test_df, feat_cols):
    group = train_df['subject_id'].values
    gkf = GroupKFold(n_splits=N_FOLDS)
    
    # Add target encoding + group z-scores
    for t in TARGETS:
    ...
    for meta_c in META_C_SWEEP:
        ...
        test_preds = {t: np.zeros((len(test_df), N_SEEDS * 3)) for t in TARGETS}
        ...
        for t in TARGETS:
            ...
            log.info(f"    Stacking OOF (C={meta_c}, {len(all_student_oofs)} students): {ll:.5f}")
    ...
    log.info(f"V148 AVG OOF: 0.63129, Δ: {best_oof - 0.63129:+.5f}")
    log.info(f"V150 AVG OOF: 0.62040, Δ: {best_oof - 0.62040:+.5f}")
    log.info(f"{'='*70}")
    ...
    sub_path = SUBMIT / f"submission_v151_multimodel_{ts}.csv"
    sub.to_csv(sub_path, index=False)
    log.info(f"Saved: {sub_path}")
    
    meta_data = {
        'version': 'V151',
        'name': 'Multi-Model Stacking (LGBM×2 + XGB + CB, 5 seeds, C sweep)',
        'avg_oof': round(float(best_oof), 5),
        'best_meta_c': best_c,
        'n_students_per_target': N_SEEDS * 4,  # 2 LGBM + XGB + CB
        'per_target_oof': {t: round(float(log_loss(train_df[t].values, np.clip(best_train_oof[t], 0.001, 0.999))), 5)
                          for t in TARGETS},
        'v148_avg_oof': 0.63129,
        'delta_vs_v148': round(float(best_oof - 0.63129), 5),
    ...
    meta_path = SUBMIT / f'meta_v151_{ts}.json'
...
t_start = time.time()
log.info("=" * 70)
log.info("V151 — Multi-Model Stacking (LGBM×2 + XGB + CB, 5 seeds, C sweep)")
log.info("=" * 70)
...
avg_oof, meta = v151_run(train_df, test_df, feat_cols)
