"""
V447 — Z-Score + Baseline Poly + Feature Clustering + Adaptive Reg

Hypothesis: V446(z-score+baseline poly+30 seeds)이 V339 0.589.
V447은:
1. Per-subject feature correlation features: 각 feature 쌍의 subject 내 correlation
2. Adaptive regularization per target based on student OOF
3. Feature clustering: highly correlated features를 1개 representative로 통합
4. Deeper feature selection: per-target top feature count optimization
5. Stacking: 5-fold OOF → meta → 다시 5-fold OOF for refined predictions
"""
...
def main():
    ...
    log.info("=" * 70)
    log.info("V447 — Feature Clustering + Adaptive Reg + Stacking")
    log.info("=" * 70)
    ...
    # ===== Per-target feature counts: adapt based on previous results =====
    # S targets benefit from more features, Q need fewer
    NFEAT = {'Q1': 30, 'Q2': 30, 'Q3': 25, 'S1': 35, 'S2': 32, 'S3': 35, 'S4': 32}
    ...
    for t_idx, target in enumerate(TARGETS):
        ...
        # Adaptive regularization based on student OOF
        stu_oof = student_oofs[target]
        if stu_oof > 0.62:  # high student OOF → stronger reg
            reg_alpha, reg_lambda = 0.1, 2.0
        elif stu_oof > 0.58:
            reg_alpha, reg_lambda = 0.05, 1.0
        else:  # good student OOF → weak reg
            reg_alpha, reg_lambda = 0.01, 0.0
    ...
    sub_path = SUBMIT / f"submission_v447_adapt_reg_{ts}.csv"
    sub.to_csv(sub_path, index=False)
    log.info(f"Saved: {sub_path}")

    meta_data = {
        'version': 'V447',
        'name': 'Feature Clustering + Adaptive Reg + Stacking',
    ...
    meta_path = EXPERIMENTS / f'v447_{ts}.json'
