"""
V398 — Adaptive Feature Threshold

Hypothesis: Instead of fixed top-K features, use MI threshold to select
features adaptively per target. V308/V392 use fixed n_feat (11-21) which
may include noise for some targets.

V392: fixed n_feat + per-target C → meta 0.617, student 0.692
V398: MI threshold + per-target C → adaptive feature count per target

Key: use mutual information to filter features before ranking.
Features with MI < threshold are excluded, then rank the rest.
"""
...
# MI threshold: keep features with MI > threshold
MI_THRESHOLD = 0.002  # very small MI indicates noise
...
def main():
    ...
    log.info("=" * 70)
    log.info("V398 — Adaptive Feature Threshold (MI-based)")
    log.info("Hypothesis: MI threshold removes noisy features before ranking")
    log.info("V392: fixed top-K → meta 0.617, student 0.692")
    log.info(f"V398: MI > {MI_THRESHOLD} filter → adaptive feature count")
    log.info("=" * 70)
    ...
    for t in TARGETS:
        ...
        # Step 1: MI-based filtering
        X_base = train_df[feat_cols_clean].fillna(0).values.astype(np.float64)
        spw_y = max(((y == 0).sum()) / max((y == 1).sum(), 1), 0.1)

        # Compute MI (discretize continuous y for MI)
        # For binary target, MI is well-defined
        mi_scores = mutual_info_classif(X_base, y, random_state=SEED)
        mi_pairs = sorted(zip(feat_cols_clean, mi_scores), key=lambda x: -x[1])

        # Filter by threshold
        mi_filtered = [name for name, score in mi_pairs if score >= MI_THRESHOLD]
        n_before = len(feat_cols_clean)
        n_after = len(mi_filtered)
        n_feat_used[t] = n_after

        log.info(f"    MI filter: {n_before} → {n_after} features (threshold={MI_THRESHOLD})")
        log.info(f"    Top-5 MI: {[f'{name}={score:.4f}' for name, score in mi_pairs[:5]]}")

        if n_after == 0:
            mi_filtered = [name for name, score in mi_pairs[:n_feat]]
            n_feat_used[t] = len(mi_filtered)
            log.warning(f"    All features below threshold, using top-{n_feat}")

        # Step 2: Rank MI-filtered features by LGBM importance
        ranked = rank_features(train_df, mi_filtered, t)
        sel_cols = ranked[:max(n_feat, len(ranked))]

        # Ensure we use at most n_feat
        ...
        log.info(f"    Config: {cfg_name}, n_feat: {n_feat} (from MI-{n_after})")
        ...
        # Level 1: Stack → per-target LR meta-learner
    ...
    log.info(f"\n{'='*70}")
    log.info(f"V398 RESULTS (Adaptive MI Threshold Filtering)")
    log.info(f"{'='*70}")
    for t in TARGETS:
        meta_c_t = META_C_PER_TARGET[t]
        nf = n_feat_used.get(t, '?')
        log.info(f"  {t}: Meta={per_target_oof[t]:.5f}, Student={per_target_student[t]:.5f}, MI-filt={nf}, C={meta_c_t}")
    ...
    sub_path = SUBMIT / f"submission_v398_mi_threshold_{ts}.csv"
    sub.to_csv(sub_path, index=False)
    log.info(f"Saved submission: {sub_path}")

    meta_data = {
        'version': 'V398',
        'name': 'Adaptive MI Threshold Filtering',
        ...
        'n_feat_used_per_target': {t: int(n_feat_used.get(t, 0)) for t in TARGETS},
    ...
    meta_path = EXPERIMENTS / f'v398_{ts}.json'
