"""
13_v12_catboost.py — V12: CatBoost ensemble + V10 configs + V11 features (rejected)
Strategy:
1. Use V10 features only (V11 features rejected) + V10 configs
2. Add CatBoost (5 seeds) to ensemble with LightGBM (20 seeds)
3. Stacked ensemble: LGB 20 seeds + CB 5 seeds = 25 seeds total
4. Weighted blending: optimize weights on OOF
"""
...
def cb_cv(feat, scols, target, seeds, spw, cfg):
    """CatBoost CV."""
    ...
    for si, seed in enumerate(seeds):
        ...
        for fold, (ti, vi) in enumerate(gkf.split(feat, y, feat["subject_id"])):
            ...
            ytr, yva = y[ti], y[vi]
            trd = Pool(Xtr, ytr, cat_features=[])
            vad = Pool(Xva, yva, cat_features=[])
            mdl = CatBoostClassifier(**sc, verbose=0, iterations=cfg["n_estimators"])
...
def main():
    t_total = time.time()
    log.info("="*70)
    log.info("V12: CatBoost ensemble + V10 features + V10 configs")
    log.info("="*70)
    ...
    # Load V10 features + personalization
    ...
    log.info(f"Features: {len(feat_cols)} base + {len(pcols)} z-score = {len(all_cols)}")
    
    # Rank features per target (V10 style)
    ...
    # V12: Evaluate LGB only, then LGB+CB ensemble
    log.info("\n=== Step 1: V12 LGB-only (baseline for comparison) ===")
    lgb_oof = {}
    lgb_scols = {}
    ...
    log.info(f"\nLGB-only avg: {lgb_avg:.6f} (V10: {v10_avg:.6f}, diff: {lgb_avg-v10_avg:+.6f})")
    
    # Check if CB is available
    log.info("\n=== Step 2: Adding CatBoost ===")
    try:
        import catboost
        log.info(f"CatBoost version: {catboost.__version__}")
    except ImportError:
        log.info("CatBoost NOT installed. Trying pip install...")
        import subprocess
        r = subprocess.run(["/home/mwoo423/miniconda3/envs/open/bin/pip", "install", "catboost", "-q"], capture_output=True)
        if r.returncode != 0:
            log.info(f"pip install failed: {r.stderr.decode()[:200]}")
            log.info("❌ CatBoost install failed. Trying V12 without CatBoost...")
            log.info("→ Will use 20-seed LGB ensemble with different configs instead")
            
            # Fallback: try different LGB configs
            log.info("\n=== Fallback: Try different LGB configs ===")
            extra_configs = [
                {"name":"C7","nl":10,"md":3,"lr":0.015,"ne":300,"ss":0.8,"cst":0.8,"ra":0.5,"rl":2.0,"mc":10},
                {"name":"C8","nl":8,"md":3,"lr":0.01,"ne":400,"ss":0.7,"cst":0.8,"ra":1.0,"rl":3.0,"mc":15},
                {"name":"C9","nl":12,"md":4,"lr":0.01,"ne":500,"ss":0.75,"cst":0.75,"ra":0.3,"rl":1.0,"mc":10},
            ]
            
            best_overall_avg = lgb_avg
            best_overall_oof = lgb_oof.copy()
            best_overall_cfgs = {}
            
            for ec in extra_configs:
                log.info(f"\n  Testing config {ec['name']}...")
                cfg = {"objective":"binary","metric":"binary_logloss","verbose":-1,
                    "num_leaves":ec["nl"],"max_depth":ec["md"],"learning_rate":ec["lr"],
                    "n_estimators":ec["ne"],"subsample":ec["ss"],"colsample_bytree":ec["cst"],
                    "reg_alpha":ec["ra"],"reg_lambda":ec["rl"],"min_child_samples":ec["mc"],
                    "force_row_wise":True,"n_jobs":-1}
                ...
                for target in TARGET_COLS:
                    leak = remove_leak(all_cols, target)
                    ranked = rank_feat(feat, leak, target)
                    n_feats = ec.get("_n_feats", tc.get("_n_feats", 10))
                    scols = [r[0] for r in ranked[:n_feats]]
                    y = feat[target].values; np_ = max((y==1).sum(),1); nn = (y==0).sum(); spw = nn/np_
                    oof = lgb_cv(feat, scols, target, RANDOM_SEEDS, spw, cfg)
                    oof_loss = log_loss(y, oof, labels=[0,1])
                    test_oof[target] = oof
                    log.info(f"    {target}: {oof_loss:.4f}")
                
                test_avg = np.mean([log_loss(feat[t], test_oof[t], labels=[0,1]) for t in TARGET_COLS])
                log.info(f"  {ec['name']} avg: {test_avg:.6f} (current best: {best_overall_avg:.6f})")
                
                if test_avg < best_overall_avg:
                    best_overall_avg = test_avg
                    best_overall_oof = test_oof
                    best_overall_cfgs = {t: ec for t in TARGET_COLS}
                    log.info(f"  ✓ New best!")
            
            if best_overall_avg < v10_avg - 0.001:
                log.info(f"\n✅ Fallback config beats V10! avg: {best_overall_avg:.6f}")
                lgb_oof = best_overall_oof
                v10_cfgs_for_v12 = best_overall_cfgs
            else:
                log.info(f"\n❌ No fallback config beats V10. Stopping.")
                return
        else:
            log.info("CatBoost installed. Retrying with CatBoost...")
            exec(open(__file__).read().replace("import catboost", "import catboost"))  # re-run with CB
            return
    
    # CatBoost available — do full V12 with CB
    # CatBoost configs (conservative)
    CB_CFGS = [
    ...
    for target in TARGET_COLS:
        ...
        best_cb_avg = float("inf")
        best_cb_oof = None
        best_cb_cfg = None
        
        for cb_cfg in CB_CFGS:
            cfg = {"iterations":cb_cfg["iterations"],"depth":cb_cfg["depth"],
                "learning_rate":cb_cfg["learning_rate"],"l2_leaf_reg":cb_cfg["l2_leaf_reg"],
                "bagging_temperature":cb_cfg["bagging_temperature"],"random_strength":cb_cfg["random_strength"],
                "subsample":cb_cfg["subsample"],"loss_function":"Logloss",
                "one_hot_max_size":3,"use_best_model":False,
                "scale_pos_weight":spw}
            t0 = time.time()
            oof = cb_cv(feat, scols, target, RANDOM_SEEDS_CB, spw, cfg)
            elapsed = time.time() - t0
            oof_loss = log_loss(y, oof, labels=[0,1])
            log.info(f"    {cb_cfg['name']}: {oof_loss:.4f} ({elapsed:.1f}s)")
            if oof_loss < best_cb_avg:
                best_cb_avg = oof_loss
                best_cb_oof = oof
                best_cb_cfg = cb_cfg
        
        cb_oof[target] = best_cb_oof
        cb_scols[target] = scols
        cb_best_cfg[target] = best_cb_cfg
        v10_cal = v10_cal_oof[target]
        log.info(f"    Best CB: {best_cb_cfg['name']}, OOF={best_cb_avg:.4f}, V10={v10_cal:.4f}")
    
    # Weighted ensemble: LGB + CB
    log.info("\n=== Step 3: Weighted Ensemble (LGB + CB) ===")
    
    # Find optimal weights on OOF
    best_blend = float("inf")
    best_w_lgb, best_w_cb = 0.5, 0.5
    
    for w in np.arange(0.0, 1.05, 0.05):
        blended = {}
        for t in TARGET_COLS:
            b = w * lgb_oof[t] + (1-w) * cb_oof[t]
            blended[t] = b
        blend_loss = np.mean([log_loss(feat[t], blended[t], labels=[0,1]) for t in TARGET_COLS])
        if blend_loss < best_blend:
            best_blend = blend_loss
            best_w_lgb, best_w_cb = w, 1-w
    
    log.info(f"  Best blend: LGB={best_w_lgb:.1f}, CB={best_w_cb:.1f}, loss={best_blend:.6f}")
    
    # Also try LGB-only if better
    log.info(f"  LGB-only: {lgb_avg:.6f}")
    log.info(f"  CB-only:  {np.mean([log_loss(feat[t], cb_oof[t], labels=[0,1]) for t in TARGET_COLS]):.6f}")
    
    if lgb_avg < best_blend:
        log.info("  LGB-only is better. Use LGB only.")
        v12_oof = lgb_oof
        v12_avg = lgb_avg
    elif best_blend < v10_avg - 0.001:
        log.info(f"\n✅ V12 BEATS V10!")
        v12_oof = {}
        for t in TARGET_COLS:
            v12_oof[t] = best_w_lgb * lgb_oof[t] + best_w_cb * cb_oof[t]
        v12_avg = best_blend
    else:
        log.info(f"\n❌ V12 NOT better than V10. Stopping.")
        return
    ...
    log.info(f"  Avg cal OOF: {avg_cal:.6f} (V10: {v10_avg:.6f}, diff: {avg_cal-v10_avg:+.6f})")
    ...
    for target in TARGET_COLS:
        ...
        # CB predictions if used
        if "cb_oof" in dir() and target in cb_oof:
            cap = np.zeros(len(tX))
            cb_cfg = cb_best_cfg[target]
            cb_lp = {"iterations":cb_cfg["iterations"],"depth":cb_cfg["depth"],
                "learning_rate":cb_cfg["learning_rate"],"l2_leaf_reg":cb_cfg["l2_leaf_reg"],
                "bagging_temperature":cb_cfg["bagging_temperature"],"random_strength":cb_cfg["random_strength"],
                "subsample":cb_cfg["subsample"],"loss_function":"Logloss",
                "one_hot_max_size":3,"scale_pos_weight":spw}
            for si, seed in enumerate(RANDOM_SEEDS_CB):
                sp = {**cb_lp, "random_state":seed}
                da = lgb.Dataset(Xa, label=y_a, feature_name=sn, params={"verbose":"-1"})
                # CB on full train
                from catboost import CatBoostClassifier, Pool
                cb_pool = Pool(Xa, y_a, cat_features=[])
                mdl = CatBoostClassifier(**sp, verbose=0)
                mdl.fit(cb_pool)
                cap += mdl.predict(tX, prediction_type="Probability")[:, 1]
            cap /= N_SEEDS_CB
        else:
            cap = ap  # LGB only fallback
        
        # Blend
        blended = np.zeros(len(tX))
        if "cb_oof" in dir():
            blended = best_w_lgb * ap + best_w_cb * cap
        else:
            blended = ap
    ...
    meta_out = {"version":"v12","submission_file":str(sp),"timestamp":ts,"n_samples":len(preds),
        ...
        "improvements":["CatBoost ensemble (5 seeds)","weighted blending on OOF","V10 features + configs"],
        "lgb_only_avg":float(lgb_avg),"avg_cal":float(avg_cal),
        "per_target":{}}
    for t in TARGET_COLS:
