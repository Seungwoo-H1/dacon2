"""
V412 — LGBM Seed Hyperparameter Fine-Tuning (V411 기반)

Hypothesis: V411 achieved Student OOF 0.654 with per-target LGBM tuning.
But V411 used only 3 configs per target (aggressive/medium/original).
By expanding the sweep to more granular configs, we may find even better
per-target params that further reduce student OOF.

V412: Per-target LGBM hyperparameter sweep with 9+ configs per target
- lr: [0.005, 0.01, 0.015, 0.02]
- num_leaves: [6, 10, 15, 20]
- reg_alpha: [1.0, 3.0, 5.0, 10.0]
- reg_lambda: [5.0, 10.0, 20.0]
- min_child_samples: [15, 20, 30]

This is a large sweep, but we'll limit it by:
1. Only sweeping on the best config family from V411
2. Using a smaller set of combinations (grid subset)
"""
...
CFGS_V411 = {
    ...
    'aggressive': {'num_leaves': 10, 'max_depth': 3, 'learning_rate': 0.01, 'n_estimators': 2000,
                   'subsample': 0.5, 'colsample_bytree': 0.5, 'reg_alpha': 5.0, 'reg_lambda': 20.0, 'min_child_samples': 30},
...
# V412 per-target fine-tuned configs — additional variants around V411 best
CFGS_V412 = {
    'ultra_aggressive': {'num_leaves': 8, 'max_depth': 3, 'learning_rate': 0.008, 'n_estimators': 2500,
                         'subsample': 0.4, 'colsample_bytree': 0.4, 'reg_alpha': 8.0, 'reg_lambda': 30.0, 'min_child_samples': 40},
    'aggressive':       {'num_leaves': 10, 'max_depth': 3, 'learning_rate': 0.01, 'n_estimators': 2000,
                         'subsample': 0.5, 'colsample_bytree': 0.5, 'reg_alpha': 5.0, 'reg_lambda': 20.0, 'min_child_samples': 30},
    ...
    'medium':           {'num_leaves': 15, 'max_depth': 4, 'learning_rate': 0.015, 'n_estimators': 1500,
                         'subsample': 0.6, 'colsample_bytree': 0.6, 'reg_alpha': 3.0, 'reg_lambda': 10.0, 'min_child_samples': 20},
    'soft_medium':      {'num_leaves': 18, 'max_depth': 4, 'learning_rate': 0.018, 'n_estimators': 1500,
                         'subsample': 0.65, 'colsample_bytree': 0.65, 'reg_alpha': 2.0, 'reg_lambda': 8.0, 'min_child_samples': 15},
    'wide':             {'num_leaves': 30, 'max_depth': 3, 'learning_rate': 0.05, 'n_estimators': 300,
                         'subsample': 0.8, 'colsample_bytree': 0.8, 'reg_alpha': 2.0, 'reg_lambda': 5.0, 'min_child_samples': 5},
    'safety':           {'num_leaves': 10, 'max_depth': 3, 'learning_rate': 0.02, 'n_estimators': 1000,
                         'subsample': 0.6, 'colsample_bytree': 0.6, 'reg_alpha': 3.0, 'reg_lambda': 10.0, 'min_child_samples': 20},
...
def main():
    ...
    log.info("=" * 70)
    log.info("V412 — LGBM Seed Hyperparameter Fine-Tuning (V411 기반)")
    log.info("Hypothesis: More granular LGBM params → lower student OOF")
    log.info("=" * 70)
    ...
    # Per-target sweep configs — V411 best + additional variants
    V411_BEST = {
        'Q1': 'medium', 'Q2': 'medium', 'Q3': 'aggressive',
        'S1': 'wide', 'S2': 'medium', 'S3': 'safety', 'S4': 'medium',
    }

    # For each target, sweep: V411 best + nearby variants
    # Limit total experiments: 7 targets × ~6 configs = ~42 runs
    # Each run = 15 seeds × 5 folds = 75 train calls
    # Total: 42 × 75 = 3150 train calls — might be slow
    # Optimization: for each target, only sweep 4 configs max

    SWEEP_PER_TARGET = {
        'Q1':  ['ultra_aggressive', 'aggressive', 'soft_aggressive', 'medium'],
        'Q2':  ['ultra_aggressive', 'aggressive', 'soft_aggressive', 'medium'],
        'Q3':  ['ultra_aggressive', 'aggressive', 'soft_aggressive', 'medium'],
        'S1':  ['ultra_deep', 'wide', 'broad', 'soft_medium'],
        'S2':  ['ultra_aggressive', 'aggressive', 'soft_aggressive', 'medium'],
        'S3':  ['ultra_aggressive', 'aggressive', 'safety', 'soft_aggressive'],
        'S4':  ['ultra_deep', 'wide', 'broad', 'soft_medium'],
    }
    ...
    for t in TARGETS:
        ...
        log.info(f"    Original: {original_cfg_name}, V411 best: {v411_best}")

        best_score = float('inf')
        best_lgb_name = v411_best
        results = {}

        for lgb_name in SWEEP_PER_TARGET[t]:
            lgb_cfg = CFGS_V411.get(lgb_name, CFGS_V412[lgb_name])
        ...
        final_lgb_cfg = CFGS_V411.get(best_lgb_name, CFGS_V412[best_lgb_name])
    ...
    log.info(f"\n{'='*70}")
    log.info(f"V412 RESULTS (LGBM Fine-Tuning V411 기반)")
    log.info(f"{'='*70}")
    for t in TARGETS:
    ...
    sub_path = SUBMIT / f"submission_v412_lgb_finetune_{ts}.csv"
    sub.to_csv(sub_path, index=False)
    log.info(f"Saved submission: {sub_path}")

    meta_data = {
        'version': 'V412',
        'name': 'LGBM Fine-Tuning (V411 기반)',
    ...
    meta_path = EXPERIMENTS / f'v412_{ts}.json'
