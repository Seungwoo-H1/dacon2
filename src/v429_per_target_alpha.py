"""
V429 — V413 LGBM + Per-Target XGB Meta Alpha Sweep

Hypothesis: V415 showed XGB meta with α=0.1, λ=1.0 is better than LR.
But V415 used same alpha for all targets. Different targets may need different
meta regularization levels. This experiment sweeps per-target XGB alpha values
on V413's LGBM base to find optimal per-target meta regularization.

V429:
1. LGBM base: V413 configs, 15 seeds (identical to V413)
2. Per-target XGB meta with target-specific alpha sweep
3. Find best alpha per target that minimizes OOF while keeping student stable
"""
...
N_FOLDS = 5
N_SEEDS = 15
ALPHA_SWEEP = [0.01, 0.05, 0.1, 0.5, 1.0, 5.0, 10.0]
...
def main():
    ...
    log.info("=" * 70)
    log.info("V429 — V413 LGBM + Per-Target XGB Meta Alpha Sweep")
    log.info(f"Alpha sweep: {ALPHA_SWEEP}")
    log.info("=" * 70)
    ...
    # ===== Phase 2: Per-Target XGB Meta Alpha Sweep =====
    log.info("\n=== Phase 2: Per-Target XGB Meta Alpha Sweep ===")
    ...
    for t_idx, target in enumerate(TARGETS):
        ...
        for alpha in ALPHA_SWEEP:
            meta_model = XGBClassifier(
                n_estimators=15, max_depth=3, reg_alpha=alpha, reg_lambda=1.0,
                learning_rate=0.1, subsample=0.8, colsample_bytree=0.8,
                random_state=SEED, n_jobs=1, min_child_weight=5, verbosity=0)
            meta_model.fit(X_meta, y)
            meta_pred = meta_model.predict_proba(X_meta)[:, 1]
            oof_val = log_loss(y, meta_pred)
            alpha_results.append((alpha, oof_val))

            if oof_val < best_oof:
                best_oof = oof_val
                best_alpha = alpha

        meta_oofs[target] = best_oof
        per_target_best[target] = best_alpha

        # Use best alpha for final model
        meta_model = XGBClassifier(
            n_estimators=15, max_depth=3, reg_alpha=best_alpha, reg_lambda=1.0,
            learning_rate=0.1, subsample=0.8, colsample_bytree=0.8,
            random_state=SEED, n_jobs=1, min_child_weight=5, verbosity=0)
        meta_model.fit(X_meta, y)
        test_preds[target] = meta_model.predict_proba(X_test_meta)[:, 1]

        # Student = simple average
        student_oofs[target] = log_loss(y, oof_preds.mean(axis=1))

        log.info(f"  {target}: best_alpha={best_alpha}, meta_oof={best_oof:.5f}, "
                 f"student={student_oofs[target]:.5f}")
        log.info(f"    Alpha sweep: {[(a, f'{o:.5f}') for a, o in alpha_results]}")
    ...
    log.info(f"\n{'='*70}")
    log.info("V429 Results:")
    log.info(f"  Best per-target alphas: {per_target_best}")
    ...
    sub_path = SUBMIT / f"submission_v429_per_target_alpha_sweep_{ts}.csv"
    sub.to_csv(sub_path, index=False)
    log.info(f"Saved submission: {sub_path}")

    meta_data = {
        'version': 'V429',
        'name': 'V413 LGBM + Per-Target XGB Meta Alpha Sweep',
        ...
        'v308_gap': 0.070,
        'n_seeds': N_SEEDS,
        'meta_type': 'xgb_per_target_alpha_sweep',
        'best_alphas': per_target_best,
    ...
    meta_path = EXPERIMENTS / f'v429_{ts}.json'
