"""
V460 — V458 Pipeline + Cross-Validation Leave-Group-Out Stacking + Target-Specific Reg

Hypothesis: V458(0.574)이 heterogeneous ensemble으로 breakthrough.
하지만 gap 0.92x가 너무 좁아 실제 LB가 더 높을 수 있음.
V460은:
1. V458의 3-model pipeline 유지 (LGBM-shallow, XGB, LGBM-deep)
2. 하지만 XGB → LGBM-another로 변경 (모두 LGBM이 but different hyperparams)
3. Cross-validation stacking: meta learner는 OOF predictions으로 학습
4. Per-target regularization tuning for gap control

Key: Same architecture diversity but different split → more stable gap.
"""
...
N_FOLDS = 5
N_SEEDS = 15
NFEAT = {'Q1': 22, 'Q2': 22, 'Q3': 20, 'S1': 26, 'S2': 24, 'S3': 26, 'S4': 24}  # Fewer feats → less overfit

# 3 LGBM configs with maximal diversity
CFGS = [
    {'num_leaves': 10, 'max_depth': 2, 'learning_rate': 0.01, 'n_estimators': 3000,
     ...
     'min_child_samples': 40},  # Very shallow + strong reg
...
def main():
    ...
    log.info("=" * 70)
    log.info("V460 — V458 Pipeline + Max Diversity + Fewer Features")
    log.info("=" * 70)
    ...
    log.info(f"  V458: 0.57436 | V459: 0.58126 | V452: 0.58019")
    log.info(f"{'='*70}")
    ...
    sub_path = SUBMIT / f"submission_v460_max_diversity_{ts}.csv"
    sub.to_csv(sub_path, index=False)
    meta_data = {'version': 'V460', 'name': 'Max Diversity + Fewer Features',
    ...
    meta_path = EXPERIMENTS / f'v460_{ts}.json'
