"""
V442 — Per-Subject Z-Score Standardization + Interaction

Hypothesis: V440의 feat*subj_mean interaction이 student 0.620을 만들었음.
V442는 더 근본적인 접근: subject별로 feature를 z-score 표준화.
- feat_z = (feat - subj_mean) / max(subj_std, eps)
- 이걸 하면 baseline effect가 자연스럽게 제거됨 (V432 baseline sub의 자동화)
- + interaction features (feat_z * global_mean) 추가
- + baseline feature (subject event rate) 추가
"""
...
def per_subject_zscore(df, subject_col='subject_id', exclude_cols=None):
    """Per-subject z-score standardization."""
    if exclude_cols is None:
        exclude_cols = META_COLS | set(TARGETS)
    num_cols = [c for c in df.columns
                if c not in exclude_cols and np.issubdtype(df[c].dtype, np.number)]

    zscore_df = df.copy()
    for col in num_cols:
        subj_mean = df.groupby(subject_col)[col].transform('mean')
        subj_std = df.groupby(subject_col)[col].transform('std').fillna(0)
        subj_std = subj_std.replace(0, 1)  # avoid div by zero
        zscore_df[f'z_{col}'] = (df[col] - subj_mean) / subj_std
    return zscore_df
...
def main():
    ...
    log.info("=" * 70)
    log.info("V442 — Per-Subject Z-Score Standardization + Interaction")
    log.info("=" * 70)
    ...
    # ===== Per-subject z-score + interaction =====
    log.info("  Adding per-subject z-score features...")
    ...
    # Interaction: z_feat * global_mean_of_feat
    log.info("  Adding interaction features...")
    int_train = pd.DataFrame(index=train_df.index)
    int_test = pd.DataFrame(index=test_df.index)
    for col in num_cols:
        gmean = train_df[col].mean()
        int_train[f'int_z_{col}_gmean'] = zscore_train[f'z_{col}'] * gmean
        int_test[f'int_z_{col}_gmean'] = zscore_test[f'z_{col}'] * gmean

    # Also add feat*subj_mean (V440 style) on original features
    for col in num_cols:
        tr_mean_s = train_df.groupby('subject_id')[col].transform('mean')
        te_mean_s = test_df.groupby('subject_id')[col].transform('mean')
        int_train[f'int_orig_{col}_ssmean'] = train_df[col] * tr_mean_s
        int_test[f'int_orig_{col}_ssmean'] = test_df[col] * te_mean_s

    # Combine: original features + zscore + interactions + baseline
    all_train_features = pd.concat([train_df[num_cols], zscore_train, int_train], axis=1)
    all_test_features = pd.concat([test_df[num_cols], zscore_test, int_test], axis=1)
    ...
    for t_idx, target in enumerate(TARGETS):
        ...
        log.info(f"{target}: zscore_feat_oof={avg_oof:.5f}")
    ...
    sub_path = SUBMIT / f"submission_v442_zscore_{ts}.csv"
    sub.to_csv(sub_path, index=False)
    log.info(f"Saved: {sub_path}")

    meta_data = {
        'version': 'V442',
        'name': 'Per-Subject Z-Score Standardization + Interaction',
    ...
    meta_path = EXPERIMENTS / f'v442_{ts}.json'
