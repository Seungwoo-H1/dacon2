"""
V427 — V418 Hybrid Base + Subject Bias Shrinkage + XGB Meta

Hypothesis: V418's hybrid LGBM+XGB base models give rich meta features
(meta OOF 0.561), but the student-meta gap (0.078) is slightly wider than ideal.
V423 showed subject bias removal can further reduce meta OOF while maintaining
student OOF. Combining both: apply partial subject-level shrinkage to the
base model predictions BEFORE stacking, which should:
1. Reduce meta OOF further by removing subject-specific noise
2. Keep student OOF similar (shrinkage is mild)
3. Narrow student-meta gap toward V308 levels

V427:
1. LGBM base: V413 configs, 15 seeds (same as V418)
2. XGB base: per-target XGB, 15 seeds (same as V418)
3. Apply partial subject bias correction (factor=0.3) to base predictions
4. Meta features = concat(corrected_LGBM preds, corrected_XGB preds)
5. Meta learner: low-reg XGB (α=0.1, λ=1.0)
"""
...
N_FOLDS = 5
N_SEEDS = 15
BIAS_SHRINKAGE = 0.3  # factor for subject bias correction
...
def apply_subject_shrinkage(predictions_2d, subject_ids, shrinkage=0.3):
    """Apply partial subject-level bias correction to 2D predictions [n_samples, n_seeds].
    For each subject, compute mean prediction across seeds, then shrink toward global mean.
    """
    n_samples, n_seeds = predictions_2d.shape
    global_mean = predictions_2d.mean()
    corrected = np.zeros_like(predictions_2d)

    for sid in np.unique(subject_ids):
        mask = subject_ids == sid
        subj_mean = predictions_2d[mask].mean(axis=0)  # per-seed mean for this subject
        corrected[mask] = global_mean + (1 - shrinkage) * (subj_mean - global_mean)
...
def main():
    ...
    log.info("=" * 70)
    log.info("V427 — V418 Hybrid Base + Subject Bias Shrinkage + XGB Meta")
    log.info(f"Hypothesis: Hybrid base + partial subject shrinkage (factor={BIAS_SHRINKAGE})")
    log.info("=" * 70)
    ...
    # ===== Phase 3: Subject Bias Shrinkage + Hybrid Meta =====
    log.info("\n=== Phase 3: Subject Shrinkage + Hybrid Meta ===")
    ...
    for t_idx, target in enumerate(TARGETS):
        ...
        # Apply subject shrinkage to base predictions
        shrink_lgb = apply_subject_shrinkage(lgb_oof_all[target], subject_ids, BIAS_SHRINKAGE)
        shrink_xgb = apply_subject_shrinkage(xgb_oof_all[target], subject_ids, BIAS_SHRINKAGE)



        # Concat shrunken LGBM + XGB → 30 meta features
        X_meta = np.hstack([shrink_lgb, shrink_xgb])
        X_test_meta = np.hstack([lgb_test_all[target], xgb_test_all[target]])  # no shrinkage on test
        ...
        # Student = avg of base predictions (no shrinkage for student metric)
    ...
    log.info(f"  AVG Student OOF: {avg_student:.5f} (Δ vs V418 {0.63979}: {avg_student-0.63979:+.5f})")
    ...
    sub_path = SUBMIT / f"submission_v427_hybrid_shrink_{ts}.csv"
    sub.to_csv(sub_path, index=False)
    log.info(f"Saved submission: {sub_path}")

    meta_data = {
        'version': 'V427',
        'name': 'V418 Hybrid Base + Subject Bias Shrinkage + XGB Meta',
        ...
        'v418_avg_student': 0.63979, 'v418_avg_meta': 0.56132,
        'v308_lb': 0.63893,
        ...
        'delta_vs_v418_meta': round(float(avg_meta - 0.56132), 5),
        ...
        'v308_gap': 0.070,
        'n_seeds': N_SEEDS,
        'meta_type': f'xgb_shrink_{BIAS_SHRINKAGE}_hybrid_30feat',
