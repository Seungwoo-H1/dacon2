"""
V428 — V418 Hybrid + Adaptive Per-Target Shrinkage

Hypothesis: V418's hybrid base gives low meta OOF (0.561) but gap is high (0.078).
V427 showed uniform shrinkage (factor=0.3) reduces gap but also kills signal.

V428 applies ADAPTIVE shrinkage:
- Per-target shrinkage based on target's student-meta gap
- Q targets (stable): shrinkage=0.0 (no shrinkage, keep signal)
- S1,S2 (stable): shrinkage=0.1 (minimal)
- S3 (unstable, gap=0.104): shrinkage=0.4 (aggressive)
- S4 (moderate): shrinkage=0.15

Goal: Keep V418's low meta OOF while narrowing gap only where needed.
"""
...
# Adaptive shrinkage factors per target (based on V427 analysis)
SHRINKAGE = {
    'Q1': 0.0, 'Q2': 0.0, 'Q3': 0.0,   # Q targets: no shrinkage
    'S1': 0.1, 'S2': 0.1,               # S1,S2: minimal
    'S3': 0.4,                           # S3: aggressive
    'S4': 0.15,                           # S4: mild
}
...
def apply_adaptive_shrinkage(preds_2d, subject_ids, factor):
    """Apply subject-level shrinkage with given factor to all seeds."""
    n_samples, n_seeds = preds_2d.shape
    global_mean = preds_2d.mean()
    corrected = np.zeros_like(preds_2d)

    for sid in np.unique(subject_ids):
        mask = subject_ids == sid
        subj_mean = preds_2d[mask].mean(axis=0)
        corrected[mask] = global_mean + (1 - factor) * (subj_mean - global_mean)
...
def main():
    ...
    log.info("=" * 70)
    log.info("V428 — V418 Hybrid + Adaptive Per-Target Shrinkage")
    log.info(f"Shrinkage factors: {SHRINKAGE}")
    log.info("=" * 70)
    ...
    # ===== Phase 3: Adaptive Shrinkage + Hybrid Meta =====
    log.info("\n=== Phase 3: Adaptive Shrinkage + Hybrid Meta ===")
    ...
    for t_idx, target in enumerate(TARGETS):
        ...
        shrink_factor = SHRINKAGE[target]

        # Apply adaptive shrinkage
        shrink_lgb = apply_adaptive_shrinkage(lgb_oof_all[target], subject_ids, shrink_factor)
        shrink_xgb = apply_adaptive_shrinkage(xgb_oof_all[target], subject_ids, shrink_factor)
        ...
        # Student = avg of base predictions (no shrinkage)
        ...
        log.info(f"  {target} (shrink={shrink_factor}): "
    ...
    sub_path = SUBMIT / f"submission_v428_adaptive_shrink_{ts}.csv"
    sub.to_csv(sub_path, index=False)
    log.info(f"Saved submission: {sub_path}")

    meta_data = {
        'version': 'V428',
        'name': 'V418 Hybrid Base + Adaptive Per-Target Shrinkage + XGB Meta',
        ...
        'v308_gap': 0.070,
        'n_seeds': N_SEEDS,
        'meta_type': f'xgb_adaptive_shrink_hybrid_30feat',
    ...
    meta_path = EXPERIMENTS / f'v428_{ts}.json'
