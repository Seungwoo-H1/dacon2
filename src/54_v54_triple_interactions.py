"""
V54 — Triple Interactions + Transformed Features + Expanded Configs

V53 achieved 0.5479 with pairwise interactions. Improvements:
1. Triple (3-way) interactions on top features
2. Log/exp transforms on skewed features
3. Expanded config pool (6 configs including V53 winners)
4. 50 seeds × 6 configs × 4 n_feat × 3 strategies = ~2520 models per target

Key hypothesis: Higher-order interactions will capture non-linear patterns better.
"""
...
CFG_V53WINNER = {'nl': 20, 'md': 5, 'lr': 0.02, 'ne': 1000, 'ss': 0.7, 'cb': 0.6, 'ra': 0.5, 'rl': 2.0, 'mc': 15}

CFGS = {'wide': CFG_WIDE, 'deep': CFG_DEEP, 'v48': CFG_V48, 'safety': CFG_SAFETY,
        'extra_deep': CFG_EXTRA_DEEP, 'v53winner': CFG_V53WINNER}
...
def add_transformed_features(feat, top_features):
    """Add log, sqrt, exp transforms on top features."""
    feat = feat.copy()
    added = []
    ...
    for f in top_features[:15]:
        ...
        # Log transform (use absolute value, avoid log(0))
        col_log = f'{f}_log'
...
def add_triple_interactions(feat, top_features):
    """Add triple (3-way) interactions on top features."""
    feat = feat.copy()
    added = []
    
    # Top 6 features → C(6,3) = 20 triple interactions
    top3 = top_features[:6]
    for i in range(len(top3)):
        for j in range(i+1, len(top3)):
            for k in range(j+1, len(top3)):
                f1, f2, f3 = top3[i], top3[j], top3[k]
                if f1 not in feat.columns or f2 not in feat.columns or f3 not in feat.columns:
                    continue
                col_triple = f'{f1}_x_{f2}_x_{f3}'
                feat[col_triple] = (feat[f1].fillna(0) * feat[f2].fillna(0) * feat[f3].fillna(0))
                added.append(col_triple)
...
def main():
    t_start = time.time()
    log.info("=" * 70)
    log.info("V54 — Triple Interactions + Transformed Features + Expanded Configs")
    log.info("=" * 70)
    ...
    for target in TARGET_COLS:
        ...
        # Strategy A: Base (no interactions) — quick baseline
        ...
        feat_pair, added_pair = add_pairwise_interactions(feat, top_features)
        ...
        # Strategy C: Triple interactions
        feat_triple, added_triple = add_triple_interactions(feat, top_features)
        all_cols_triple = get_feature_cols(feat_triple)
        all_cols_triple = [c for c in all_cols_triple if c not in META | set(TARGET_COLS)]
        ranked_triple = rank_features_importance(feat_triple, remove_leak(all_cols_triple, target), target)

        for cfg_name, cfg in CFGS.items():
            for n_feat in [8, 10, 15, 20]:
                sel_cols = ranked_triple[:n_feat]
                oof = train_cv_model(feat_triple, sel_cols, y, SEEDS, cfg, n_folds=5)
                oof_avg = np.clip(oof.mean(axis=1), 0.0001, 0.9999)
                iso_cal, ok = isotonic_calibrate(oof_avg, y)
                if ok:
                    iso_cal = mean_match(iso_cal, train_rates[target])
                ...
                config_str = f"triple_{cfg_name}_n{n_feat}"
                if iso_cal_loss < best_overall_cal:
                    best_overall_cal = iso_cal_loss
                    best_overall_oof = iso_cal
                    best_config_str = config_str

        # Strategy D: Transformed features (log/sqrt/abs)
        feat_trans, added_trans = add_transformed_features(feat, ranked[:15])
        ...
        # Strategy E: Combined (pairwise + transforms)
        feat_combined, _ = add_pairwise_interactions(feat, ranked[:10])
        feat_combined, _ = add_transformed_features(feat_combined, ranked[:15])
        all_cols_combined = get_feature_cols(feat_combined)
        all_cols_combined = [c for c in all_cols_combined if c not in META | set(TARGET_COLS)]
        ranked_combined = rank_features_importance(feat_combined, remove_leak(all_cols_combined, target), target)

        for cfg_name, cfg in CFGS.items():
            for n_feat in [10, 15, 20, 30]:
                sel_cols = ranked_combined[:n_feat]
                oof = train_cv_model(feat_combined, sel_cols, y, SEEDS, cfg, n_folds=5)
                oof_avg = np.clip(oof.mean(axis=1), 0.0001, 0.9999)
                iso_cal, ok = isotonic_calibrate(oof_avg, y)
                if ok:
                    iso_cal = mean_match(iso_cal, train_rates[target])
                ...
                config_str = f"combined_{cfg_name}_n{n_feat}"
                if iso_cal_loss < best_overall_cal:
                    best_overall_cal = iso_cal_loss
                    best_overall_oof = iso_cal
                    best_config_str = config_str
    ...
    oof_path = DATA_PROCESSED / "oof_v54.csv"
    ...
    meta = {
        'version': 'V54',
        'name': 'Triple Interactions + Transformed Features + Expanded Configs',
    ...
    meta_path = DATA_PROCESSED / "v54_meta.json"
