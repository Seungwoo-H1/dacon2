"""
09_v11_remake.py - V11 Remake: V10 baseline + lightweight new features
Strategy: V10 code + 15 new features (date, interaction, stats)
NO heavy personalization z-score expansion (keep V10's approach)
"""
...
def tune_target(feat, fcols, target):
...
def main():
    log.info("="*70)
    log.info("V11 Remake: V10 + 15 new features (date, interaction, stats)")
    log.info("="*70)
    
    # Load V10 features (153 cols)
    log.info("\n=== Load V10 features ===")
    feat = pd.read_parquet("data_processed/features.parquet")
    log.info(f"V10 features: {feat.shape}")
    
    # Add V11 features to V10 base
    log.info("\n=== Add V11 features ===")
    ...
    log.info(f"Total features: {len(feat.columns) - len(META_COLS) - len(TARGET_COLS)}")
    
    # Personalization (V10 style)
    log.info("\n=== Personalization ===")
    feat_cols = get_feat_cols(feat)
    feat, pcols = add_personalization(feat, feat_cols)
    log.info(f"Personalization: {len(pcols)} z-score cols added")
    
    # V10 baseline OOF check
    log.info("\n=== V10 baseline check (without new features) ===")
    v10_feat = pd.read_parquet("data_processed/features.parquet")
    v10_cols = get_feat_cols(v10_feat)
    v10_feat, v10_pcols = add_personalization(v10_feat, v10_cols)
    v10_train_rate = {}
    for t in TARGET_COLS:
        v10_train_rate[t] = float(v10_feat[t].mean())
    
    # Quick V10 OOF check per target
    v10_results = {}
    for target in TARGET_COLS:
        leak = remove_leak(v10_cols + v10_pcols, target)
        ranked = rank_features(v10_feat, leak, target)
        best_v10_cv, best_v10_oof = float("inf"), None
        best_v10_cfg, best_v10_cols = None, None
        
        for nf in [10, 20, 30]:
            if nf > len(ranked): continue
            sc = [r[0] for r in ranked[:nf]]
            y = v10_feat[target].values; np_ = max((y==1).sum(),1); nn = (y==0).sum(); spw = nn/np_
            for cfg in LGB_CONFS:
                tc = {**LGB_BASE,"num_leaves":cfg["nl"],"max_depth":cfg["md"],"learning_rate":cfg["lr"],
                    "n_estimators":cfg["ne"],"subsample":cfg["ss"],"colsample_bytree":cfg["cst"],
                    "reg_alpha":cfg["ra"],"reg_lambda":cfg["rl"],"min_child_samples":cfg["mc"]}
                oa, of, cl, cs, fl = lgb_cv_predict(v10_feat, sc, target, RANDOM_SEEDS, spw)
                tr = y.mean(); ps = abs(oa.mean() - tr)
                score = cl + 0.5*cs + 0.1*ps
                if score < best_v10_cv:
                    best_v10_cv, best_v10_oof = score, oa
                    best_v10_cfg, best_v10_cols = {**cfg,"_n_feats":nf}, sc
        
        y = v10_feat[target].values
        oof_loss = log_loss(y, best_v10_oof, labels=[0,1])
        v10_results[target] = {"loss": oof_loss, "cfg": best_v10_cfg, "cols": best_v10_cols, "oof": best_v10_oof}
        log.info(f"  V10 {target}: OOF={oof_loss:.4f}, config={best_v10_cfg}, feats={len(best_v10_cols)}")
    
    v10_avg = np.mean([v10_results[t]["loss"] for t in TARGET_COLS])
    log.info(f"\n  V10 avg OOF: {v10_avg:.4f} (meta says 0.6038)")
    ...
    log.info(f"{'Target':<6} {'V10 OOF':<10} {'V11 OOF':<10} {'Diff':<10}")
    v11_avg = 0
    for t in TARGET_COLS:
        y = feat[t].values; v11_loss = log_loss(y, all_oof[t], labels=[0,1])
        v10_loss = v10_results[t]["loss"]
        diff = v11_loss - v10_loss
        v11_avg += v11_loss
        better = "✓" if diff < 0 else "✗"
        log.info(f"  {t:<6} {v10_loss:<10.4f} {v11_loss:<10.4f} {diff:+.4f} {better}")
    v11_avg /= 7
    v10_avg_meta = 0.6038
    log.info(f"\n  V10 avg OOF (meta): {v10_avg_meta:.4f}")
    log.info(f"  V10 avg OOF (recomputed): {v10_avg:.4f}")
    log.info(f"  V11 avg OOF: {v11_avg:.4f}")
    log.info(f"  Improvement: {v11_avg - v10_avg_meta:+.4f}")
    
    # If V11 < V10, generate submission; otherwise stop
    if v11_avg >= v10_avg_meta:
    ...
    meta = {"version":"v11","submission_file":str(sp),"timestamp":ts,"n_samples":len(preds),"n_seeds":N_SEEDS,
        "n_splits":N_SPLITS,
        "improvements":["date features (day_of_week, is_weekend, month, quarter)","cross-sensor interaction (7)","ratio features (3)"],
        "per_target":{}}
    for t in TARGET_COLS:
        ol = log_loss(feat[t], all_oof[t], labels=[0,1])
        cl = log_loss(feat[t], cal_oof[t], labels=[0,1])
        meta["per_target"][t] = {"config":all_cfg[t],"n_features":len(all_cols[t]),
    ...
    log.info(f"\n{'='*70} V11 SUMMARY {'='*70}")
    log.info(f"{'Target':<6} {'V10 OOF':<10} {'V11 OOF':<10} {'V11 Cal':<10} {'Test Mean':<10} {'Train Rate':<10}")
    for t in TARGET_COLS:
        ol = log_loss(feat[t], all_oof[t], labels=[0,1])
        cl = log_loss(feat[t], cal_oof[t], labels=[0,1])
        log.info(f"{t:<6} {v10_results[t]['loss']:<10.4f} {ol:<10.4f} {cl:<10.4f} {preds[t].mean():<10.4f} {train_rate[t]:<10.3f}")
    log.info(f"\nV11 avg cal OOF: {avg_cal:.4f} (V10: 0.6038, diff: {avg_cal-0.6038:+.4f})")
