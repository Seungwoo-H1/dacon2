"""V118: V115 pipeline WITH test predictions and submission generation

V115 had isotonic calibration but only saved OOF, not test submissions.
V118 extends V115 to:
1. Track test predictions per fold + seed
2. Average test predictions across folds/seeds
3. Apply isotonic calibration (trained on OOF) to test predictions
4. Mean-match + clip
5. Save submission CSV + OOF CSV

This should produce a submission with OOF ≈ 0.5476 and better LB generalization.
"""
...
def add_personalization(df, feature_cols, fit_stats=None, for_test=False):
    """Add personalization (z-score) features.
    
    If fit_stats is provided, use them (for test set consistency).
    Otherwise compute stats from this df (for train set fitting).
    """
    personal_cols = []
    df = df.copy()
    all_stats = {}  # col -> {mean, std}
...
def train_cv_model(feat, feat_tst, cols, y, seeds, cfg, n_folds=5):
    ...
    Args:
        feat: training dataframe (with all features)
        feat_tst: test dataframe (with same columns as feat)
    ...
    Returns:
        oof: (n_samples, n_seeds) OOF predictions
        test_preds: (n_test, n_seeds) test predictions
    """
...
def isotonic_calibrate_from_oof(oof_preds, y_true):
    """Train isotonic regression on OOF predictions and true labels."""
...
feat_test_z, zscore_cols_test, _ = add_personalization(feat_test, feat_cols_raw, fit_stats=fit_stats, for_test=True)
...
# ============================================================
# Run experiments with TEST predictions
# ============================================================
log.info(f"\n--- Multi-strategy experiments (with test preds) ---")
log.info(f"  {len(CFGS)} configs × 4 n_feat × {len(SEEDS)} seeds × 3 strategies × 7 targets")

all_results = {}  # target -> {method, oof, cal_oof, ll, test_preds, iso_model}
...
for target in TARGETS:
    ...
    # Strategy A: Base — rank on raw features (personalization applied later)
    ...
    for cfg_name, cfg in CFGS.items():
        for n_feat in [8, 10, 15, 20]:
            ...
            iso, ok = isotonic_calibrate_from_oof(oof_avg, y)
            if ok:
            ...
            if ll < best_cal:
                ...
                best_test = iso_cal_test.copy() if ok else iso_cal_oof.copy()
    ...
    feat_test_pair = add_pairwise_interactions(feat_test_z.copy(), top_features)[0]
    ...
    # Re-rank with pairwise features
    ...
    feat_test_trans = add_transformed_features(feat_test_z.copy(), ranked_lgb[target][:15])[0]
...
oof_path = DATA / f'oof_v118_{ts}.csv'
...
sub_path = SUBMIT / f'submission_v118_{ts}.csv'
...
with open(EXPERIMENTS / f'v118_{ts}.json', 'w') as f:
    json.dump(exp_log, f, indent=2, default=str)

log.info(f"  Log: {EXPERIMENTS / f'v118_{ts}.json'}")
