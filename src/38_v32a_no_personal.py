"""
V32-A — Rolling mean only, no personalization
Test whether rolling alone (without personalization) is better or worse
"""
...
def build_features():
    """Build features: base + rolling mean, NO personalization."""
    ...
    # Rolling mean only (3d, 7d) — NO personalization
    rolling_cols = []
    ...
    log.info(f"  After rolling: {len(num_cols) + len(rolling_cols)} total")
    
    return merged, num_cols + rolling_cols
...
def main():
    log.info("V32-A: Rolling mean ONLY (no personalization)")
    ...
    log.info(f"  V32-A Avg Cal OOF: {avg:.4f}")
