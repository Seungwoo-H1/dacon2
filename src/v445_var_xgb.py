"""
V445 — Z-Score + Baseline Poly + Per-Subject Variance + XGB Base

Hypothesis: V443의 z-score+baseline poly(student 0.604)가 현재 최고.
V445는:
1. Per-subject variance features: 각 feature의 subject 내 variance, skewness, kurtosis
2. XGB base learners with multiple seeds alongside LGBM
3. Hard feature selection: top-30 per fold (V443은 25-30)
4. Quantile features: per-subject feature percentile within subject distribution
5. Ensemble: LGBM seeds + XGB seeds → weighted average
"""
...
V445_NFEAT = {'Q1': 25, 'Q2': 25, 'Q3': 22, 'S1': 30, 'S2': 28, 'S3': 30, 'S4': 28}

SEED = 42
N_FOLDS = 5
N_SEEDS = 10  # 10 LGBM + 10 XGB = 20 seeds total
...
def main():
    ...
    log.info("=" * 70)
    log.info("V445 — Z-Score + Baseline Poly + Per-Subject Variance + XGB Base")
    log.info("=" * 70)
    ...
    # ===== Per-Subject Variance Features =====
    log.info("  Adding per-subject variance features...")
    subj_vars = {}
    for col in num_cols:
        g = train_df.groupby('subject_id')[col]
        subj_vars[f'var_{col}'] = g.transform('var')
        subj_vars[f'skew_{col}'] = g.transform(lambda x: x.skew())
        subj_vars[f'kurt_{col}'] = g.transform(lambda x: x.kurtosis())
    zscore_train = pd.concat([zscore_train, pd.DataFrame(subj_vars, index=train_df.index)], axis=1)

    subj_vars_test = {}
    for col in num_cols:
        g = test_df.groupby('subject_id')[col]
        subj_vars_test[f'var_{col}'] = g.transform('var')
        subj_vars_test[f'skew_{col}'] = g.transform(lambda x: x.skew())
        subj_vars_test[f'kurt_{col}'] = g.transform(lambda x: x.kurtosis())
    zscore_test = pd.concat([zscore_test, pd.DataFrame(subj_vars_test, index=test_df.index)], axis=1)

    # ===== Quantile Features =====
    log.info("  Adding quantile features...")
    for col in num_cols:
        # Per-subject percentile (0-1)
        tr_mean = train_df.groupby('subject_id')[col].transform('mean')
        tr_std = train_df.groupby('subject_id')[col].transform('std').fillna(0).replace(0, 1)
        tr_median = train_df.groupby('subject_id')[col].transform('median')

        # How far is this value from the median in terms of IQR (using std as proxy)
        zscore_train[f'q_{col}'] = (train_df[col] - tr_median) / tr_std
        zscore_test[f'q_{col}'] = (test_df[col] - tr_median) / tr_std
    ...
    for t_idx, target in enumerate(TARGETS):
        ...
        # LGBM seeds (first half)
        for s in range(N_SEEDS // 2):
            sk = SEED + s * 7 + t_idx
        ...
        # XGB seeds (second half)
        skf_xgb = GroupKFold(n_splits=N_FOLDS)
        xgb_cfg = {'n_estimators': 1500, 'max_depth': 4, 'learning_rate': 0.05,
                    'subsample': 0.7, 'colsample_bytree': 0.7, 'reg_alpha': 1.0,
                    'reg_lambda': 5.0, 'min_child_weight': 5, 'gamma': 0.0,
                    'n_jobs': 1, 'verbosity': 0}
        for s in range(N_SEEDS // 2):
            sk = SEED + (s + 1) * 11 + t_idx
            seed_oof = np.zeros(len(train_df))
            seed_test = np.zeros(len(test_df))

            for fold, (tr_idx, va_idx) in enumerate(skf_xgb.split(X_all, y, groups_arr)):
                x_train, y_train = X_all[tr_idx], y[tr_idx]
                x_val = X_all[va_idx]

                spw = max(((y_train == 0).sum()) / max((y_train == 1).sum(), 1), 0.1)
                mm = XGBClassifier(**xgb_cfg, random_state=sk,
                    scale_pos_weight=spw, tree_method='hist')
                mm.fit(x_train, y_train)

                seed_oof[va_idx] = mm.predict_proba(x_val)[:, 1]
                seed_test += mm.predict_proba(X_test_all)[:, 1] / N_FOLDS
                del mm
                gc.collect()

            oof_seed_arr[:, N_SEEDS // 2 + s] = np.clip(seed_oof, 0.001, 0.999)
            test_seed_arr[:, N_SEEDS // 2 + s] = np.clip(seed_test, 0.001, 0.999)

        all_oof_seed[target] = oof_seed_arr
        all_test_seed[target] = test_seed_arr
        avg_oof = log_loss(y, oof_seed_arr.mean(axis=1))
        log.info(f"{target}: ensemble_oof={avg_oof:.5f} ({N_SEEDS} seeds)")
    ...
    sub_path = SUBMIT / f"submission_v445_var_xgb_{ts}.csv"
    sub.to_csv(sub_path, index=False)
    log.info(f"Saved: {sub_path}")

    meta_data = {
        'version': 'V445',
        'name': 'Z-Score + Baseline Poly + Per-Subject Variance + XGB Base',
    ...
    meta_path = EXPERIMENTS / f'v445_{ts}.json'
