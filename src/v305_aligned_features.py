"""
V305 — Train on train_features_clean_v60.parquet (73 features, identical train/test)
Uses V302's logic but with proper aligned features.

Key: 73 common features between train and test (all mBle/mGps/mWifi/mUsageStats included!)
"""
...
# V53 SWEEP configs (same as V302)
V53_SWEEP = {
...
def train_cal_100(feat, y, group, top_k, cfg, n_seeds):
    """Train with GroupKFold + calibration + multi-seed."""
...
def train_on_full_for_test(feat_train, y, top_k, feat_test, cfg, n_seeds):
    """Train on full train set and predict test."""
...
def main():
    t0 = time.time()
    print("=" * 70)
    print("V305 — V303 with IDENTICAL train/test features (73 cols)")
    print("=" * 70)
    
    # Use gen_v60_train_test output (73 features, identical train/test)
    # and merge labels to get targets
    feat = pd.read_parquet(DATA / 'train_features_clean_v60.parquet')
    feat_test = pd.read_parquet(DATA / 'test_features_clean_v60.parquet')
    
    # Merge labels to train
    labels = pd.read_csv(DATA.parent / 'data_raw' / 'ch2026_metrics_train.csv', parse_dates=['sleep_date', 'lifelog_date'])
    labels['date'] = pd.to_datetime(labels['lifelog_date']).dt.date
    feat = feat.merge(labels[['subject_id', 'date', 'sleep_date', 'lifelog_date'] + TARGETS], 
                     on=['subject_id', 'date'], how='left')
    # Keep only rows that have labels (450 rows)
    feat = feat.dropna(subset=TARGETS).reset_index(drop=True)
    print(f'  After label merge: {feat.shape}')
    
    # Merge sleep_date, lifelog_date for test
    sample = pd.read_csv(DATA.parent / 'data_raw' / 'ch2026_submission_sample.csv', parse_dates=['sleep_date', 'lifelog_date'])
    sample['date'] = sample['lifelog_date'].dt.date
    if 'sleep_date' not in feat_test.columns:
        feat_test = feat_test.merge(sample[['subject_id', 'date', 'sleep_date', 'lifelog_date']], 
                                    on=['subject_id', 'date'], how='left')
    ...
    print("\nComputing top-K features per target...")
    top_k_sets = {}
    for t in TARGETS:
        y = feat[t].values
        k = V53_SWEEP[t]['n_feat']
        top_k_sets[t] = importance_select(feat, y, feat_cols, k)
        print(f"  {t}: K={k} from {len(feat_cols)} -> selected {k}")
    ...
    print(f"V304: 0.64646 (25 common features)")
    print(f"Δ vs V304: {avg - 0.64646:+.5f}")
    ...
    for t in TARGETS:
        ...
        print(f"  {t}: mean={pred.mean():.4f}, std={pred.std():.6f}, nunique={np.unique(pred).shape[0]}")
    ...
    # Verification: ensure no all-zero or low-std predictions
    print("\nSubmission verification:")
    for t in TARGETS:
        vals = submit[t].values
        print(f"  {t}: std={vals.std():.6f}, nunique={submit[t].nunique()}, min={vals.min():.4f}, max={vals.max():.4f}, mean={vals.mean():.4f}")
    ...
    with open(EXPERIMENTS / f'v305_{ts2}.json', 'w') as f:
