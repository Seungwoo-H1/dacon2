"""
V332 — V329 + 30 Seeds + C=500 (V313 Lessons Applied)

Hypothesis: V313 proved 30 seeds + C=500 is powerful. But V313 LB failed
due to OOF-LB gap inflation. V332 applies this to V329's stronger features
with the same C=500 + 30 seeds combination.

Key: C=500 meta-learner, 30 seeds, V329 feature set.
"""
...
SEED = 42
N_FOLDS = 5
N_SEEDS = 30  # V332: 30 seeds (V313 lesson)
META_C = 500.0  # V332: C=500 (V313 lesson)
...
def main():
    ...
    log.info("=" * 70)
    log.info("V332 — V329 + 30 Seeds + Meta C=500 (V313 Lessons)")
    log.info("30 seeds × GroupKFold 5-fold → LR meta (C=500)")
    log.info("=" * 70)
    ...
    for t in TARGETS:
        ...
        for si in range(N_SEEDS):
            ...
            if si < 3 or si % 10 == 0 or si == N_SEEDS - 1:
                ...
                log.info(f"    Seed {si:2d}: OOF={s_oof:.5f} (30 seeds, C=500)")
    ...
    log.info(f"\n{'='*70}")
    log.info(f"V332 RESULTS (30 Seeds + Meta C=500)")
    log.info(f"{'='*70}")
    for t in TARGETS:
    ...
    log.info(f"  V329:  0.54365 | V328: 0.56298 | V313: 0.59512")
    ...
    sub_path = SUBMIT / f"submission_v332_30seeds_c500_{ts}.csv"
    sub.to_csv(sub_path, index=False)
    log.info(f"Saved submission: {sub_path}")
    
    meta_data = {
        'version': 'V332',
        'name': 'V329 + 30 Seeds + Meta C=500 (V313 Lessons)',
