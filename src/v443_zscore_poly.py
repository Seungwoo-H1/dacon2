"""
V443 — Z-Score + Baseline Polynomial + Per-Target Feature Optimization

Hypothesis: V442의 z-score 표준화가 student 0.616을 만들었음.
V443은:
1. Z-score features + baseline polynomial (baseline, baseline², log)
2. Per-target feature count optimization (S targets 더 많은 features 허용)
3. Z-score × baseline interaction features
4. Stronger regularization for Q targets (더 noisy)
"""
...
# V443: increased feature counts for S targets
...
def main():
    ...
    log.info("=" * 70)
    log.info("V443 — Z-Score + Baseline Poly + Per-Target Opt")
    log.info("=" * 70)
    ...
    # Z-score × baseline interactions
    log.info("  Adding z×baseline interactions...")
    zbase_train = pd.DataFrame(index=train_df.index)
    zbase_test = pd.DataFrame(index=test_df.index)
    for t in TARGETS:
        subject_ids = np.unique(train_subjects)
        subject_baseline = {}
        for sid in subject_ids:
            mask = train_subjects == sid
            s_y = train_df.loc[mask, t].values
            n_samples = mask.sum()
            global_rate = train_df[t].mean()
            subj_rate = s_y.mean() if n_samples > 0 else global_rate
            subject_baseline[sid] = 0.7 * subj_rate + 0.3 * global_rate

        for col in num_cols:
            z_col = f'z_{col}'
            if z_col in zscore_train.columns:
                train_bl = np.array([subject_baseline[sid] for sid in train_subjects])
                test_bl = np.array([subject_baseline[sid] for sid in test_df['subject_id'].values])
                zbase_train[f'zb_{t}_{col}'] = zscore_train[z_col] * train_bl
                zbase_test[f'zb_{t}_{col}'] = zscore_test[z_col] * test_bl

    # Baseline polynomial features
    log.info("  Adding baseline polynomial features...")
    poly_train = pd.DataFrame(index=train_df.index)
    poly_test = pd.DataFrame(index=test_df.index)
    for t in TARGETS:
        train_bl = np.array([subject_baseline[sid] for sid in train_subjects])
        test_bl = np.array([subject_baseline[sid] for sid in test_df['subject_id'].values])
        poly_train[f'bl2_{t}'] = train_bl ** 2
        poly_test[f'bl2_{t}'] = test_bl ** 2
        poly_train[f'logbl_{t}'] = np.log1p(train_bl)
        poly_test[f'logbl_{t}'] = np.log1p(test_bl)

    # Combine all features
    all_train_features = pd.concat([train_df[num_cols], zscore_train, zbase_train, poly_train], axis=1)
    all_test_features = pd.concat([test_df[num_cols], zscore_test, zbase_test, poly_test], axis=1)

    log.info(f"  Train features: {all_train_features.shape[1]}, Test features: {all_test_features.shape[1]}")
    ...
    for t_idx, target in enumerate(TARGETS):
        ...
        log.info(f"{target}: zscore_poly_oof={avg_oof:.5f}")
    ...
    log.info(f"  V442 V339: 0.60315 (for comparison)")
    log.info(f"{'='*70}")
    ...
    sub_path = SUBMIT / f"submission_v443_zscore_poly_{ts}.csv"
    sub.to_csv(sub_path, index=False)
    log.info(f"Saved: {sub_path}")

    meta_data = {
        'version': 'V443',
        'name': 'Z-Score + Baseline Poly + Per-Target Opt',
    ...
    meta_path = EXPERIMENTS / f'v443_{ts}.json'
