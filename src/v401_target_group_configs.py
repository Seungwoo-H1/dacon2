"""
V401 — Target-Group Specific Model Configs

Hypothesis: Q targets have fundamentally different characteristics than S targets.
Q targets: more noise, lower positive rate, different feature correlations
S targets: more stable, higher positive rate

V308/V392 use same LGBM config for all targets → suboptimal for Q targets
V401: different LGBM configs for Q vs S groups

Q-group: lower num_leaves, lower learning_rate, higher n_estimators (more regularization)
S-group: use original configs
"""
...
# All configs available in one dict
ALL_CFGS = {
    # V308 default configs
    ...
    # V401 Q-group ultra-conservative
    'ultra_deep': {'num_leaves': 8, 'max_depth': 3, 'learning_rate': 0.01, 'n_estimators': 2000,
...
# Q targets use ultra_deep config
Q_CONFIG = 'ultra_deep'
S_CONFIGS = {
    'S1': 'wide', 'S2': 'deep', 'S3': 'safety', 'S4': 'wide',
}
Q_NFEAT = {'Q1': 19, 'Q2': 14, 'Q3': 11}
S_NFEAT = {'S1': 21, 'S2': 19, 'S3': 23, 'S4': 20}
...
def main():
    ...
    log.info("=" * 70)
    log.info("V401 — Target-Group Specific Model Configs")
    log.info("Hypothesis: Q-group needs different LGBM config than S-group")
    log.info("V392: same config for all → Q student ~0.80-0.82")
    log.info("V401: Q→ultra_deep, S→original")
    log.info("=" * 70)
    ...
    for t in TARGETS:
        ...
        n_feat = Q_NFEAT[t] if t.startswith('Q') else S_NFEAT[t]
        meta_c = META_C_PER_TARGET[t]

        if t.startswith('Q'):
            cfg_name = Q_CONFIG
        else:
            cfg_name = S_CONFIGS[t]
    ...
    log.info(f"\n{'='*70}")
    log.info(f"V401 RESULTS (Q→ultra_deep, S→original)")
    log.info(f"{'='*70}")
    for t in TARGETS:
        if t.startswith('Q'):
            cfg_used = Q_CONFIG
        else:
            cfg_used = S_CONFIGS[t]
        log.info(f"  {t}: Meta={per_target_oof[t]:.5f}, Student={per_target_student[t]:.5f}, C={META_C_PER_TARGET[t]}, cfg={cfg_used}")
    ...
    sub_path = SUBMIT / f"submission_v401_target_group_configs_{ts}.csv"
    sub.to_csv(sub_path, index=False)

    meta_data = {
        'version': 'V401',
        'name': 'Target-Group Specific Model Configs (Q→ultra_deep)',
    ...
    meta_path = EXPERIMENTS / f'v401_{ts}.json'
