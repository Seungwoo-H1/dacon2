"""
V441 — Baseline Feature + Deep Interaction + Rolling Statistics

Hypothesis: V440의 interaction features(feat*subj_mean)가 student 0.620을 만들었음.
하지만 interaction이 linear(feat*mean)에 그침. V441은:
1. Higher-order interactions: feat², baseline², feat×baseline, feat×subj_std
2. Rolling statistics per subject (7-day, 14-day windows)
3. Baseline as polynomial feature (baseline, baseline², log(baseline+1))
4. Target group interaction features (Q-group avg, S-group avg from seed predictions)
"""
...
def deep_interact(df, subject_col='subject_id'):
    """Add deep interaction features."""
    log.info("  Adding deep interaction features...")
    num_cols = [c for c in df.columns
                if c not in META_COLS | set(TARGETS)
                and np.issubdtype(df[c].dtype, np.number)]

    # Subject statistics
    subj_stats = df.groupby(subject_col)[num_cols].agg(['mean', 'std', 'min', 'max']).fillna(0)
    subj_stats.columns = [f'ss_{c}_{stat}' for c, stat in subj_stats.columns]

    # High-order interactions: feat * feat (pairwise for top domains)
    # Only interact with subject stats (reduced dimension)
    interactions = []
    interaction_names = []

    # feat * subj_mean (like V440)
    for col in num_cols:
        if col in META_COLS | set(TARGETS):
            continue
        m = df.groupby(subject_col)[col].transform('mean')
        interactions.append(df[col] * m)
        interaction_names.append(f'int_{col}_ss_mean')

    # feat * subj_std
    for col in num_cols:
        if col in META_COLS | set(TARGETS):
            continue
        s = df.groupby(subject_col)[col].transform('std')
        interactions.append(df[col] * s)
        interaction_names.append(f'int_{col}_ss_std')

    # Baseline interactions: baseline * baseline, baseline^2, log(baseline+1)
    target_bases = [f'{t}_subject_baseline' for t in TARGETS]
    for tb in target_bases:
        if tb in df.columns:
            interactions.append(df[tb] ** 2)
            interaction_names.append(f'base2_{tb}')
            interactions.append(np.log1p(df[tb]))
            interaction_names.append(f'logbase_{tb}')

    # feat^2 (only for top domains to avoid explosion)
    domain_feats = {
    ...
    for domain, feats in domain_feats.items():
        ...
        for f in existing:
            interactions.append(df[f] ** 2)
            interaction_names.append(f'q_{f}')
            if len(existing) > 1:
                for f2 in existing:
                    interactions.append(df[f] * df[f2])
                    interaction_names.append(f'cross_{f}_{f2}')

    if len(interactions) > 0:
        int_df = pd.DataFrame(np.column_stack(interactions), columns=interaction_names)
        df = pd.concat([df, int_df], axis=1)
        log.info(f"    Added {len(interactions)} interaction features (total: {df.shape[1]})")
...
def main():
    ...
    log.info("=" * 70)
    log.info("V441 — Deep Interaction + Rolling Statistics")
    log.info("=" * 70)
    ...
    # ===== Add deep interaction features =====
    train_df = deep_interact(train_df)
    test_df = deep_interact(test_df)
    ...
    for t_idx, target in enumerate(TARGETS):
        ...
        log.info(f"{target}: deep_int_oof={avg_oof:.5f}")
    ...
    log.info(f"  V440 V339: 0.60769 (for comparison)")
    log.info(f"{'='*70}")
    ...
    sub_path = SUBMIT / f"submission_v441_deep_int_{ts}.csv"
    sub.to_csv(sub_path, index=False)
    log.info(f"Saved: {sub_path}")

    meta_data = {
        'version': 'V441',
        'name': 'Deep Interaction + Rolling Statistics',
        ...
        'n_seeds': N_SEEDS,
        'meta_type': 'xgb_weighted_cross_deep_int',
    ...
    meta_path = EXPERIMENTS / f'v441_{ts}.json'
