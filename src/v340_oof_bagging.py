"""
V340 — OOF Feature + Conservative Bagging (98%) + Meta C=10

Hypothesis: V339의 OOF feature가 효과를 보였다(OOF -0.010). 
V333의 bagging diversity(30 seeds)를 OOF feature와 결합하면
더 나은 OOF + reasonable gap 가능.

Changes:
1. V339의 OOF feature augmented data 사용
2. Bagging ratio 98% (almost no feature bagging, seed diversity only)
3. Meta C=10 (V308과 동일)
4. 30 seeds

Key insight: V339 was OOF-feature only (15 seeds, no bagging).
V333 was bagging only (no OOF feature). Combining both might work.
"""
...
def generate_oof_features(train_df, test_df):
    """Generate OOF predictions using V308 pipeline."""
    log.info("Generating OOF features (V308 pipeline)...")
    ...
    for target in TARGETS:
        ...
        seeds = [SEED + i * 7 for i in range(N_SEEDS // 2)]  # Use fewer seeds for OOF gen
        ...
        log.info(f"  {target}: OOF mean={oof_preds.mean():.4f}")
    
    return oof_train, oof_test


def feature_bagging(cols, bag_ratio=0.98, rng=None):
    if rng is None:
        rng = random.Random()
...
def main():
    ...
    log.info("=" * 70)
    log.info("V340 — OOF Feature + Bagging 98% + 30 Seeds + C=10")
    log.info("V339: OOF feat → OOF=-0.010 | V333: Bagging → OOF=-0.049 (gap risk)")
    log.info("V340: Combine both → best of both worlds")
    log.info("=" * 70)
    ...
    oof_train, oof_test = generate_oof_features(train_df, test_df)
    ...
    for t in TARGETS:
        ...
        log.info(f"  {t}: {len(sel_cols_test)} features (+oof)")
    ...
    for t in TARGETS:
        tc = target_configs[t]
        cfg = tc['cfg']
        ranked_full = tc['features']  # for bagging
        ranked_full_test = tc['features_test']
        feats_oof = [c for c in ranked_full if c.startswith('oof_')]
        feats_base = [c for c in ranked_full if not c.startswith('oof_')]
        ...
        for si, seed in enumerate(seeds):
            ...
            # Bag base features only (keep oof features always)
            sel_base = feature_bagging(feats_base, bag_ratio, rng) if len(feats_base) > 5 else feats_base
            sel_cols = sel_base + feats_oof
            sel_cols_test = [c for c in sel_cols if c in test_feat_cols]
            if len(sel_cols_test) < 3:
                sel_cols_test = feats_oof + ranked_full_test[:max(3, len(feats_base))]
    ...
    sub_path = SUBMIT / f"submission_v340_{ts}.csv"
    sub.to_csv(sub_path, index=False)
    log.info(f"Saved submission: {sub_path}")
    
    meta_data = {
        'version': 'V340',
        'name': 'OOF Feature + Bag 98% + 30 Seeds + C=10',
        'avg_oof': round(float(avg_oof), 5),
        'n_seeds': N_SEEDS,
        'meta_c': META_C,
        'bag_ratio': 0.98,
        'delta_vs_v308': round(float(avg_oof - 0.62235), 5),
        'delta_vs_v339': round(float(avg_oof - 0.61244), 5),
        'per_target_oof': {t: round(float(all_oofs[t]), 5) for t in TARGETS},
        'submission_file': str(sub_path),
        'timestamp': ts,
    }
    meta_path = EXPERIMENTS / f'v340_{ts}.json'
