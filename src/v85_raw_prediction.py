"""
V85 — Full Pipeline: OOF + Test with Raw Prediction

Key changes from V84:
- Test data prediction (raw, no mean_match on test)
- Test logloss calculation for leaderboard estimation
- Per-target raw mean tracking for distribution shift analysis

Architecture:
- V84 OOF: 0.6664 (same as V53 baseline)
- Test prediction with raw model output
"""
...
def train_full_and_predict(train_p, test_p, sel_cols, y, cfg_name, target):
    """
    Train models on FULL train data (V84 style: 1 fold = 1 model, random_state=42),
...
def main():
    t_start = time.time()
    log.info("=" * 70)
    log.info("V85 — Full Pipeline: OOF + Test with Raw Prediction")
    log.info("=" * 70)
    ...
    # Personalization on train (fit) + test (transform)
    ...
    # Test rate (for reference only — NOT used for prediction)
    test_rate = {t: test_p[t].mean() for t in TARGETS}
    log.info(f"  Test rates:  { {t: f'{r:.3f}' for t, r in test_rate.items()} }")
    
    # V85: per-target config + GroupKFold + test prediction
    target_results = {}
    ...
    for target in TARGETS:
        ...
        # Test loss (for reference — leaderboard estimation)
        y_test = test_p[target].values.astype(np.float64)
        test_loss_raw = log_loss(y_test, test_raw, labels=[0, 1])
        ...
        log.info(f"  Test Raw logloss: {test_loss_raw:.4f}")
    ...
    avg_test_raw = np.mean([r['test_loss_raw'] for r in target_results.values()])
    
    log.info(f"\n{'='*70}")
    log.info("V85 RESULTS")
    log.info(f"{'='*70}")
    log.info(f"{'Target':<8} {'OOF Raw':<12} {'OOF Cal':<12} {'Test Raw':<12} {'Δ(OOF)'}")
    log.info(f"{'-'*56}")
    for t in TARGETS:
        r = target_results[t]
        log.info(f"{t:<8} {r['oof_loss_raw']:<12.4f} {r['oof_loss_cal']:<12.4f} {r['test_loss_raw']:<12.4f} {r['oof_loss_raw']-r['oof_loss_cal']:+.4f}")
    
    log.info(f"{'='*70}")
    log.info(f"  AVG OOF Raw:  {avg_oof_raw:.4f}")
    log.info(f"  AVG OOF Cal:  {avg_oof_cal:.4f}")
    log.info(f"  AVG Test Raw: {avg_test_raw:.4f}")
    log.info(f"  V53 reported: 0.5479 (reported as Cal OOF)")
    log.info(f"  V10 baseline: 0.6038")
    log.info(f"  Δ vs V10 (OOF raw):  {avg_oof_raw - 0.6038:+.4f}")
    ...
    sp_raw = SUBMIT_DIR / f'submission_v85_raw_{ts}.csv'
    submission_raw.to_csv(sp_raw, index=False)
    log.info(f"  Raw submission saved: {sp_raw}")
    for t in TARGETS:
        log.info(f"    {t}: mean={submission_raw[t].mean():.4f} (train={train_rate[t]:.4f}, test={test_rate[t]:.4f})")
    ...
    sp_cal = SUBMIT_DIR / f'submission_v85_cal_{ts}.csv'
    submission_cal.to_csv(sp_cal, index=False)
    log.info(f"  Cal submission (OOF mean_match only) saved: {sp_cal}")
    
    # Save meta
    meta = {
        'version': 'V85',
        'method': 'GroupKFold_5fold_raw_prediction_test',
        'mean_match': 'OOF_only (NOT on test)',
        'per_target': {},
        'avg_oof_raw': float(avg_oof_raw),
        'avg_oof_cal': float(avg_oof_cal),
        'avg_test_raw': float(avg_test_raw),
    ...
    for t in TARGETS:
        ...
        meta['per_target'][t] = {
            ...
            'test_raw': float(r['test_loss_raw']),
            'train_rate': float(train_rate[t]),
            'test_rate': float(test_rate[t]),
            'test_mean_pred': float(submission_raw[t].mean()),
        }
    
    meta_path = ROOT / 'experiments' / 'v85_raw_prediction.json'
