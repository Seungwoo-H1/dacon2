...
Hypothesis: per-target meta C (Q=10, S=100) + 30 seeds (double V308)
maintains calibration while improving OOF through more diversity.

V392: per-target C + 15 seeds → meta 0.617, student 0.692 (same as V308)
V376: 30 seeds (no per-target C) → meta 0.595, student 0.692, but LB gap risk

V396: per-target C + 30 seeds → try to get V376-level OOF with V392's calibration
"""
...
def main():
    ...
    log.info("=" * 70)
    log.info("V396 — Per-Target Meta C + 30 Seeds")
    log.info("Hypothesis: 30 seeds with per-target meta C improves OOF while maintaining calibration")
    log.info("V392: per-target C + 15 seeds → meta 0.617, student 0.692 (same as V308)")
    log.info("V376: 30 seeds → meta 0.595, student 0.692 (gap risk)")
    log.info("V396: per-target C + 30 seeds → calibrate better with more seeds")
    log.info("=" * 70)
    ...
    log.info(f"N_SEEDS: {N_SEEDS} (double V308)")
    ...
    for t in TARGETS:
        ...
        log.info(f"    Config: {cfg_name}, n_feat: {n_feat}, meta C: {meta_c}, seeds: {N_SEEDS}")
        ...
        log.info(f"    {t}: Meta OOF={ll:.5f}, Student OOF={student_oof:.5f} (C={meta_c}, {N_SEEDS} seeds)")
    ...
    log.info(f"\n{'='*70}")
    log.info(f"V396 RESULTS (Per-Target Meta C + 30 Seeds)")
    log.info(f"{'='*70}")
    for t in TARGETS:
    ...
    for t in TARGETS:
        ...
        meta_t = LogisticRegression(C=META_C_PER_TARGET[t], max_iter=2000, random_state=SEED)
    ...
    sub_path = SUBMIT / f"submission_v396_per_target_meta_c_30seeds_{ts}.csv"
    ...
    meta_data = {
        'version': 'V396',
        'name': 'Per-Target Meta C + 30 Seeds',
    ...
    meta_path = EXPERIMENTS / f'v396_{ts}.json'
