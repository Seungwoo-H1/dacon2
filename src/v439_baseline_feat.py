"""
V439 — Baseline Subtraction + Baseline as Feature + Weighted Cross-Target

Hypothesis: 두 가지 개선
1. Baseline을 prediction에서 빼는 대신, baseline을 explicit feature로 추가
   → model이 baseline을 학습할 수 있음
2. Cross-target features에 correlation-based weighting 적용
   → correlated targets는 더 높은 weight, uncorrelated는 낮춤

V435: baseline subtraction + all cross → gap=1.32x, V339=0.61920
V439: baseline feature + weighted cross → gap↓ meta↓
"""
...
def main():
    ...
    log.info("=" * 70)
    log.info("V439 — Baseline as Feature + Weighted Cross-Target")
    log.info("=" * 70)
    ...
    # ===== Phase 1: LGBM base WITHOUT baseline subtraction =====
    # Instead, add baseline as explicit feature
    all_oof_seed = {}
    all_test_seed = {}
    ...
    for t_idx, target in enumerate(TARGETS):
        ...
        # Add baseline as feature to feature set
        feat_cols = get_feature_cols(train_df)
        feat_cols = remove_leak(feat_cols, target)

        # Check if baseline feature already exists
        base_cols = ['subject_id', 'lifelog_date', 'sleep_date', 'date']
        baseline_feat = f'{target}_subject_baseline'
        ...
        # Build X with baseline as feature
        ...
        for s in range(N_SEEDS):
            ...
            for fold, (tr_idx, va_idx) in enumerate(skf.split(X_all, y, groups_arr)):
                ...
                ds_train = lgb.Dataset(x_train, label=y_train,
                    feature_name=[sanitize_col(c) for c in top_features + [baseline_feat]])
                ds_val = lgb.Dataset(x_val, label=y[va_idx],
                    feature_name=[sanitize_col(c) for c in top_features + [baseline_feat]], reference=ds_train)
        ...
        log.info(f"{target}: baseline_feat_oof={avg_oof:.5f}")
    ...
    # Correlation-based weights for cross-target
    # Q targets correlate with each other, S targets correlate with each other
    Q_TARGETS = ['Q1', 'Q2', 'Q3']
    ...
    for t_idx, target in enumerate(TARGETS):
        ...
        # Weighted cross-target: Q targets weighted by group
        cross_oof_list = []
        cross_test_list = []
        weights_list = []

        for i, t_cross in enumerate(TARGETS):
            if t_cross == target:
                continue
            if t_cross.startswith('Q') == target.startswith('Q'):
                # Same group: weight 1.0
                w = 1.0
            else:
                # Different group: weight 0.5 (uncorrelated)
                w = 0.5
        ...
        if len(cross_oof_list) > 0:
            cross_oof = np.column_stack(cross_oof_list)
            cross_test = np.column_stack(cross_test_list)
        else:
            cross_oof = np.zeros((len(y), 0))
            cross_test = np.zeros((len(test_df), 0))
    ...
    sub_path = SUBMIT / f"submission_v439_baseline_feat_{ts}.csv"
    sub.to_csv(sub_path, index=False)
    log.info(f"Saved: {sub_path}")

    meta_data = {
        'version': 'V439',
        'name': 'Baseline as Feature + Weighted Cross-Target',
        ...
        'n_seeds': N_SEEDS,
        'meta_type': 'xgb_weighted_cross_baseline_feat',
    ...
    meta_path = EXPERIMENTS / f'v439_{ts}.json'
